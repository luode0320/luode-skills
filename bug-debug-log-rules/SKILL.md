...
3. 如需继续展开，再读 `references/debug-log-cleanup.md`，需要规划日志生命周期和回收方式。
4. 需要对照边界或正反例时，再读 `references/debug-log-examples.md`，需要对照日志诊断正反例。
5. 输出调试日志方案、落点说明、Bug 根目录记录方案和清理要求，并更新到当前 Bug 根目录下的 `README.md`。
6. 日志加完后继续回到 `bug-runtime-debug-rules` 或 `bug-root-cause-rules` 使用证据定位；诊断完成后必须交给 `cleanup-format-review-rules` 检查清理状态。

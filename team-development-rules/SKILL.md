...
- 注释必须使用中文，重点解释“为什么这样做”；只要代码块存在大于等于 2 个步骤，就必须补充中文步骤注释，并用 `1.`、`2.`、`3.` 标识步骤。
...
- 所有测试资产统一存放在项目根目录 `test/` 下，不得在真实业务代码目录创建临时测试程序、测试脚本或补充验证代码。
- 每次新增测试任务，必须在 `test/` 下创建独立子目录，命名格式为 `yyyy-MM-DD HH:mm:ss + 测试任务中文简介`。
- 若同一需求需要多个测试程序或多轮独立验证，应分别创建多个测试任务子目录；每个子目录仅存放对应测试任务的脚本、代码、用例、模拟数据和验证说明，不得散落到其他目录。
- 每个测试任务子目录必须包含 `README.md`，说明本次测试的目的、执行方式、依赖数据、覆盖范围和验证结论。
...
- 交付与复盘检查时，必须核对新增或修改的代码块；只要代码块存在大于等于 2 个步骤，就应补充中文步骤注释，并用 `1.`、`2.`、`3.` 标识步骤。

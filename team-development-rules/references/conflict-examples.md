...
- 应命中：`api-contract-rules`、`api-contract-rules`、`api-contract-rules`、`logging-trace-rules`、`error-handling-rules`。
...
- 裁决：阻断进入测试域，先进入 `code-style-consistency-rules`。
...
- 应命中：`implementation-planning-rules`。
- 裁决：先进入 `implementation-planning-rules`，把当前优先闭环的文件落点、任务顺序和验证点拆清后，再进入编码基线域和代码位点域。
- 原因：阶段上仍处于“编码前实施规划”而不是“直接实现”；此时缺的是实施总览/实施周期，不是需求拆分或测试执行。

### 样例 11：需求拆分与实施规划同时被提起

- 当前信号：一个大需求既包含多个独立子系统，又要求顺便把开发步骤一起列出来。
- 应命中：`requirement-splitting-rules` + `implementation-planning-rules`。
- 裁决：先由 `requirement-splitting-rules` 拆出子系统 / 子项边界，只保留当前优先闭环；边界稳定后，再由 `implementation-planning-rules` 继续细化该优先子项的文件落点和执行步骤。
- 原因：拆分先回答“先做哪一块”；实施规划再回答“这块具体怎么落地”，两者不能倒置。

### 样例 14：测试和审核都完成，准备做最终放行

- 当前信号：测试结果已形成，审核结论已形成，团队想确认这次需求是否可以正式验收通过。
- 应命中：`delivery-summary-rules`。
- 裁决：先进入 `delivery-summary-rules`，基于需求主文档、验收标准、实施文档、测试结果和审核结果做逐条放行判断。
- 原因：这是后置验收阶段，不应再混回需求域或测试域。

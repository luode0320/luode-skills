...
> 本节遵循 `test-strategy-rules` 的《测试隔离红线（强制）》单一权威来源：严禁为测试污染生产代码（新增测试专用方法/数据/结构体字段）、不得通过改造生产代码“配合测试”制造回归通过结果、发现污染立即判定回归结论无效并阻断、回归自动化只用 `local` 环境（禁连 `test`/`prod`/`staging` 等非 local 服务）。本 skill 不重复展开，仅承接回归验证侧的落地。接口类回归统一遵循 `test-strategy-rules` 的《接口测试执行通道（强制）》：在 apifox「AI 团队」对应项目中执行（复用 apifox 测试套件），environment 只允许指向 local（localhost）。
...
2. 确认当前回归验证已经对应到中央约定的测试主文档，并核对回归测试、mock、stub、fake、fixture 和 helper 是否位于对应源码相对路径镜像；需要连接本地真实环境（数据库、缓存、消息队列、HTTP/RPC 上游等）做回归时，按 `test-strategy-rules` 的「本地环境配置发现与连接」去本地 `local` 配置文件读取连接信息，并遵守其隔离安全约束；不得改用 `test` / `prod` / `staging` 等非 local 环境连接。接口回归先确认被测服务已在本地启动、apifox environment 指向 localhost。
...
6. 接口回归范围确定后，在 apifox 中通过 `test-suite` 或 `test-case run` 执行（复用 apifox 测试套件），用例落在 apifox「AI 团队」对应项目；本地 go test/pytest 只跑代码级回归。
7. 回归范围未稳定前，不直接宣告任务具备交付条件。
...
- 通过：本次改动涉及的旧能力、关联链路、共享依赖和兼容行为在已判定回归范围内未发现新的破坏性问题；接口回归用例已保存到 apifox（caseId/suiteId 可追溯、可复跑）；中文测试主文档 已能清楚说明结论，根 `test/` 回归测试、同源码镜像的 mock/stub/fake 与证据路径均可追溯。

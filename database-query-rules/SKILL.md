...
4. **特别强调：GORM 查询、统计、更新、创建、保存必须显式写出 `Model(&models.X{})`，即使 `Find(&slice)` / `First(&obj)` 可以推导表名，也不允许依赖推导。**
5. 输出访问层落点、事务范围、锁策略、分页方式和性能风险说明。
6. 如果问题本质落回 schema 结构或缓存策略，停止停留在 query 层并转给相邻 skill。
...
- 当前 GORM 访问目标表不明确，或依赖目标变量推导导致读者无法一眼看出查询表模型。
...
- 若使用 GORM，归档或评审结论中需确认访问链路已显式声明目标模型，未依赖类型推导。

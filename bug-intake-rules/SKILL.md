...
description: 当用户描述报错、异常行为、结果不符、线上问题、偶发问题、接口异常、页面异常、数据错误或性能异常时触发。负责把 Bug 描述标准化，整理现象、影响范围、环境条件、期望结果和实际结果，并统一建立符合 `artifact-storage-rules` 约定的 Bug 根目录记录；不要用它代替根因定位、运行时调试或修复方案制定 skill。
---
...
- Bug 根目录落地时，除 `README.md` 外必须同步产出 SVG 流程图与 SVG 时序图。
...
3. 同步按 `artifact-storage-rules` 约定生成 `flow.svg`（流程图）与 `sequence.svg`（时序图）。
...
- 当前 Bug 根目录必须同时包含 `flow.svg` 与 `sequence.svg` 两张图，分别用于流程与时序表达；缺任一图视为 Bug 入口记录不完整。
- 如果只是轻量问题且后续马上进入复现，可以简化记录，但仍应继续沿用同一个 Bug 根目录。
- 进入最终回复前，必须联动 `artifact-delivery-gate-rules` 核对当前 Bug 根目录、`README.md`、`flow.svg` 与 `sequence.svg` 是否已经真实落盘；未落盘不得判定 Bug 入口记录完成。

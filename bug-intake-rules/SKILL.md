...
description: 当用户描述报错、异常行为、结果不符、线上问题、偶发问题、接口异常、页面异常、数据错误或性能异常时触发。负责把 Bug 描述标准化，整理现象、影响范围、环境条件、期望结果和实际结果；不要用它代替根因定位、运行时调试或修复方案制定 skill。
---
...
1. 先抽取异常现象，不要先猜根因。
2. 再整理影响范围、出现环境、触发条件和出现频率。
3. 区分期望结果与实际结果。
4. 判断当前信息是否已经足够进入复现和定位。
...
- 将标准化后的 Bug 入口信息记录到 `analysis/`。
- 归档内容至少包含现象、影响范围、环境条件、期望结果、实际结果、缺失项和下一步动作。
- 如果只是轻量问题且后续马上进入复现，可以简化记录，但关键信息仍要保留。

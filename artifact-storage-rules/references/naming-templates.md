...
- 6-review 风格回归：`doc/6-review/YYYY-MM-DD_HHmmss_<来源对象标识>_6-review.md`
- 历史验收标准：`doc/7-验收/YYYY-MM-DD_HHmmss_<来源对象标识>_验收标准.md`（只读）
- 历史最终验收：`doc/7-验收/YYYY-MM-DD_HHmmss_<来源对象标识>_最终验收.md`（只读）
...
- 需求与实施计划全量顺序实施方案是跨需求 / 跨来源对象的实施顺序总表，负责链接需求主文档、实施总览、实施周期和周期内最小任务，并记录总执行顺序、依赖、状态和阻断项；`AC-*` 完成条件直接放在实施计划中。
...
- 历史验收文档只读保留，不参与当前活动路由。
- 当前完成条件由实施计划中的 `AC-*` 表达，真实行为由测试负责，测试后仅执行 `6-review` 风格回归。
...
- 风格回归主文档：`doc/6-review/YYYY-MM-DD_HHmmss_<来源对象标识>_6-review.md`
- 风格回归汇总入口：`doc/6-review/README.md`

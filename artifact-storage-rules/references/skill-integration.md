...
- 需要判断过程产物是否已到退场条件、或需要时间阈值与四道前置守卫细则时，读取 `../artifact-storage-rules/references/lifecycle-policy.md` 与 `path-map.yaml` 中的 `process_doc_*` 策略键；不得在下游 skill 另设退场阈值或守卫条件
...
- 不建议在各自 `SKILL.md` 中重复维护完整的 `doc/1-架构/`、`doc/2-需求/`、`doc/3-实施/`、`doc/4-bugs/`、`doc/5-tests/`、活动 `doc/6-review/`、历史 `doc/6-审查/`、`doc/7-验收/`、`doc/`、`skills/` 模板说明。
...
- 若当前结论属于测试后的代码风格回归、目录归位、格式清理、语法检查或注释核对，必须归档到活动 `doc/6-review/`；历史 `doc/6-审查/` 仅供读取且不得新增或改写，根级 `review/` 不再作为正式入口，`analysis/` 只有在项目明确声明该目录时才允许作为正式入口。

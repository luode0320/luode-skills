...
- 当前可信度：高。本文以真实目录结构、各 skill 目录中的 `SKILL.md` / `references` / `agents`、字典生成脚本、静态页面和最近 Git 变更为主依据；对历史规划文档仅作辅助参考。
- 最近同步时间：`2026-04-09 15:07:57`
- 主要依据：
  - 根目录说明文档：`README.md`
  - 主规划文档：`编码skill.md`
  - 当前可检索字典：`字典.md`
...
- 弱参考说明：
  - `编码skill.md` 已由“审查稿”同步为“持续迭代稿”，并纳入 `code-context-resync-rules`。
  - `skill-dictionary/data.js` 已在本轮刷新（`2026-04-09 15:07:57`），当前统计为 `planned_total=63`、`implemented_total=63`、`seed_total=15`；后续变更后仍需手动刷新以防再次滞后。
...
| 编码基线域 | 所有编码任务默认生效的基础质量规则 | `code-minimal-change-rules`、`code-context-resync-rules`、`code-readability-rules`、`code-style-consistency-rules`、`naming-rules`、`comment-placement-granularity-rules`、`comment-completion-gate-rules` |
...
| 扩展种子 | 尚未完全并入主规划的辅助 skill | `doc`、`pdf`、`spreadsheet`、`code-review-automation-rules`、`context-compression-rules`、`find-skills`、`skill-split-preserve-rules`、`work-report-summary-rules` |
...
- `README.md` 仍显示“已落地 Skill=62、扩展种子 Skill=8”（统计时间点较早），而本轮刷新后的字典统计为“已落地 63、扩展种子 15”。
- `编码skill.md` 与 `字典.md` 已同步到当前规划状态，但根目录说明与部分历史描述仍可能滞后。
- 后续应保持“主规划文档、字典统计、README 概览”三者同步，避免新读者误判仓库状态。

### 8.2 生成产物已同步但仍可能再次滞后

- 本轮已执行 `python3 skill-dictionary/generate_dictionary.py`，`skill-dictionary/data.js` 与 `字典.md` 已同步到当前状态。

...
4. 建立或恢复当前新任务自己的 projection，成功持久化后立即调用 `update_plan`；仅投影持久化失败、会话归属冲突/不确定或执行状态不明时硬阻断，单纯 UI 同步失败时进入 `UI_SYNC_BLOCKED` 降级继续：保留磁盘 projection + 继续领域执行，下一检查点重试 UI，不得声称 UI 已恢复。
...
- 当前存在未决的 Plan Mode 选择、真实阻断或未知非幂等操作时停止，交接包必须保留这些状态，不能用“继续”覆盖；`UI_SYNC_BLOCKED` 仅表示 UI 同步通道不可用（磁盘投影已保存且归属明确），不阻断领域执行，交接包如实记录并在下一检查点重试 UI，不得声称 UI 已恢复。

...
4. 恢复并标记“编码许可状态”：若未见明确确认证据，默认 `unknown`。
5. 以“可继续执行”为目标，整理保留项、折叠项和剔除项。
6. 输出最小上下文包后，立即转交当前主域 skill 继续执行。
...
- 通过：压缩后信息更短但可直接继续执行，且关键目标、约束、风险、下一步完整保留；编码许可状态明确且可用于后续是否进入编码的判断。
- 驳回：压缩后虽然更短，但无法支持继续执行，或丢失关键约束导致方向错误；或未输出编码许可状态，导致压缩后直接误入编码。

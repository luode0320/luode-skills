...
- 在压缩后强制重新读取当前项目根目录 `AGENTS.md`，恢复仓库级硬规则、必命中 skill 和流程闸门。
...
- 压缩后即将继续执行，但仓库级 `AGENTS.md` 规则可能已经脱离当前上下文。

## 进入后先做什么

1. 先确认上下文压缩已经完成，而不是仍处在压缩前阶段。
2. 立即联动 `recent-context-bootstrap-rules`，补齐最近活动摘要与可用 skill 索引。
3. 强制重新读取当前项目根目录 `AGENTS.md`；若项目存在仓库级硬规则，必须先恢复这些规则，再继续任何领域动作。
...
5. 强制重新读取当前项目根目录 `AGENTS.md`；若未完成该动作，判定为压缩收口未完成，禁止继续主任务。
6. 输出压缩后的上下文包，至少包含：当前目标、已确认事实、关键约束、关键路径、待确认项、下一步动作、编码许可状态（`confirmed` / `not-confirmed` / `unknown`）、当前项目 `AGENTS.md` 已重载状态。
...
- 不得跳过当前项目根目录 `AGENTS.md` 的重新读取；若仓库级硬规则未恢复，禁止直接继续主任务。
...
- 通过：压缩后信息更短但可直接继续执行，且关键目标、约束、风险、下一步完整保留；编码许可状态明确；当前项目根目录 `AGENTS.md` 已重新读取且仓库级硬规则已恢复。
- 驳回：压缩后虽然更短，但无法支持继续执行，或丢失关键约束导致方向错误；或未输出编码许可状态；或未重新读取当前项目根目录 `AGENTS.md` 就直接继续主任务。
...
- 若压缩后补做了 `AGENTS.md` 规则恢复，应在压缩包中显式记录“`AGENTS.md` 已重载”结论。

...
执行命令、CLI、API、浏览器、安装器、生成器或测试入口出现非预期失败时，追加式正反例契约见 [execution-case-notes.md](execution-case-notes.md)。失败学习只在根因确认且同输入 local 复验通过后进入 `candidate`；案例当前状态以最后一条状态事件为准。
...
8. 每次 `create` 或 `append` 返回 `verified=true` 后立即按 [引用台账](#引用台账) 登记沉淀条目；没有 readback 证据的写入不登记。

如果本轮包含非预期执行失败，按 [execution-case-notes.md](execution-case-notes.md) 追加失败特征、反例、正确方案、验证证据和状态事件；不得把未经复验的猜测写为正例，不得覆盖历史事件。
...
| 字段 | 取值 | 说明 |
| --- | --- | --- |
| 笔记名 | 发起 bridge 调用时所用 path 的文件名部分 | 必须取自本地路径字符串，**不使用 CLI 回显文本**；官方 CLI 回显中文会乱码 |
...
- 台账为空时不得构造占位条目；收口侧会据此整节省略知识引用小节。
- 台账只记录 vault 笔记操作；项目本地四件套的读写不进台账。

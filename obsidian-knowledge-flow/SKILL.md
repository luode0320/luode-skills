...
固定读取顺序是“父目录规则 -> `PROJECT_CURRENT.md` -> `PROJECT_MEMORY.md`”，缺失的项目文件先创建最小 UTF-8 模板。项目本地 Markdown 使用标准文件工具；vault 内笔记的检索、读取、创建和追加只能通过公开 bridge allowlist（由 bridge 调用官方 Windows CLI）完成。项目本地四件套已经提供当前项目上下文时，不得为了形式再次读取 vault。

## 选择性默认判定

仓库任务默认先做轻量判断，但不默认调用 CLI。判断结果必须归入以下四类：

- `不适用`: 当前问题不依赖历史知识、知识库内容、长期用户偏好或重复实体，本轮也没有值得未来复用的事实、决策、流程、定义、来源或调试经验；记录判断即可，不调用 bridge。
- `检索`: 当前问题依赖历史决策、项目事实、用户偏好、重复实体、知识库内容，或出现“上次”“之前”“我们约定”“当时怎么说”“Obsidian 里”“知识库里”等信号；必须通过 bridge 检索并读取匹配笔记后再引用。
- `沉淀`: 会话总结、阶段收口或最终回复前，确认本轮形成可复用事实、决策、流程、定义、偏好、来源、调试经验或规则口径；必须先通过 bridge 检索现有承接笔记，再决定创建、追加或沉淀。
- `阻断`: 本应检索或沉淀，但 bridge doctor 失败、目标 vault 未注册、根目录不一致、命令无法限定到目标 vault，或读写命令失败；必须说明阻断原因，不得直接读写 vault 文件作为替代。WSL 缺少原生 CLI 时必须先由 bridge 走 Windows interop。
...
3. 直接使用固定映射 `D:\obsidian_data` + `知识库/`，不自行选择 shell、CLI 路径或 vault selector，并按场景读取参考文件：
   - 决定目录落点时读 [vault-layout.md](references/vault-layout.md)。
   - 使用 bridge 前读 [cli-operations.md](references/cli-operations.md)。
...
bridge、vault 注册、路径、超时或读写入口出现非预期失败时，先触发 `execution-failure-learning-rules` 的 `recover`，查阅 [references/cli-failure-casebook.md](references/cli-failure-casebook.md)。CLI 阻断仍必须停止并报告，不得用文件系统 fallback；已验证的新经验只写入 candidate。

执行失败持续学习的统一笔记契约、状态转换和 bridge-only 写入边界见 [execution-case-notes.md](references/execution-case-notes.md)。动态案例正文只写入 Obsidian；仓库中的 casebook 只能作为规则种子或验证来源，不能替代动态案例笔记。
...
- 默认使用 bridge allowlist 的 `doctor`、`search`、`search-context`、`read`、`create`、`append`、`open` 操作固定 vault；Windows 由适配器调用官方 CLI，WSL 由 bridge 自动走 Windows interop。
- bridge 的版本、应用恢复、目标 vault、阻断条件和命令模板统一见 [cli-operations.md](references/cli-operations.md)。

...
description: 当新增或修改任意代码文件、脚本文件、配置型代码或测试代码时触发；当用户以文字或截图指出某写法不对、这个风格不对、不能这么写、这样写不行时也必须触发。负责跟随项目现有写法，并在本轮已触发 `code-generation-style-rules` 时依据其产出的代码风格契约检查局部一致性，避免局部风格跳变和个人偏好入侵；测试完成后由本 skill 提供唯一活动 `6-review` 风格回归入口，输出 `STYLE: PASS` 或 `STYLE: FIX_REQUIRED`，只判断写法、位置、格式和既有习惯，不判断业务正确性、需求覆盖或发布放行；同时作为全局用户风格反例库的唯一 owner，把用户否定的写法规范化为反例、把用户期望的写法记为正例，经用户确认后写入 `references/user-style-feedback-library.md` 全局永久生效，供写码前加载规避；不要用它代替最小改动、可读性、注释规范、代码归位规则或代码生成风格契约入口。
---
...
`6-review` 只能在真实测试完成后执行一次，记录到项目的 `doc/6-review/`。它只产出 `STYLE: PASS` 或 `STYLE: FIX_REQUIRED`，检查范围固定为：

- 格式、换行、UTF-8、尾随空白和项目既有格式化口径；
- 命名、局部写法、日志、注释、可读性和 `PROJECT_STYLE.md` 约定；
- 文件目录位置、依赖方向、测试资产归位和已有公共工具复用；
- 代码与测试是否出现局部风格跳变、临时文件或路径漂移。
...
- 多条相邻的逐行 `var` 声明，行尾中文注释按列对齐，保持视觉列齐整。
...
- 每次真实测试后的 `6-review` 都必须归档到活动 `doc/6-review/`，不得以“常规跟随现有风格且无争议”为由跳过。
- 归档内容至少包含 `STYLE: PASS` 或 `STYLE: FIX_REQUIRED`、检查范围、来源对象、跟随依据、发现位置和处理结论。

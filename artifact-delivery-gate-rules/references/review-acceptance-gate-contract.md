...
这份契约只用于读取历史审查、验收、功能验证、浏览器联调和第三方验证资料中的既有 `review_acceptance_gates`。新流程以实施计划、真实测试和 `6-review` 风格回归收口，不得新增该字段、阶段或放行分支。

它不放宽文档格式、链接、追踪、编码或损坏图形等交付硬门禁；它只决定某项验证在当前来源和当前阶段是否适用，以及条件不足时能否继续推进。

## 2. YAML 字段

历史门禁记录放在文档 YAML front matter 的 `review_acceptance_gates` 列表中。每条记录必须包含以下字段：
...
新实施计划的完成条件、异常边界和测试映射都在 `doc/3-实施/` 中维护；历史门禁不得作为新任务的放行依据。
...
当历史审查结论为“阻断”或历史最终验收结论为“不通过/待重验”时，必须转入 [任务阻断收口契约](task-blocker-closure-contract.md)，记录唯一 `BLK-*`、恢复计划和重入验证。`limited` 与 `not_applicable` 继续按本契约处理，不能被升级为“任务已阻断”。

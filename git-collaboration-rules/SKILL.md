...
2. 按 `references/commit-scope-and-clean-tree.md` 冻结业务提交范围和清空目标。
...
8. 用户目标为“提交git”时按业务目标循环，直到 `git status --short` 为空或出现明确阻断。
...
- 当前轮授权与实际 Git 动作一致。
- 每个 commit 只覆盖一个业务目标；同一业务目标的代码实现、可执行测试、流程文档和项目状态同步合并为同一笔提交，不再按提交域二次拆分。

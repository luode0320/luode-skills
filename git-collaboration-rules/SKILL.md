...
- 识别到 Git 协作意图后，首条中间进度第一行必须输出：
  - `HIT-CHECK:PASS; GIT-RULE:PASS`
- 若未输出该行，仅允许执行盘点命令：
  - `git status --short`
  - `git diff --cached --stat`
- 执行其它 git 命令视为违规，判定 `BLOCK`。

## -1.5 执行前自检标记（新增，强制）
- 在执行任何 `git add/commit/push/pull/rebase/merge/cherry-pick/stash` 前，必须先在首条中间进度出现：
  - `PRECHECK:READY`
- 若没有 `PRECHECK:READY`，只允许盘点命令，禁止继续执行其他 git 命令。

## -1.6 违规自恢复（新增，强制）
- 若发生“未完成首条命中检查就执行 git 命令”，必须立即进入恢复模式：
1. 立即停止非盘点 git 命令。
2. 输出 `HIT-CHECK:BLOCK; GIT-RULE:BLOCK` 与 `RECOVERY:RESTART_REQUIRED`。
3. 重走首条模板 + 证据三要素 + `PRECHECK:READY`，完成后才允许继续。
...
- 证据模板已完整填写
- 固定校验行完整输出（`HIT-CHECK:PASS`、`GIT-RULE:PASS`）

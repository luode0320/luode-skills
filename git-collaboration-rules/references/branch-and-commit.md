# 分支与提交

## 铁律：拆分业务修改，多次提交

**提交代码时必须分析未提交的代码，如果可以分成不同的业务修改，就用多次不同的 git 提交。**

- 不要一次提交所有导致业务没有区分清楚。
- 按业务功能、业务模块、业务目标来拆分提交。
- 每个提交只承载一个清晰的业务目标。
- 同一业务目标的改动合并为一笔提交：代码实现、运行配置、可执行测试、`doc/2-需求/`、`doc/3-实施/`、`doc/4-bugs/`、`doc/5-tests/`、`doc/6-review/`、`doc/7-验收/` 等流程文档和项目状态同步文件随该业务一起提交，不再按提交域二次拆分。
- 项目状态同步文件至少包括 `PROJECT_CURRENT.md`、`PROJECT_MEMORY.md`、`PROJECT_HISTORY.md`、`PROJECT_STYLE.md`、`编码skill.md`、`字典.md`、`skill-dictionary/data.js`，随所属业务一并提交，不额外拆 `chore`。
- **落点规则与提交边界无关**：可执行测试仍需放在根 `test/**`、`*_test.*`、`*.spec.*`、`*.test.*`，`doc/5-tests/**` 仍只放测试说明 / 证据文档，`internal/service/*.go` 仍需子目录化；合并提交不放宽这些落点约束。
- 根目录 `README.md` 改动日志作为配套记录跟随对应 commit 一起提交。

## 铁律：`提交git` 请求必须全量收口

当用户明确请求“提交git”时，完成标准不是“提交一部分”，而是：

- 允许按业务拆分成多次提交。
- 但“全量收口”不等于“混成一笔提交”；拆分依据是业务目标，互不相关的业务必须拆开，同一业务内部的实现、测试与文档才合并提交。
- 但必须持续提交直到当前未提交改动清空。
- 最终必须满足 `git status --short` 为空。
- 除阻断级规则（如 Go 禁放扫描失败）外，不允许以“无关改动”名义提前结束。

同义短指令也按相同规则处理，包括但不限于：

- `提交 git`
- `git提交`
- `commit一下`
- `帮我提交代码`

以上短指令都视为“执行提交动作”，不要求用户先补充“测试已完成”或“准备交付”的阶段描述。

## 提交前必须补 README 改动日志

在正式执行 git 提交之前，必须先更新**项目根目录** `README.md`，在“改动日志”区域最后一条提交日志后追加一行本次改动日志。

日志内容固定为：

```text
yyyy-MM-dd HH:mm:ss 提交标题
```

要求如下：

- 标题直接使用本次 git 提交标题，保持一致，不要另外改写一套说法。
- 时间戳使用当前北京时间，格式为 `yyyy-MM-dd HH:mm:ss`。
- 一次 git 提交对应一条 README 改动日志。
- 如果拆成多次 git 提交，就要按提交次数追加多条日志。
- 必须始终追加到当前“改动日志”最后一条提交日志后面，不插到更前面，也不追加到更靠前的旧日志上方。
- 默认不处理“补记历史提交并按时间回插”的场景；本 skill 只要求本次提交日志始终落在改动日志末尾。
- 行尾注意（CRLF 仓库）：用编辑工具改写 README 可能把整文件行尾重写为 LF，导致 diff 把整个文件标成改动、提交成“整文件行尾重写”。追加日志行前先确认原文件行尾（`tail -c 30 README.md | xxd`），按原行尾精确追加——回退用 `git checkout -- README.md`，再 `printf 'yyyy-MM-dd HH:mm:ss 标题\n' >> README.md`；提交前用 `git diff -- README.md` 确认仅 +1 行。WSL 挂载的仓库尤甚（9P 行尾 + core.autocrlf 交互）。

示例：

```text
2026-07-01 14:25:30 fix: [订单创建接口] 修复订单金额计算错误
2026-07-01 14:30:00 feat: [用户登录] 新增验证码校验能力
```

正确示例：

```text
2026-06-28 01:12:00 feat: [平台安全] 增加内网来源 IP 白名单中间件
2026-06-28 04:17:15 refactor: [租户会话与订单路由] 拆分注册函数
```

错误示例：

```text
2026-06-28 04:17:15 refactor: [租户会话与订单路由] 拆分注册函数
2026-06-28 01:12:00 feat: [平台安全] 增加内网来源 IP 白名单中间件
```

上面的错误示例里，较新的提交如果被插到已有末尾日志前面，就会破坏“始终从末尾继续追加”的规则，不允许这样记录。

不要写成模糊描述，例如：

```text
今天修了一个 bug
更新了一些内容
```

## 提交消息格式

提交消息必须遵循以下格式：

### 标题模板（必选）

```text
<type>: [中文简要说明] 标题说明
```

标题规则：

- `<type>` 必须来自「格式三：规范类型字典」的固定前缀集合，当前支持 `feat`、`fix`、`docs`、`style`、`refactor`、`perf`、`cr`、`test`、`build`、`revert`，维护类杂项沿用 `chore`；禁止自造其它前缀（如 `update`、`modify`、`opt` 等）。
- `中文简要说明` 直接描述本次改动对象，例如“用户登录”“订单结算”“订单创建接口”。
- 禁止使用 `type(scope): 标题` 的括号 scope 写法（例如 `feat(reqsend): 转账详细统计支持多Sheet导出错误明细`）。
- 遇到 scope 场景必须改写为 `type: [scope] 标题`（例如 `feat: [reqsend] 转账详细统计支持多Sheet导出错误明细`）。
- `中文简要说明` 不允许加“功能/”“业务/”“接口/”等分类前缀。
- `中文简要说明` 不允许直接写 API URL（如 `POST /api/order/create`）。
- `标题说明` 用动宾短句说明动作，例如“新增短信验证码登录能力”“修复优惠券重复抵扣”。

正误对照（必须遵循）：

```text
错误：feat(reqsend): 转账详细统计支持多Sheet导出错误明细
正确：feat: [reqsend] 转账详细统计支持多Sheet导出错误明细
```

### 格式一：feat（新增功能）

```
feat: [用户登录] 新增短信验证码登录能力

说明：详细说明本次新增的功能内容、影响范围等。
```

示例：

```
feat: [用户登录] 新增密码与验证码双登录能力

说明：
- 支持手机号验证码登录
- 支持密码登录
- 登录成功后返回用户基本信息和 token
```

### 格式二：fix（修复 bug）

```
fix: [订单创建接口] 修复优惠券重复抵扣问题

说明：详细说明修复的问题、原因、影响范围等。
```

示例：

```
fix: [订单结算] 修复订单创建时金额计算错误

说明：
- 问题：订单创建时优惠券折扣计算错误
- 原因：折扣计算逻辑中缺少对满减条件的判断
- 影响：已创建的订单金额可能不正确
```

### 格式三：规范类型字典（权威）

`<type>` 必须取自下表，禁止使用约定外前缀。本表同步自版本控制规范（`Git/代码规范.md`）。

| 类型 | 含义 | 说明 |
| :--- | :--- | :--- |
| `feat` | 新增特性 | 新增功能 / 特性 |
| `fix` | 修复 bug | 修复缺陷（A bug fix） |
| `docs` | 仅文档变更 | 只改文档，不改代码或逻辑（Documentation only changes） |
| `style` | 代码格式调整 | 不影响代码逻辑（white-space、formatting、missing semi colons 等） |
| `refactor` | 重构 | 不改变外部行为的代码重构（refactor） |
| `perf` | 性能优化 | 提升性能的改动（A code change that improves performance） |
| `cr` | 代码评审 | code review 相关修改 |
| `test` | 测试相关 | 新增或修改测试代码（Adding missing tests or correcting existing tests） |
| `build` | 构建/依赖变更 | 影响构建系统或外部依赖的改动（Changes that affect the build system or external dependencies） |
| `revert` | 回滚 | 撤销某次提交（Reverts a previous commit） |

补充说明：

- `chore`：本 skill 沿用 `chore` 表示构建/工具类维护杂项；与 `build`（构建系统或外部依赖变更）语义接近时两者皆可，但一个提交只选其一，不并列使用。
- `docs` 仅限纯文档变更；本仓库流程/状态文档通常随所属业务合并提交，只有整笔都是纯文档改动时才使用 `docs`（见「按业务合并提交」），不视为 `chore`。
- 类型后一律跟英文冒号 `:`（半角），再接 `[中文简要说明] 标题说明`，禁止 `type(scope):` 括号 scope 写法。

## 多行说明写法（跨 shell 统一）

推荐优先使用以下两种方式，避免把 `\n` 写进提交消息正文：

### 写法一：多 `-m`（跨 shell 稳定）

```bash
git commit \
  -m "fix: [财务配置] 适配 financeConfig 新字段与动态开关规则" \
  -m "1. 新增 financeConfig 的 version/limit/enable 字段映射。
2. 根据 gVersion 与 ipCountry 动态计算 enable。
3. 补充关键结构与函数中文注释。"
```

### 写法二：`-F -`（长正文推荐）

```bash
cat <<'MSG' | git commit -F -
fix: [财务配置] 适配 financeConfig 新字段与动态开关规则

1. 新增 financeConfig 的 version/limit/enable 字段映射。
2. 根据 gVersion 与 ipCountry 动态计算 enable。
3. 补充关键结构与函数中文注释。
MSG
```

## 提交后强制校验（新增硬规则）

提交完成后必须执行一次：

```bash
git log -1 --pretty=%B | rg '\\n'
```

判定规则：

- 命中为空：通过。
- 命中非空：说明提交正文含字面量 `\n`，必须立即 `git commit --amend` 修正为真实换行。

## Windows/PowerShell 提交说明格式（重点：换行）

**提交类普通命令同样优先 bash（能 bash 不 PowerShell）；下方 PowerShell 示例仅作 PowerShell 专项场景保底，且专项场景优先 PowerShell 7、5.1 仅阻断回退。判定以 `AGENTS.md` 的「Windows / WSL 执行规则」内联摘要为准。**

Windows/PowerShell 场景下，提交消息格式必须满足：

```text
<type>: [中文简要说明] 标题说明

说明：
- 第一条
- 第二条
```

重点要求：

- 标题行和说明正文之间必须有且仅有一空行。
- 说明正文必须是“真实换行”，不允许出现字面量 `\n`。
- 正文建议使用短句或列表，避免一整段大长句。
- 提交后仍需执行 `git log -1 --pretty=%B | rg '\\n'` 校验，确保没有字面量 `\n` 遗留。

推荐写法一（`-m` + here-string，适合常规提交）：

```powershell
git commit -m "fix: [订单结算] 修复订单创建时金额计算错误" -m @"
说明：
- 问题：订单创建时优惠券折扣计算错误
- 原因：折扣计算逻辑中缺少对满减条件的判断
- 影响：已创建的订单金额可能不正确
"@
```

推荐写法二（`-F -`，适合更长说明）：

```powershell
@"
fix: [订单结算] 修复订单创建时金额计算错误

说明：
- 问题：订单创建时优惠券折扣计算错误
- 原因：折扣计算逻辑中缺少对满减条件的判断
- 影响：已创建的订单金额可能不正确
"@ | git commit -F -
```

错误写法（PowerShell 下会把 `\n` 当普通字符）：

```powershell
git commit -m "fix: [订单结算] 修复订单创建时金额计算错误\n\n说明：\n- 问题：订单创建时优惠券折扣计算错误"
```

## 分支原则

- 一次提交最好只承载一个清晰目标。
- 提交说明应能看出做了什么、为什么做。
- 分支要尽量围绕单任务或单主题，避免长期混杂演进。
- 无关格式化、试验代码和调试残留不要混进正式提交。
- 对这个 skill 来说，默认以 git 提交本身和项目根目录 `README.md` 改动日志作为持久化记录，不再额外复制到 `delivery/` 之类目录。
- 项目根目录 `README.md` 改动日志默认只要求“每次从末尾继续追加”，不再要求为了时间排序去回插旧位置。

## Go 提交前禁放扫描

Go 项目在执行 `git commit` 前，必须执行：

```powershell
rg --files -g "*_test.go" | rg -v "^test/" | rg -v "^doc/5-tests/"
```

判定规则：

- 命中为空：通过，可继续提交。
- 命中非空：阻断提交，先按 `test-program-rules` 的三级替代出路降级白盒诉求，并迁移到根 `test/` 镜像路径后再提交；历史 `doc/5-tests/` 可执行资产只读豁免，不作为活动测试代码根。

## 生产代码测试污染扫描

本次提交包含生产代码改动时，`git commit` 前必须执行：

```bash
python test-strategy-rules/scripts/scan_test_pollution.py --root . --diff-only
```

判定规则：

- `POLLUTION: PASS`（退出码 0）：通过，可继续提交。
- `POLLUTION: FAIL`（退出码 1）：阻断提交。命中符号是为测试而加的，先迁移到根 `test/` 镜像路径；确属反射调用、配置驱动注册、对外导出 API 或待接线新功能的，在项目根 `.test-pollution-allowlist` 登记路径、符号与理由后再提交。
- `SUSPECT` 与 `ORPHAN` 为提示级，不阻断提交，交 `6-review` 判断。

判据、豁免语义和治理步骤由 `test-strategy-rules` 唯一定义，本节只固定提交前的执行时机与阻断口径。

这里的 `*_test.go` 只解决 Go 测试落点问题。落点约束不因提交粒度变化而放宽：可执行测试文件仍必须位于根 `test/**`、`*_test.*`、`*.spec.*`、`*.test.*`，`doc/5-tests/**` 仍只作为测试说明 / 证据文档；但它们可以随所属业务与代码实现同笔提交。


## 每次提交前的基础代码核查

每次 `git commit` 前，必须直接核查当前 staged 改动并完成基础验收；不读取、不要求、也不以 `doc/6-review/` 或 `doc/7-验收/` 的最近文档作为提交前提，不默认启动完整项目审查或正式最终验收。先运行：

```bash
git diff --cached --check
```

随后按当前 staged 的实际文件逐项核查：

- 格式：除上述空白错误检查外，代码、脚本、配置或测试改动要复用项目已有的格式化检查命令。
- 注释：非直观规则、并发约束、错误处理和边界行为的必要注释准确、未过期，中文与 UTF-8 正常。
- 安全性：输入、权限、敏感信息、日志、文件/网络访问和注入面不存在明显误放行、泄露或不安全默认值。
- 并发安全性：共享可变状态、锁、goroutine/线程、取消、资源关闭和异步回调不引入竞态、死锁、泄漏或重复执行。
- 系统崩溃风险：空值、panic/异常、越界、类型断言、资源耗尽和错误传播不会意外中断进程或关键链路。
- 边界条件：空值、零值、范围/溢出、超时、分页/批量上限、状态顺序和外部依赖失败路径可靠。
- 基础验收：按当前 staged 改动的实际风险确认项目已有的测试、功能验证或等价验证是否已覆盖；没有适用验证时明确记录“不适用 + 原因”。

纯文档提交同样要执行核查，但没有关联源码的维度必须在提交证据中写为“不适用 + 原因”。如果任一维度发现 P0/P1、格式错误、关键注释缺失、安全/并发/崩溃风险、边界缺陷或明显潜在 bug，必须先修复再提交。

Git 提交的基础核查与基础验收只记录在提交证据中，禁止因提交自动生成 `doc/6-review/` 或 `doc/7-验收/` 文档。显式总审查或正式最终验收仍由各自独立任务决定并归档；归档不是本条 Git 提交前闸门的前置条件。

## Go 服务层提交前扫描

Go 项目在执行 `git commit` 前，还必须检查 staged 文件中是否出现服务层根目录直落实现：

```powershell
git diff --cached --name-only | rg "^internal/service/[^/]+\.go$"
```

判定规则：

- 命中为空：通过，可继续提交。
- 命中非空：阻断提交，先迁移到 `internal/service/<domain>/` 后再提交。

staged 同时包含代码实现、`docs` 与 `test` 文件**不再构成阻断**：它们属于同一业务目标时应合并为一笔提交。只有确认夹带了不属于该业务目标的改动时，才拆出或停下确认。

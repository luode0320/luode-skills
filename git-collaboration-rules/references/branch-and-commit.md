...
- 每个提交只承载一个清晰的业务目标。
- 同一任务的流程文档默认合并为一笔 `docs` 提交，覆盖 `doc/2-需求/`、`doc/3-实施/`、`doc/4-bugs/`、`doc/5-tests/`、`doc/6-review/`、`doc/7-验收/` 和项目状态同步文件。
- 项目状态同步文件至少包括 `PROJECT_CURRENT.md`、`PROJECT_MEMORY.md`、`PROJECT_HISTORY.md`、`PROJECT_STYLE.md`、`编码skill.md`、`字典.md`、`skill-dictionary/data.js`，默认跟随同一任务的 `docs` 提交。
- 可执行测试文件（至少根 `test/**`、`*_test.*`、`*.spec.*`、`*.test.*`）归入 `test` 提交，不和代码实现 / 运行配置或 `docs` 提交混提；`doc/5-tests/**` 仅作为测试说明 / 证据文档归入 `docs`。
- 代码实现 / 运行配置单独提交，不与 `docs` 或 `test` 域混提；根目录 `README.md` 改动日志作为配套记录可跟随对应 commit 一起提交。
...
- 允许按业务拆分成多次提交。
- 但“全量收口”不等于“混成一笔提交”；同一任务默认最多按 `docs`、`test`、实现 / 修复拆分，必须继续拆完再提。
- 但必须持续提交直到当前未提交改动清空。
...
如果 staged 同时包含代码实现文件与 `docs` / `test` 任一域，或同时命中 `docs` 与 `test`，也必须阻断，先按提交域拆开后再继续。同一任务内多个流程文档目录和项目状态同步文件同属 `docs` 域，可以合并提交。

...
3. 再确认本轮验收范围内的全部实施周期都已收口；当前周期若仍有最小任务未完成实现、真实测试、审查、验收任一环节，直接阻断最终验收。
4. 按验收标准逐条对照当前结果，标记通过 / 不通过 / 阻断。
5. 找到当前来源对象对应的最终验收文档；如果还没有，就按 `artifact-storage-rules` 的命名模板初始化单独文档。
6. 若发现测试、审核、周期收口或最小任务闭环任一未完成，直接输出“未满足最终验收前置条件”，不要继续伪装放行。
...
8. 若实施周期未按顺序收口，或任一纳入验收范围的最小任务缺少实现、真实测试、审查、验收证据，最终验收文档中只能输出“未满足最终验收前置条件”，不得写通过结论。

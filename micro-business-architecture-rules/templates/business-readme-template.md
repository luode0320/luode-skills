...
> 层内落点沿用 `package-structure-rules` 分层规则，本包只在其上补「业务横向隔离 + JSON RPC」这一层。

```text
internal/business/<域>/
├── README.md         # 本文件：业务包统一说明
├── service/          # <业务领域逻辑>
├── entity/           # <业务私有数据结构（仅本包内部使用）>
├── util/             # <业务私有辅助能力>
└── rpc/              # <按需创建：其他业务可导入的 JSON 字符串公开函数>
```

## 对外接口

本业务在 `rpc/` 暴露、供其他业务调用的 JSON 字符串函数：
...
<指向本包 entity 的说明：本业务的私有数据结构集中在 `internal/business/<域>/entity/`，仅供本包内部使用，不对外暴露；外部只能经上面的 RPC 获取 JSON 响应。列出关键模型入口文件或类型。>

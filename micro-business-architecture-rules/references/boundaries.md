...
| `project-agents-bootstrap` | 衔接 | 新会话首轮的规则文件与项目记忆四件套自举交给它；微业务标记的 upsert 由本 skill 自有脚本完成，不侵入 bootstrap 脚本。 |
...
## 5. `project-agents-bootstrap`（衔接）

- 衔接内容：新会话首轮的规则文件（`CLAUDE.md` / `AGENTS.md`）检测与创建、项目记忆四件套（`PROJECT_CURRENT.md` / `PROJECT_MEMORY.md` / `PROJECT_HISTORY.md` 等）的自举，全部由 `project-agents-bootstrap` 负责。

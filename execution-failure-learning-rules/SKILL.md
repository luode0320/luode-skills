...
将一次执行失败变成可复用的预防与恢复知识。此 skill 是跨领域路由器；案例正文始终归属于唯一领域 skill，不在这里建立全局错误大杂烩。动态案例的唯一持久化位置是固定知识库目录，静态 owner casebook 只作为种子、模板和回归基线。
...
- `learn`：根因已确认、方案已应用且同输入复验通过时，按案例模板脱敏、去重并经标准文件工具自动写入唯一 owner 的 `candidate`。`active` 晋级必须满足授权和全部门禁。
...
- 固定知识库入口为 `D:\谷歌云盘\知识库\20-Knowledge\execution-failure-cases\<owner>\<case-id>.md`，导航 MOC 为 `30-MOCs/执行失败案例.md`。案例路径一律使用相对知识库根的裸相对路径，禁止再加 `知识库/` 前缀。案例的检索、创建、追加和读回统一使用标准文件工具：检索用 Grep/rg，读取用 Read，写入用 Write/Edit；写入后必须回读校验内容一致才算持久化成功。
...
- 知识库目录不可达、路径不合法或写入后回读不一致时，停止知识库写入并报告 `candidate persistence: blocked`；不得改用静态 casebook fallback、无变化重试或声称案例已写入。该故障按 `knowledge-flow` owner 继续走 `recover`，当前案例保留结构化脱敏交接。

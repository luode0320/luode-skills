...
- 别名: 文档目录混合命名
- 类型: 目录风格
- 示例: `doc/1-架构/`、`doc/2-需求/`、`doc/3-实施/`、`doc/4-bugs/`、`doc/5-tests/`、`doc/6-review/`、历史 `doc/6-审查/`、`doc/7-验收/`
- 说明: `doc/` 根目录保持英文，活动子目录按流程顺序加数字前缀；`doc/6-review/` 是测试后的唯一风格回归入口，旧审查/验收目录只读保留。中文语义目录保留中文，`bugs`、`tests` 这类工程高频词保留英文，避免脚本和协作摩擦。
...
- 来源: 用户本轮需求、`comment-rules`（补齐闸门分区 + 放置颗粒度分区）
...
- 说明: 新增、修改或重构代码前，先由 `code-generation-style-rules` 读取 `PROJECT_STYLE.md`、当前文件和同目录样例，形成本轮代码风格契约；如果当前上下文存在高度统一的编码风格，新增内容必须沿用既有结构，只做必要模板替换，不加入多余代码；如果实现已有接口，必须优先参考项目中已有该接口实现的代码风格；契约用精简字段约束后续实现，不写成抽象口号。
- 局部执行示例: `changelly.New().Start(true)`、`swft.New().Start(true)` 等同一注册区段新增对象时，继续使用 `xx.New().Start(true)`，不额外引入 helper、变量、循环或抽象。
- 接口实现示例: 先查找同文件、同目录/模块或同语言项目中的既有实现，沿用其方法顺序、字段组织、构造/注册、错误处理、日志、注释和测试方式。
...
### 后端工具落点区分 utils 与 common/util
- 别名: 后端 utils 归位, common/util 归位, IP 工具包归位, 项目无关工具包与高关联工具函数分流
- 类型: 包结构风格
- 示例: `utils/time/parse.go`、`utils/ip/address.go`、`common/util/project_root.go`、`business/<domain>/util/<helper>.go`
- 说明: 后端中与项目无关、可独立复制的工具包或 SDK 放入根 `utils/<package>/`；根 `utils/` 不得有直接文件且不得引用项目其他包。请求 IP 提取、规范化、公私网判断和国家/地区归属查询适配固定放入 `utils/ip/`，不放代理信任、风控、业务黑白名单或业务地域策略。需要引用项目文件、路径、配置或项目约定但不承载业务流程的工具函数，直接放入独立后端 `common/util/<function>.<ext>`；源码根 `util/` 为废弃位置。业务域私有辅助仍留在 `business/<domain>/util/`。前端 `src/util/`、前端 `src/common/util/` 不受此条影响。
...
### Git 提交域隔离写法
- 别名: 同一任务文档合并, 文档测试与代码分开提交, commit domain split
- 类型: 流程风格
- 示例: `docs: [稳定币费率任务] 更新需求实施与 6-review`、`test: [费率计算] 补充结算回归用例`、`feat: [费率计算] 新增稳定币清算规则`
- 说明: Git 提交默认按同一任务的 `docs`、`test`、实现 / 修复三类域拆分。同一任务的需求、实施、Bug、测试说明、6-review、验收和项目状态同步文件合并为一笔 `docs` 提交；`doc/5-tests/**` 仅保存说明、日志、报告、截图和非可执行证据。根 `test/**`、`*_test.*`、`*.spec.*`、`*.test.*` 等可执行测试归入独立 `test` 提交，代码实现 / 运行配置归入独立 `feat` 或 `fix` 提交，不与 `docs` 或 `test` 混提。历史 `doc/6-审查/`、`doc/7-验收/` 仅按归档资料处理，`README.md` 改动日志可跟随对应 commit 一起更新。
- 来源: 对话确认、`git-collaboration-rules`
...
- 来源: `requirement-intake-rules` 的 `initial-discovery` 路由、对话确认
...
- 2026-08-01：活动流程收敛为 `doc/1-架构/` 到 `doc/6-review/`；`doc/6-审查/`、`doc/7-验收/` 仅作为历史只读归档。
...
- 2026-08-01：补充周期与最小任务书写风格，明确实施周期按第一期 / 第二期 / 第三期等大进度排序，周期内最小任务逐个完成实现、真实测试、6-review 风格回归后才进入下一任务 / 下一周期。
- 2026-07-05：补充全量顺序实施方案写法，明确新项目 / 多来源对象先用总表串起需求、验收标准、实施总览、实施周期和最小任务，再进入单来源对象实施总览。
- 2026-07-03：补充代码风格回归注释判定写法，明确代码改动进入 `6-review` 时，方法头 `[参数]` / `[返回]` / `最近修改时间` / 改动原因、字段/结构体字面量中文说明、步骤编号 `1/2/3` 就近注释、补丁“做了什么 + 为什么要加”注释任一缺失都按 `FIX_REQUIRED` 处理。
...
- 2026-08-04：后端工具落点改为根 `utils/<package>/` 与独立后端 `common/util/<function>.<ext>` 分流；源码根 `util/` 废弃，保留业务域私有 `util/`，不改变前端工具目录。
- 2026-07-29：在既有根 `utils/` 分流内固定 `utils/ip/`，仅承载通用 IP 处理和地址归属查询适配，不引入代理或业务策略。

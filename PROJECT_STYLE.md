...
- 别名: 后端 utils 归位, 源码根 util 归位, 项目无关工具包与高关联工具函数分流
- 类型: 包结构风格
- 示例: `utils/time/parse.go`、`internal/util/project_root.go`、`business/<domain>/util/<helper>.go`
- 说明: 后端中与项目无关、可独立复制的工具包或 SDK 放入根 `utils/<package>/`；根 `utils/` 不得有直接文件且不得引用项目其他包。需要引用项目文件、路径、配置或项目约定但不承载业务流程的工具函数，直接放入语言源码根 `util/<function>.<ext>`；业务域私有辅助仍留在 `business/<domain>/util/`。前端 `src/util/`、前端 `src/common/util/` 不受此条影响。

...
### 后端工具落点区分 utils 与源码根 util
- 别名: 后端 utils 归位, 源码根 util 归位, IP 工具包归位, 项目无关工具包与高关联工具函数分流
- 类型: 包结构风格
- 示例: `utils/time/parse.go`、`utils/ip/address.go`、`internal/util/project_root.go`、`business/<domain>/util/<helper>.go`
- 说明: 后端中与项目无关、可独立复制的工具包或 SDK 放入根 `utils/<package>/`；根 `utils/` 不得有直接文件且不得引用项目其他包。请求 IP 提取、规范化、公私网判断和国家/地区归属查询适配固定放入 `utils/ip/`，不放代理信任、风控、业务黑白名单或业务地域策略。需要引用项目文件、路径、配置或项目约定但不承载业务流程的工具函数，直接放入语言源码根 `util/<function>.<ext>`；业务域私有辅助仍留在 `business/<domain>/util/`。前端 `src/util/`、前端 `src/common/util/` 不受此条影响。
...
- 2026-07-28：后端工具落点改为根 `utils/<package>/` 与源码根 `util/<function>.<ext>` 分流；保留业务域私有 `util/`，不改变前端工具目录。

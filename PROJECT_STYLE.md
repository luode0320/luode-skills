...
- 类型: 文档风格
- 示例: `最小任务A: 支付路由闭环`、`依赖: 配置 -> 接口 -> 页面`、`预计文件数: 4`、`闭环: 实现 -> 测试 -> 审查 -> 验收`
- 说明: 实施计划正文优先按依赖关系与端到端垂直切片组织最小任务，不按前端 / 后端 / 数据库水平分层堆任务。每个最小任务默认单次专注完成，预计触达文件数尽量控制在约 5 个以内，并显式写出自己的测试、审查、验收闭环后再进入下一任务。

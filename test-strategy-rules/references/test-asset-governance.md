...
- `doc/5-tests/` 是唯一活动测试说明与证据根。每轮只保存一份扁平测试主文档，日志、报告、截图和非可执行运行产物内联进正文，不得新增测试程序或 mock/stub/fake。
- 历史 `doc/5-tests/` 中已有可执行资产按指纹只读保留；首次修改、改名或新增时迁至根 `test/`，不批量迁移历史包。

## 位置与命名

- 单文件源码：`src/order/service.py -> test/src/order/service_test.py`。
- 目录级测试：`code-style-consistency-rules/ -> test/code-style-consistency-rules/static_owner_router_test.py`；测试主文档必须列出被测文件。
...
- Go 测试必须位于根 `test/` 的 ASCII 路径，以外部 `<target>_test` 包导入目标模块；源码目录禁止 `*_test.go`。白盒需求不得靠改生产代码解决，按 `production-test-pollution.md` 的三级替代出路降级，不保留同包例外。

## 说明与证据

- 每次真实测试在 `doc/5-tests/YYYY-MM-DD_HHmmss_<测试任务中文主题>.md` 记录测试目的、运行命令、样本、结果和证据路径。
- 脱敏日志、报告、截图和非可执行运行产物以代码围栏内联进测试主文档正文；它们不得混入 `test/`。
- 测试代码的变更不因时间戳测试主文档而重建；时间戳只用于说明和证据轮次。

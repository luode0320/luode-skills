...
- `doc/5-tests/` 只保留时间戳 README、日志、报告、截图和非可执行证据；不得新增测试程序或 mock/stub/fake，历史可执行资产按指纹只读，首次修改时迁至 `test/`。
...
- 测试辅助能力必须通过测试目录内脚本、mock、stub、fake、fixture、数据构造和可替换 seam 解决，禁止向生产实现注入“仅测试可用”的业务代码。
...
- 本地配置常含敏感信息（数据库密码、密钥）：代码、配置、普通维护文档和 Git 提交允许有意持久化原值；测试 README、测试证据、控制台、对外输出和项目记忆不得回显原值。

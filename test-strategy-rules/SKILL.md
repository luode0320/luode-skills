...
- **不替用户保留（强制）**：测试启动的服务不得默认留给用户继续联调使用；用户需要时会自行启动，代理不得以「方便后续联调」为由把测试进程留在后台。
- **残留清理**：发现占用被测端口或验证目标的历史残留进程时，先核验归属，确属测试残留后清理，避免旧代码服务污染验证结论；清理动作同样需要「关闭 + 检查」证据。
- **联调场景衔接**：联调开场的「进程基线收口」（允许先关闭既有前后端进程再按本轮目标重启，见本项目联调条件化规则）与收尾强制关闭互补；禁止只做开场基线、不做收尾关闭。
...
6. **测试优先可重启（强制）**：多会话共享同一被测服务时，测试代码/配置有改动需要重启服务，允许杀死并重启——**无论服务由谁启动**（用户手动/其他会话）；测试前快照服务指纹（PID+lstart），用例失败先做服务一致性检查，服务被重启导致的失败归因 `ENV_RESTARTED`（可恢复型环境阻断，**不写缺陷**，健康恢复后 rerun 为准），禁止把重启窗口内的失败直接记 Bug。执行细节见 `apifox-cli__skillhub/modules/environment.md`「测试优先可重启授权」与 `modules/test-data-and-judgement.md`。

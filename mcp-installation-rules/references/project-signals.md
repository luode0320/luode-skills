...
- 用户明确提到“谷歌浏览器 MCP”“Google Chrome MCP”“Chrome MCP”“Chrome DevTools for agents”，并希望当前项目后续由浏览器工具接管
...
- 需求明确要求“打开页面”“验证前端交互”“看网络请求”“抓性能数据”
...
- 若仓库同时包含前端与 Godot 标记，不冲突，默认两类 CLI 工具链都需要准备。
- 若只有网页管理后台或游戏官网前端，不会削弱 Godot 标记；两者仍应分别接管各自工具面。
- 若仓库只是包含文档示例或少量静态文件，不能直接判定为真实前端项目。
- 若用户口头只说“谷歌浏览器 MCP”，但仓库内没有真实前端页面、浏览器验证或调试需求证据，不要仅凭名称机械判定必须重新引入旧 MCP。

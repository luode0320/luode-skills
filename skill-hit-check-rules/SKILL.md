...
- 首条中间进度三行模板后，必须追加一行：
  - `PRECHECK:READY`
- 在输出 `PRECHECK:READY` 前，禁止调用任何非盘点命令。
- 缺少该标记时，后续领域命令一律视为违规执行。

## -1.6 违规自恢复（新增，强制）
- 若已发生“先执行命令、后做命中检查”，必须立即中断后续领域命令，并在下一条中间进度显式输出：
  - `HIT-CHECK:BLOCK; GIT-RULE:<PASS/NA/BLOCK>`
  - `RECOVERY:RESTART_REQUIRED`
- 自恢复阶段仅允许盘点命令；重新输出完整首条模板并恢复到 `PRECHECK:READY` 后，才能继续主流程。
...
3. 输出 `PRECHECK:READY`
4. 再进入主任务执行
5. 若判定存在 Git 意图，必须在同一首条中间进度显式写出“联动命中 git-collaboration-rules”

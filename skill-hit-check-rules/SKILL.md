...
- 新会话、上下文恢复或用户消息包含任意“继续”语义时，首条命中列表必须先把 `task-plan-rehydration-rules` 列为候选 Owner；“继续”语义包括但不限于“继续”“接着做”“接着执行”“恢复任务”“恢复执行”“按原计划继续”“继续上次任务”“往下做”“继续刚才的工作”及同义自然语言表达。当前回合处于 Plan Mode 时该 Owner 必须明确退出，不读取 `PROJECT_CURRENT.md` 或调用 `update_plan`；只有 Plan Mode 已结束且当前回合能证明与活动投影属于同一来源对象时，才在领域动作前读取、校验并调用 `update_plan`。若托管投影缺失，则先触发只读子代理收集当前会话与项目文档证据，再由主代理决定走 `exact` 正式补建还是固定三步 `fallback` 安全恢复列表。失活、损坏、过期、来源不匹配或归属不确定则由该 Owner 明确退出，禁止静默跳过或错投到其它任务。

...
- 若首轮检测到根目录 `AGENTS.md` 不存在，必须先创建完成；创建完成前一律禁止进入主任务（包括分析、读码、执行命令、修改代码）。
- 若出现“先做主任务、后补 `AGENTS.md`”的顺序，按严重流程违规处理：立即停止当前任务，回滚到首轮闸门重走。
...
2. 首条输出两行命中信息
3. 若为新会话第一轮，先执行 `project-agents-bootstrap` 的 `AGENTS.md` 检测
4. 若缺失则立即创建 `AGENTS.md`，创建完成前禁止进入主任务
5. 仅当 `AGENTS.md` 已存在（原有或新建）后，才允许进入主任务执行

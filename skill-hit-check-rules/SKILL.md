...
5. 当请求包含“补充注释 / 只补注释 / 注释规范检查 / 注释完善”时，必须同时命中 `code-comment-rules`、`chinese-comment-rules`、`skill-compliance-gate-rules`，不得缺任一项。
...
6. 若本轮是补注释请求，强制校验是否已命中 `code-comment-rules`、`chinese-comment-rules`、`skill-compliance-gate-rules`，并确认注释范围优先覆盖未提交改动代码。

...
- 未输出该行即判定 `阻断`，除盘点命令外禁止执行任何领域命令。

## -1.5 执行前自检标记（新增，强制）
- 首条中间进度三行模板后，必须追加一行：
  - `前置自检:就绪`
- 在输出 `前置自检:就绪` 前，禁止调用任何非盘点命令。
- 缺少该标记时，后续领域命令一律视为违规执行。

## -1.6 违规自恢复（新增，强制）
- 若已发生“先执行命令、后做命中检查”，必须立即中断后续领域命令，并在下一条中间进度显式输出：
  - `命中检查:阻断; Git规则:<通过/不适用/阻断>`
  - `自恢复:需要重启`
- 自恢复阶段仅允许盘点命令；重新输出完整首条模板并恢复到 `前置自检:就绪` 后，才能继续主流程。

## -1.7 前置门禁（新增，强制）
- 本轮首次 assistant 输出必须包含 `Skill 命中清单`（可由“命中技能”行承载）；未输出前，禁止任何工具调用（含 shell/git/并行工具）。
- 若本轮未命中任何 skill，也必须显式输出 `未命中 + 理由`，然后才能继续后续动作。
- 违反本门禁的调用一律判定为无效执行，必须进入 `-1.6 违规自恢复` 重启流程。
...
4. 若 gate 脚本缺失，证据清单中对应项必须标注 `回退` 且补齐等价检查结果；否则判定未完成。

...
5. 本轮后续每一条中间进度消息（commentary）也必须以命中检查区块开头，不得省略。
6. 任何工具调用前，若要先发中间进度消息，该消息必须先输出命中检查区块。
7. 最终回复也必须再次以命中检查区块开头，确保收口时可复核。
8. 若未命中任何 skill，输出未命中原因后，按通用能力继续处理当前问题。
...
- 本轮后续每条中间进度消息（commentary）开头也必须输出该区块。
- 工具执行前的中间进度说明，必须先输出该区块。
- 本轮最终回复也必须先输出该区块。
- 禁止只在最终回复输出，导致执行过程不可见。
...
- 通过：首条中间消息、后续中间消息、最终回复都先给出命中检查结果，且命中 skill 列表与本轮任务一致。

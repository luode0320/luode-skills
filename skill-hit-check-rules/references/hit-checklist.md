...
- 新会话、上下文恢复或当前消息包含“继续”“继续任务”“按计划继续”等继续语义时，首条命中列表必须包含 `task-plan-rehydration-rules`，不得等领域动作开始后再补命中。
- 首条字段输出后先读取并校验 `PROJECT_CURRENT.md` 的唯一任务投影托管区；有效活动投影必须在领域动作前真实调用 `update_plan`，并说明进行中步骤先核验中断点。

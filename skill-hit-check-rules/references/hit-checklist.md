...
- 若唯一托管区缺失，则先触发只读子代理收集 `candidate_source_documents`、`completed_step_hints`、`current_step_hint` 和 `conflicts`；主代理把这些证据与 `PROJECT_CURRENT` 普通正文传给 `synthesize`。唯一来源且存在明确步骤提示时补建 `exact` 正式列表；其余情况统一生成固定三步 `fallback` 安全恢复列表。

...
- 当已命中任一业务 skill 时，默认追加命中 `subagent-dispatch-rules`，先做委派判定再执行业务 skill；仅在用户明确禁止或环境不支持时回退本地执行。
...
- 当本轮已命中业务 skill 时，默认补命中 `subagent-dispatch-rules` 并优先尝试委派。
- `subagent-dispatch-rules` 只负责“是否委派 + 如何切分”，不替代业务 skill 本身。
- 若用户明确禁止 subagent，则保留命中但必须回退本地执行，不得强行委派。

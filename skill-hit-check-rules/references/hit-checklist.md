...
- 若本轮涉及函数/方法改动，最终回复必须包含“函数注释核对清单”（至少含文件、函数名、`[参数]`、`[返回]`、`最近修改时间`、改动原因是否齐全）。
- 若本轮不涉及函数/方法改动，最终回复必须明确“函数位点 0 个”。
- 若本轮在原有方法中存在补丁位点（兜底/兼容/防护/重试/短路分支），最终回复必须包含“补丁注释核对清单”（至少含文件、函数、位点、做了什么、为什么要加）。
- 若本轮无补丁位点，最终回复必须明确“补丁位点 0 个”。
...
- 不允许在函数/方法改动场景下省略函数注释核对清单并仍宣称“注释已补齐”。
- 不允许在补丁位点场景下省略“为什么要加”的就近注释并仍宣称“注释已补齐”。

...
- `autonomous-execution-rules` 负责“开工后自动推进”，但它不能替代开工授权：只有用户明确“开始实施/开始实现/开始执行/直接做/继续做完/按文档实现”，且已有执行计划、完成定义、停止条件和最大推进边界后，才允许从实施文档切进正式编码；开始后则按实施周期自动串行推进实现、测试、审核与验收收口
...
| `skill-compliance-gate-rules` | 在编码、审查、测试或交付收口阶段做一次 skill 执行完整性闸门检查；若原始目标已完成或用户明确要求结束，强制无下一步，不输出等待类占位文案。 |
| `reasoning-summary-structure-rules` | 在最终推理总结或结束输出阶段自动触发，强制检查总结结构字段完整性；只有原始目标未完成且用户未要求结束，或用户明确要求后续建议时，才允许输出下一步状态。 |

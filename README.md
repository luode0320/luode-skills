...
- 文档落盘闸门不只约束需求入口、需求与实施计划全量顺序实施方案、实施总览/实施周期、验收文档、Bug 入口、测试总结和总审查入口；需求补齐/边界/拆分/变更、实施周期调整、Bug 复现/根因/运行时诊断/修复建议/回归风险、测试策略/命名/程序/目录/散落资产治理等中间链路，只要本轮已经形成应持久化结论，最终收口前同样必须联动 `artifact-delivery-gate-rules`；实施域还要检查新项目 / 多来源对象总顺序、周期顺序、期次定位、周期内最小任务顺序和已执行最小任务闭环状态
...
- 一旦用户明确开始实施，且计划与停止边界齐备，就默认先定位当前实施周期（第一期、第二期、第三期等大进度），再在当前周期内按最小任务自动串行推进“最小任务实现 -> 最小任务真实测试 -> 最小任务审查 -> 最小任务验收 -> 下一最小任务”；每个最小任务都必须先完成自己的真实测试、审查、验收闭环，当前周期全部最小任务收口后再进入下一周期，不再等待用户逐步下达“进入测试/进入审查/继续下一个任务或周期”，也不得先连续实现多个任务后统一测试 / 审查 / 验收
...
| `final-acceptance-rules`      | 在测试和审查完成后生成独立的最终验收文档，基于验收标准逐条给出放行结论，并检查实施范围内周期已按顺序收口、最小任务已有实现 / 真实测试 / 审查 / 验收证据。                                 |

...
- `autonomous-execution-rules` 负责“开工后自动推进”，但它不能替代开工授权：只有用户明确“开始实施/开始实现/开始执行/直接做/继续做完/按文档实现”，且已有执行计划、完成定义、停止条件和最大推进边界后，才允许从实施文档切进正式编码；开始后只推进原执行计划内未完成的必需动作，最终收口只允许“原执行计划内未完成必需项 / 阻断项 / 用户显式要求的建议/backlog”三类合法后续，其他可选优化默认不输出下一步
- `implementation-planning-rules` 负责正式编码前的只读计划模式：计划阶段禁止写代码、禁止边计划边试做，只允许读仓库、定依赖、列风险、拆任务和写实施文档；计划优先按“依赖图 + 垂直切片”拆最小闭环，不按前端 / 后端 / 数据库水平分层堆任务，单任务默认尽量控制在约 5 个文件以内
...
- “开始实施/开始实现/开始执行/直接做/继续做完/按文档实现”等开工词不是无边界授权；必须已有执行计划，或先给出包含完成定义、停止条件和最大推进边界的本轮计划，缺少计划或停止条件时不得直接实现
- 一旦用户明确开始实施，且计划与停止边界齐备，就默认先在当前实施周期内按最小任务自动串行推进“最小任务实现 -> 最小任务测试 -> 最小任务审查 -> 最小任务验收 -> 下一最小任务”；每个最小任务都必须先完成自己的测试、审查、验收闭环，当前周期全部最小任务收口后再进入下一周期，不再等待用户逐步下达“进入测试/进入审查/继续下一个任务或周期”

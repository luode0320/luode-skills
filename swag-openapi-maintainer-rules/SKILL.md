...
- 单接口文件名默认采用“路径名 + 中文简要说明”格式，兼顾稳定主键与人工可读性。
...
6. 按 `references/naming-rules.md` 生成单接口文件名：基础路径名在前，中文简要说明在后；中文后缀需做字符清洗与长度截断。
...
- 单接口文件命名符合“路径名 + 中文简要说明”规则；method 冲突处理、字符清洗和长度截断稳定。
...
- 单接口 YAML 导入时默认新增父目录。
- 单接口文件名未追加中文简要说明，或中文后缀只能靠任意猜测、随机命名或未清洗非法字符得到。

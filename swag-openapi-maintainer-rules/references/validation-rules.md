...
10. 单接口文件名符合“路径名 + 中文简要说明”规则；若回退纯路径文件名，manifest 必须显式标记 `summary_source: unresolved`。
...
- 若单接口 YAML 带 `tags`、文件名不符合新规则、manifest 映射缺 `summary_source`，或字段中文说明缺失，视为 skill 未通过，不得以“可导入”代替“规则合格”。

...
- `code-quality-rules` 与 `code-style-consistency-rules` 负责 client/响应结构体代码的可读性和审查；本文件不要求修改业务代码。
- `api-contract-rules` 约束本项目对外暴露接口的请求模型；上游请求属于出站调用发现，不能反向修改业务 DTO 契约。

...
3. 需求主文档、前置验收标准和实施规划都已真实落盘后，用户再明确“开始实施 / 开始实现 / 开始执行 / 直接做 / 继续做完 / 按文档实现”，且已有执行计划、完成定义、停止条件和最大推进边界后，才允许从实施文档切入正式编码；缺少计划或停止边界时，只能先补受限计划并停在计划收口处。
...
- discovery 输出后仍缺关键业务取舍：一次只推进一个真实关键缺口问题，不预填业务答案，不把 agent 猜测包装成待确认选项。
...
- 需求稳定后才移交前置验收标准与实施域；正式需求主文档未真实落盘前，不得进入实施规划或正式编码；实施文档确认后，仍必须等待用户明确“开始实施 / 开始实现 / 开始执行 / 直接做 / 继续做完 / 按文档实现”，并确认执行计划、完成定义、停止条件和最大推进边界齐备，才能进入正式编码。
- 实施规划阶段默认保持只读；计划优先按“依赖图 + 垂直切片”拆最小闭环任务，单任务默认尽量控制在约 5 个文件以内。
- 一旦用户已明确开始实施，且计划与停止边界齐备，后续应按“最小任务实现 -> 最小任务测试 -> 最小任务审查 -> 最小任务验收 -> 下一最小任务 -> 当前周期收口 -> 下一周期”的顺序自动推进；每个最小任务都必须先完成自己的测试、审查、验收闭环，而不是每步重新向用户要指令。

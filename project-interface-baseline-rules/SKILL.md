...
如果需要执行测试、判定接口响应、输出上线放行结论，转交 `project-interface-release-execution-rules`；如果是任务级功能验证或改动影响面回归，分别转交 `functional-validation-rules`、`test-regression-rules`；如果是测试文档组织或目录结构，转交 `test-doc-rules`、`test-task-root-layout-rules`。
...
- 冷启动涉及创建测试任务根目录、生成初版测试计划骨架时，联动 `test-task-root-layout-rules` 和 `project-interface-release-execution-rules` 完成对应骨架，本 skill 只负责基线资产骨架本身。
...
- 所有归档遵循 `artifact-storage-rules` 和 `test-doc-rules` 的要求。

...
- 当前仓库功能验证默认属于本地测试：自动化只能使用 `local` 环境信息执行，不得改动 `test` 配置，也不得连接 `test` 环境数据库、缓存或其他隔离测试服务。
...
4. 确认测试资源、测试文档和执行环境已经具备基本验证条件；需要连接本地真实环境（数据库、缓存等）时，按 `test-strategy-rules` 的「本地环境配置发现与连接」去本地 `local` 配置文件读取连接信息，并遵守其隔离安全约束；不得回退使用 `test` 环境配置。

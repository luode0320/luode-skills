...
description: 当需要验证新功能、修改后的功能、接口行为、页面交互、输入输出结果是否满足当前需求、当前变更和验收标准时触发。负责界定本次功能验证范围、验证步骤、通过驳回标准和结论留痕；必须以 `artifact-storage-rules` 与 `test-location-rules` 为基准，把功能验证结论写回中央约定的测试任务主说明 `README.md`，并把详细执行证据放到同一时间戳根目录下的 ASCII 真实代码路径镜像目录中；不要用它代替 test-strategy-rules、test-location-rules 或 test-regression-rules。
---
...
- 不负责测试目录、命名、测试程序和测试文档组织，但必须服从 `test-location-rules` 的新结构。
...
- 驳回：当前变更存在与需求或验收标准不符的功能问题；或结论无法回溯到明确的测试资产与证据；或功能验证留痕继续散落在错误目录中。

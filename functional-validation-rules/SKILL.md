...
description: 当需要验证新功能、修改后的功能、接口行为、页面交互、输入输出结果是否满足当前需求、当前变更和验收标准时触发。负责界定本次功能验证范围、验证步骤、通过驳回标准和结论留痕；必须以 `test-location-rules` 为基准，把功能验证结论写回 `test/yyyy-MM-DD_HHmmss/测试任务中文简介/README.md`，并把详细执行证据放到同一时间戳根目录下的 ASCII 真实代码路径镜像目录中；不要用它代替 test-strategy-rules、test-location-rules 或 test-regression-rules。
---
...
- 将功能验证结论记录到 `test/yyyy-MM-DD_HHmmss/测试任务中文简介/README.md`。
- README 至少包含验证对象、验证范围、执行环境、步骤摘要、结果结论、未通过项和下一步建议。
- 详细执行步骤、截图、接口返回样例、补充案例和原始日志统一放到 `test/yyyy-MM-DD_HHmmss/<ASCII 真实代码路径镜像>/` 下。

...
- `~/.codex/config.toml` -> `model_provider` 对应 provider 的 `base_url`
- 运行时内部仍桥接为 `OPENAI_API_KEY` / `OPENAI_BASE_URL`，因为当前 CLI 使用 OpenAI-compatible SDK
...
- Use `codex-auth:active_provider_api_key` to bridge the current local Codex auth key.
- Use `codex-config:active_provider_base_url` to bridge the current provider URL from `~/.codex/config.toml`.

...
def _download_image_bytes(url: str) -> bytes:
    request = Request(url, headers={"User-Agent": "codex-imagegen/1.0"})
    with urlopen(request, timeout=120) as response:
        return response.read()


def _extract_image_bytes(item: Any) -> bytes:
    ...
    _die("Image API response item did not contain usable b64_json or url data.")
...
async def _run_generate_batch(args: argparse.Namespace) -> int:
    ...
    async def run_job(i: int, job: Dict[str, Any]) -> Tuple[int, Optional[str]]:
        ...
        try:
            ...
            images = [_extract_image_bytes(item) for item in result.data]

...
description: 当问题需要构造步骤、确定触发条件、判断是否稳定发生、确认出现频率或复现环境时触发。负责输出复现路径、稳定性判断、Bug 根目录记录和无法复现时的结论处理；不要用它代替根因分析。
---
...
5. 复现成功后转 `bug-scoping-rules`、`bug-root-cause-rules` 或 `bug-runtime-debug-rules`；若稳定性不足但怀疑时序问题，可继续转运行时诊断路径。
...
- 实际复现脚本、模拟数据和验证程序仍应按 `test-location-rules` 放在 `test/` 目录；本 skill 只统一 Bug 结论记录落点。
...
- 将复现步骤、环境、输入数据和结论统一记录到项目根目录 `bug/` 下的当前 Bug 根目录中。
- 当前 Bug 根目录必须包含 `README.md`，至少写明前置条件、操作步骤、结果表现和稳定性判断。
- 如果本轮复现使用了脚本、模拟数据或验证程序，应按 `test-location-rules` 放入 `test/` 任务目录，并在当前 Bug 根目录中记录对应测试目录路径。

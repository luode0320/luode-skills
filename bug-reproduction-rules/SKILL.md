...
4. 若复现需要连接本地真实环境（数据库、缓存等），按 `test-strategy-rules` 的「本地环境配置发现与连接」去本地配置文件读取连接信息，并遵守其隔离安全约束。
...
- 实际复现脚本、模拟数据和验证程序仍应按 `artifact-storage-rules` 与 `test-task-root-layout-rules` 约定放在统一测试目录；若当前资产原本散落在错误位置，还需同步遵循 `test-scattered-asset-location-rules`；本 skill 只统一 Bug 结论记录落点。
...
- 如果本轮复现使用了脚本、模拟数据或验证程序，应按 `artifact-storage-rules` 与 `test-task-root-layout-rules` 放入对应测试任务目录；若资产原本散落在错误位置，还需先按 `test-scattered-asset-location-rules` 收拢，并在当前 Bug 根目录中记录对应测试目录路径。

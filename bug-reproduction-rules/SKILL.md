...
如果当前基础入口信息还不足，请先转 `bug-intake-rules` 的 `discovery-and-gap` 条件路由；如果已经进入代码和运行时定位，请转交相邻定位 skill。
...
- 实际复现脚本、模拟数据和验证程序仍应按 `artifact-storage-rules` 与 `test-strategy-rules 的 test-asset-governance 条件路由` 约定放在统一测试目录；若当前资产原本散落在错误位置，还需同步遵循 `test-strategy-rules 的 test-asset-governance 条件路由`；本 skill 只统一 Bug 结论记录落点。

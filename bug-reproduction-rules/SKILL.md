...
description: 当问题需要构造步骤、确定触发条件、判断是否稳定发生、确认出现频率或复现环境时触发。负责输出复现路径、稳定性判断和无法复现时的结论处理；不要用它代替根因分析。
---
...
2. 梳理环境、账号、数据、输入和前置操作。
3. 尝试收敛出最小步骤，而不是堆一长串无关动作。
4. 记录复现结果、稳定性和失败时的差异。
...
- 将复现步骤、环境、输入数据和结论记录到 `analysis/` 或测试记录。
- 归档内容至少包含前置条件、操作步骤、结果表现和稳定性判断。

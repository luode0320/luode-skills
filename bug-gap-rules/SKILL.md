...
4. 把缺口按阻断级和非阻断级排序。
5. 若缺口涉及本地环境信息（数据库、缓存等连接条件），优先按 `test-strategy-rules` 的「本地环境配置发现与连接」去本地配置文件查找，能从配置发现的不计入待补缺口，并遵守其隔离安全约束。
6. 输出待补信息列表和补齐优先级。
...
6. 可通过看代码 / 查日志 / 只读连本地库比数据主动查到的缺口，先回流 `bug-discovery-rules` 侦察解决，不计入待补缺口；缺口补齐后再进入 `bug-reproduction-rules` 或 `bug-root-cause-rules`；若基础问题描述本身混乱，可回流 `bug-intake-rules`。

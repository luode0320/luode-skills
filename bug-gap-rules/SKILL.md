---
name: bug-gap-rules
description: 当 Bug 描述缺少复现条件、环境信息、输入数据、报错日志、影响范围或关键时间线，导致后续复现与定位无法可靠推进时触发。负责识别缺失项、区分阻断级与非阻断级缺口，并阻止盲目进入定位。
---

# Bug 缺口识别规则

只在 Bug 已经被提出来，但入口信息还不够支撑复现、归属或定位时使用这个 skill。
如果当前已经具备足够信息开始复现或定位，请转交 `bug-reproduction-rules`、`bug-scoping-rules` 或 `bug-root-cause-rules`。
...
2. 如需继续展开，再读 `references/blocking-signals.md`，需要判断哪些缺口会阻断推进。
3. 需要对照边界或正反例时，再读 `references/gap-examples.md`，需要对照缺口识别正反例。
4. 输出 Bug 缺口清单、阻断判断和补充优先级。
5. 缺口补齐后再进入 `bug-reproduction-rules`、`bug-scoping-rules` 或 `bug-root-cause-rules`；若基础问题描述本身混乱，可回流 `bug-intake-rules`。
...
- 将缺口清单、阻断结论和补充优先级记录到 `analysis/`。
- 归档内容至少包含当前已知信息、缺失项、影响面和下一步动作。
- 如果缺口来自历史信息断裂，应保留来源说明，便于后续追溯。

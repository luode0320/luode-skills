...
| 3 | `CODEBUDDY_AUTOCOMPACT_PCT_OVERRIDE` | 环境变量 | 压缩阈值退回默认 90%，拖到紧急压缩才触发 | `60` |
| 4 | `CODEBUDDY_CODE_MAX_TURNS` | 环境变量 | 单回合工具调用上限退回默认 500 | `1000` |
| 5 | `CODEBUDDY_MAX_RETRIES` | 环境变量 | 失败重试次数退回默认，429/超时更易中断 | `15` |
| 6 | `CODEBUDDY_PRE_MESSAGE_COMPACT_PCT` | 环境变量 | 消息前压缩阈值退回默认 | `10` |
...
- **默认值来源**：官方 env-vars 文档 + 本机 `ACC_PRODUCT_CONFIG_V3` 的 `tokenUsageThresholds`（`inputTokens.emergency: 0.9` 即默认 90%）

| 变量 | 目标值 | 设计意图 |
|---|---|---|
| `CODEBUDDY_AUTOCOMPACT_PCT_OVERRIDE` | `60` | 更早触发压缩（60%），避免拖到 90% 紧急压缩导致输出截断 |
| `CODEBUDDY_CODE_MAX_TURNS` | `1000` | 单回合工具调用上限放宽（默认 500），长任务少被步数截断 |
| `CODEBUDDY_MAX_RETRIES` | `15` | 失败重试拉满（上限 15），网络/瞬时错误自动恢复 |
| `CODEBUDDY_PRE_MESSAGE_COMPACT_PCT` | `10` | 消息前压缩阈值 10%，用户发消息前先压缩再处理 |

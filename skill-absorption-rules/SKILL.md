...
8. **环境依赖吸收（2026-08-23 补，防配置漂移）**：吸收经验不能只吸收"规则正文"——规则依赖的**环境配置**（环境变量 / 宿主配置 / hook / 依赖安装 / 路径引用）离开当前机器就会失效。任何吸收或内部更新，只要来源条目隐含环境依赖（出现"设置环境变量 / 修改配置 / 部署 hook / 安装依赖 / 引用路径"等信号），必须：① 在裁决表登记「环境依赖」列（`N/A + 理由` 或依赖项清单）；② 落盘时把依赖定义写入 manifest（单一权威，其余 skill 引用不重复定义）；③ 环境依赖 ≥ 1 项的 skill 必须附带自检能力（SKILL.md 内「环境自检」小节，或 `env-bootstrap-check.py` 式脚本支持 `--check/--dry-run/--fix`）；④ 换机器/干净环境模拟是效果验证的必做场景。执行细则见 `references/env-dependency-absorption.md`，WorkBuddy 平台级配置权威清单见 `references/workbuddy-env-manifest.md`。
9. **通用性优先（agent 无关，2026-08-30 补，防单 agent 绑定）**：吸收 / 优化改进的 skill 默认要求**通用所有 agent 都能使用**，不得与指定 agent 工具（如 Codex、Claude、ZCode、WorkBuddy 等）绑定。裁决来源条目时，凡结果依赖或声明特定 agent 专属能力的一律拒绝或改写为通用形态：① **禁止专属路径 / 配置 / hook**——不得写入 `~/.claude/`、`~/.codex/`、`~/.workbuddy/`、codex/claude 专属 MCP、专属 hook 等单一 agent 才生效的落点或机制；② **禁止单 agent 声明**——不得在产物中声明"仅适用于某 agent"（如 `agent_created` 限定、`tool:` 单 agent 前缀、正文"仅 Codex"字样）；③ **禁止单 agent 术语 / 命令**——不得出现只对某一 agent 生效的专属命令、环境变量、会话/任务 ID 读取方式；④ **要求通用落点**——吸收成果优先落到所有 agent 都能读取的通用位置（如仓库根 `skills/`、纯 `SKILL.md + references` 结构），而非工具专属路径。本条为**软性优先**：确有特殊场景需绑定单 agent 时允许例外，但**例外必须在裁决表登记理由**（登记「agent 通用性」列：`通用` 或 `例外: <绑定对象> + <为什么必须绑定>`），不允许未登记静默绑定。本条与第 8 条「环境依赖吸收」互补：第 8 条关注配置离开机器失效，本条关注机制离开特定 agent 失效。
...
4. 用户确认后按"单一可编辑资产"原则逐 skill 落盘：新增或修改 reference，同步更新 SKILL.md 的 References 引用；**同步执行存量整理**——合并目标文件中与新增内容同义的重复段落、消除冗余引用、删除已过时规则（语义零丢失），整理动作与新增内容一起计入 diff。**同步执行环境依赖登记**——按 `references/env-dependency-absorption.md` 第 2 节逐条判定来源条目是否隐含环境依赖，隐含则登记「环境依赖」列并写入 manifest；环境依赖 ≥ 1 项的 skill 落盘时必须附带自检能力（SKILL.md「环境自检」小节或自检脚本）。
...
9. 全部完成后更新裁决表与来源记录，检查文件 UTF-8、引用链一致、无断链；若外部通道且源是本地安装（第 0 步场景），删除源 skill 目录并把"已删除源"写入登记；输出收口说明（调整了什么[吸收或内部更新] / 拒绝了什么 / **整理去重了什么（含同域清理位置）** / **同域冗余扫描结论** / **环境依赖登记 X 项 / 自检能力 Y 项** / 净增体积变化 / 分数变化 / 删除了哪个源[外部通道]）。
...
- 不把吸收 / 优化结果绑定到单一 agent 工具（Codex / Claude / ZCode / WorkBuddy 等）：产物默认要求通用所有 agent，确需绑定的例外必须在裁决表「agent 通用性」列登记理由（见设计内核第 9 条）。
...
- 通过：能说清来源拆成了哪些原子条目（外部精华或内部调整诉求或执行中 gap）、每条三态裁决及理由、落点文件、**整理去重了什么（合并段落 / 消除引用 / 删除过时规则，含同域清理位置）**、**同域冗余扫描结论（范围 / 发现 / 清理 / PASS-FAIL）**、**环境依赖登记与自检能力（环境类条目必须有）**、净增体积变化、8 维评分前后对比，并完成登记与验证。执行中 gap 回补时，能明确说清是哪个 skill 不完善、缺在哪里、为什么影响当前任务、建议补哪些文件、是否阻断、补完后从哪一步继续（`resume-workflow.md`）。
- 驳回：外部通道没有拿到原文就凭印象吸收、内部通道调整诉求不明却硬改、裁决表只有"不错/有用"没有理由、调整后未跑棘轮验证、**只增不减导致文件膨胀（新增内容未伴随同等规模的存量整理，且无正当理由）**、把外部整套工作流原样复制进来、**或未执行同域冗余扫描 / 扫描发现可清理冗余（重复段落、门控层叠、散落产物）却未在同一调整闭环内清理即收口**、**环境类条目未登记环境依赖或未附带自检能力即收口**、**吸收产物绑定单一 agent 工具（Codex / Claude / ZCode / WorkBuddy 等）却未在裁决表「agent 通用性」列登记例外理由即收口**、或执行中 gap 回补时把业务问题误判成 skill 问题。

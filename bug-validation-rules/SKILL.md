...
6. 若验证依赖自动化测试或本地环境连接：验证对象是接口时，走 apifox 真实测试（按 `test-strategy-rules` 的《接口测试执行通道（强制）》在 apifox「AI 团队」对应项目执行并落地用例，environment 指向 localhost）；代码级验证走本地测试。只能使用 `local` 环境信息，禁止改动 `test` 配置，禁止连接 `test` / `prod` / `staging` 环境数据库、缓存、消息队列、HTTP/RPC 上游或其他非 local 服务。
...
- 不在未验证关键路径时轻易宣告问题关闭。
- 实际测试脚本、测试数据和测试说明仍应放在 `artifact-storage-rules` 约定的测试目录；本 skill 只统一 Bug 结论记录落点。
...
- 通过：能够说明原问题是否已消失、验证覆盖了哪些关键路径、是否存在副作用和未覆盖项，以及是否具备关闭条件；接口类验证有 apifox 用例与运行记录可追溯（caseId/报告链接内联）；相关结论统一落在当前 Bug 主文档下。
...
- 若验证依赖浏览器、第三方接口或授权环境，必须同时读取 `../artifact-delivery-gate-rules/references/review-acceptance-gate-contract.md`，区分可继续、受限交接和正式放行。
- 只有关闭必需验证无法继续且无替代验证，或修复问题必须回改时，必须读取 `../artifact-delivery-gate-rules/references/task-blocker-closure-contract.md`；受限交接和普通待补测不适用。

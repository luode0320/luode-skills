...
6. 明确测试环境、鉴权信息、依赖数据要求，准备测试前置条件；若接口依赖外部服务，检查环境配置中的代理设置（如 `httpConfig.proxy`）是否正确启用。
7. 按 `references/test-data-construction-rules.md` 查询数据库获取真实数据，识别接口对应的数据库表，为每个必测接口构造请求参数。
...
6. 为每个必测接口准备测试用例：按 `references/test-data-construction-rules.md` 查询数据库获取最近真实记录，构造请求参数、预期语义规则、环境依赖和清理方式；不得用不存在的数据测试并接受失败为通过。

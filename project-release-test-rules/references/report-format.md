...
结论：本模板用于统一说明上线前接口测试是否支持发布；影响：发布决策者可以了解覆盖、风险和阻断情况；范围：覆盖测试概览、基线对账、接口明细和门禁结论；非范围：不以未核实的接口或环境问题替代真实发布判断；变化：先说明发布相关结果，再在附录保留执行和追踪材料；完成标准：所有必测接口均有结果、依据和清晰门禁结论；术语说明：无技术术语需要解释；验证状态：上线测试执行后以 local 环境的真实结果确认。

本文件定义测试报告的标准化输出格式，接口测试明细不得使用 Markdown 表格，必须使用块状格式；请求参数和简要响应必须是格式化的 JSON 字符串。
...
- local 环境、执行步骤、精确命令、接口样本、完整日志、SQL、接口报文、原始响应、清理和回滚：放入当前测试任务的 ASCII 镜像目录。
- `interface-test-results.md` 继续保持本模板规定的块状接口明细格式，不因附录分层改为表格。

## 追踪附录

- 接口标识、来源、测试 ID、基线和双索引证据、依赖追踪、门禁结论、产物路径和机器可读定位。
- 附录维护规则：执行附录与追踪附录按此顺序连续位于文档末尾，不得在追踪附录后追加正文。

...
- 注释质量：只要本轮存在代码改动，就必须读取并执行 `comment-placement-granularity-rules` 与 `comment-completion-gate-rules`；本总审查只消费两个 Owner 的 PASS / FAIL、缺口和证据，不复制函数头、步骤、字段或补丁注释细则。任一 Owner 为 FAIL 时不得以“通过审查”收口。
...
若本轮存在代码改动，则 `comment-placement-granularity-rules` 与 `comment-completion-gate-rules` 不再是“可建议回流”的弱提示，而是当前总审查的必读必查项；未取得两个 Owner 的 PASS / FAIL 和可复核证据前，不得给出“审查结论: 通过”。

...
6. 无论是用户显式要求审查当前改动，还是最终收口前自动总审查，都必须归档到 `doc/审查/`；禁止因为“未发现阻断项”而省略审查文档落盘。
...
- 显式总审查：默认将审查报告归档到 `doc/审查/`。
- 自动总审查：同样必须将审查报告归档到 `doc/审查/`，即使结论为“未发现阻断项”也要保留正式审查记录。
- 归档内容至少包含审查范围、问题分级、关键证据、剩余风险和下一步动作。
- 进入最终回复前，必须联动 `artifact-delivery-gate-rules` 核对当前审查报告是否已经真实落到 `doc/审查/`；未落盘不得判定总审查完成。

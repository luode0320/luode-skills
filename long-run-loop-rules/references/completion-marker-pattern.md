...
> 来源：吸收 `loop-engineering` 反模式「反馈信号污染」。完成标记是 worker 的**自报信号**，不是完成依据。

- 检测到完成标记后，控制器**不得直接收口**，必须先做独立验证
- 独立验证手段（至少一项，验证动作与结果写入状态文件）：
  1. 真实运行测试/校验命令，核对退出码与断言结果
  2. 确认产物文件真实存在且内容非空（必要时校验哈希/大小）
  3. diff 或变更清单人工核对，确认与目标一致
  4. 类型检查 / lint 真实通过
- 独立验证失败但标记存在 -> 按「智能熔断」处理，不得放行
- 平台无独立验证工具时，由独立 worker（非执行 worker）复检，并如实记录降级

...
- 别名: 归档闸门, 收口前落盘检查
- 类型: 流程规则
- 定义: 需求、实施、验收、Bug、测试、审查任务在最终收口前必须联动 `artifact-delivery-gate-rules`，核对主文档、配套 SVG、README、需求与实施计划全量顺序实施方案、实施总览/实施周期、验收文档和证据路径是否已经真实落盘到 `doc/2-需求/`、`doc/3-实施/`、`doc/4-bugs/`、`doc/5-tests/`、`doc/6-审查/`、`doc/7-验收/`；实施域还必须核对新项目 / 多来源对象总顺序、周期顺序、期次定位、周期内最小任务顺序和已执行最小任务的实现 / 真实测试 / 审查 / 验收状态，未落盘或缺闭环状态不得判定任务完成。

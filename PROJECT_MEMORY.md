...
- 类型: 流程规则
- 定义: 来源对象文档（需求或 Bug）、前置验收标准和实施总览/实施周期即使都已完成，也不构成自动开工授权；必须由用户在当前任务中明确说“开始实施”“开始实现”“开始执行”“直接做”“继续做完”或“按文档实现”，且当前任务已有执行计划、完成定义、停止条件、最大推进边界和验证点，才允许从实施文档切入正式编码。实施周期是计划管理容器，不是默认执行粒度；真正执行单元必须先拆到最小任务。若用户给出开工类指令但没有计划或停止条件，必须先补本轮受限计划并停在计划收口处，不得直接实现或进入长文本无限执行。开工后按 `autonomous-execution-rules` 默认遵循“当前实施周期内最小任务A实现 -> 最小任务A测试 -> 最小任务A审查 -> 最小任务A验收 -> 最小任务B…… -> 当前实施周期收口 -> 下一实施周期”的自动串行闭环，除阻断级事件外不再等待用户逐步下达“进入测试/进入审查/继续下一个任务或周期”指令。
...
- 类型: 测试域核心规则
- 定义: 上线前项目级全接口测试门禁，替代人工接口回归验证，输出上线准入结论，所有测试资产落地到doc/5-tests/对应时间戳根目录，强制接口明细用块状格式、请求响应为JSON字符串，Agent自动判定接口是否通过
- 来源: skill/project-release-test-rules/SKILL.md
- 适用范围: 全项目上线前接口测试、回归验证、上线准入判定
- 更新时间: 2026-06-30 17:21:01
- 状态: 启用

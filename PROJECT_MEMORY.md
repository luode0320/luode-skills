...
- 类型: 流程规则
- 定义: 来源对象文档（需求或 Bug）、前置验收标准和实施总览/实施周期即使都已完成，也不构成自动开工授权；必须由用户在当前任务中明确说“开始实施”“开始实现”“开始执行”“直接做”“继续做完”或“按文档实现”，且当前任务已有执行计划、完成定义、停止条件、最大推进边界和验证点，才允许从实施文档切入正式编码。实施规划阶段默认采用只读计划模式：禁止写代码、禁止边计划边试做，只允许读仓库、定依赖、列风险、拆任务、写实施文档。实施周期是计划管理容器，不是默认执行粒度；真正执行单元必须先拆到最小任务，并优先按“依赖图 + 垂直切片”组织，避免按前端 / 后端 / 数据库水平分层堆计划。单任务尽量单次专注完成，默认控制在约 5 个文件以内；明显超过则继续拆分。若用户给出开工类指令但没有计划或停止条件，必须先补本轮受限计划并停在计划收口处，不得直接实现或进入长文本无限执行。开工后按 `autonomous-execution-rules` 默认遵循“当前实施周期内最小任务A实现 -> 最小任务A测试 -> 最小任务A审查 -> 最小任务A验收 -> 最小任务B…… -> 当前实施周期收口 -> 下一实施周期”的自动串行闭环；每个最小任务都必须先完成自己的测试、审查、验收，才允许进入下一个任务。
...
### Windows / WSL 执行边界
- 别名: 普通命令不默认进 WSL, 执行类动作才进 WSL, Git Bash 优先
- 类型: 环境规则
- 定义: 当项目代码位于 WSL 文件系统内且 agent 运行在 Windows 时，默认 shell 优先 Git Bash 或稳定 `bash`；搜索、读写文件、规则检查、普通 git 盘点等非执行动作留在 Git Bash / bash。只有编译、运行、启动程序、测试、调试，以及会真实启动运行时的依赖安装，才通过 `wsl.exe --cd /home/<user>/<project> <command>` 进入 WSL。纯 Windows 项目，或当前任务本身不需要启动/执行程序时，不应误触发 WSL 执行规则。
...
- 2026-06-30：统一实施执行口径为“最小任务闭环优先于实施周期浏览”，并统一主执行链术语为“实现 -> 测试 -> 审查 -> 验收”。
- 2026-06-29：将实施执行粒度从“实施周期闭环”细化为“最小任务闭环”；实施周期继续作为文档管理容器，真正执行顺序改为每个最小任务依次完成实现、测试、审查、验收后再进入下一个最小任务。

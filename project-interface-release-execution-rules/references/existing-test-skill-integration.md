...
| `test-strategy-rules 的 test-asset-governance 条件路由` | 测试目录的创建和结构规范 | 本 skill 必须遵循测试双根规则：可执行测试脚本、mock、stub、fake、fixture、helper 和数据构造统一落在根 `test/`，源码关联资产按被测源码路径镜像；`doc/5-tests/` 只保存扁平中文主报告 md，机器产物落在 `test/release-artifacts/YYYY-MM-DD_HHmmss_release-interface-test/` |
| `test-strategy-rules 的 test-asset-governance 条件路由` | 测试文档的结构和归档规则 | 本 skill 的报告必须遵循它的要求，`doc/5-tests/` 只保存扁平中文主报告 md；响应、报告、日志和对账结果等机器产物落在 `test/release-artifacts/YYYY-MM-DD_HHmmss_release-interface-test/` |
...
| `test-strategy-rules 的 test-asset-governance 条件路由` | 散落测试资产的收拢规则 | 本 skill 的活动测试代码和可执行资产必须收拢到根 `test/` 的源码镜像（跨源码复用资产进入 `test/shared/`）；中文主报告收拢到 `doc/5-tests/` 的扁平 md，机器产物收拢到 `test/release-artifacts/YYYY-MM-DD_HHmmss_release-interface-test/`，不得散落 |
...
1. 本 skill 的测试目录创建和结构必须完全遵循 `test-strategy-rules 的 test-asset-governance 条件路由` 的双根要求，不得自定义目录结构；可执行测试脚本、mock、stub、fake、fixture、helper 和数据构造位于根 `test/` 源码镜像，`doc/5-tests/` 仅承载扁平中文主报告 md，机器产物落在 `test/release-artifacts/YYYY-MM-DD_HHmmss_release-interface-test/`。
2. 本 skill 的测试文档、测试报告和归档必须完全遵循 `test-strategy-rules 的 test-asset-governance 条件路由` 和 `artifact-storage-rules` 的要求。
3. 本 skill 的接口测试执行可以调用 `functional-validation-rules` 完成，结果直接复用，不需要重复测试；调用所需的可执行脚本与模拟程序仍遵循根 `test/` 源码镜像规则。

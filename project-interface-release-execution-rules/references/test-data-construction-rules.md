...
本文件定义 `project-interface-release-execution-rules` 在准备接口测试请求参数时，如何从可复用参数、上游接口、local 数据库 / 缓存、OpenAPI 示例、fixture 和明确规则中构造可追踪参数，避免用不存在的数据测试并接受失败为"符合预期"。
...
6. **fixture 样本**：使用项目已维护的 fixture、mock 数据或测试样本文件；本项仅指非可执行样本数据，模拟程序仍按根 `test/` 源码镜像规则存放。

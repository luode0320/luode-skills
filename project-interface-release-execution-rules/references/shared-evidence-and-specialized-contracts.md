...
- 已有脚本能覆盖时必须复用；缺能力时优先扩展已有通用脚本；项目专属适配配置（仅允许非可执行 YAML/JSON 等配置）写入项目 `doc/5-tests/基线/script-adapter.yaml` 或当轮测试任务目录；可执行适配器、mock、stub、fake、fixture、helper 必须放入根 `test/` 并按被测源码路径镜像，不写死到通用脚本。

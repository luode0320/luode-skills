...
| 10 | 会话动态重命名规则 | `BODY_THREAD_TITLE` |
| 11 | 注释任务强制流程 | `BODY_COMMENT_TASK` |
| 12 | 上下文压缩续做规则 | `BODY_CONTEXT_COMPRESS` |
| 13 | 文件编码与写入规则 | `BODY_CHINESE_ENC` |
| 14 | 变更最小化 | `BODY_MIN_CHANGE` |
| 15 | 本地连接调试测试红线（最高优先级，强制） | `BODY_LOCAL_ONLY_CONNECTION` |
| 16 | 依赖与工具复用优先规则 | `BODY_REUSE_FIRST` |
| 17 | 最小实现优先级阶梯 | `BODY_MINIMAL_LADDER` |
| 18 | 输出格式规则 | `BODY_OUTPUT_FORMAT` |
| 19 | Windows / WSL 执行规则 | `BODY_WINDOWS_WSL` |
| 20 | CodeGraph 强制准备规则 | `BODY_CODEGRAPH` |
| 21 | 代码库探索规则 | `BODY_CODE_EXPLORE` |
| 22 | 插件检测安装规则 | `BODY_PLUGIN` |
| 23 | 图像生成强制规则 | `BODY_IMAGEGEN` |
| 24 | Godot 项目工具配置（仅 Godot 项目） | `GODOT_TOOLING_SECTION` |
| 25 | 图像生成配置（仅 Godot 项目） | `GODOT_IMAGEGEN_SECTION` |

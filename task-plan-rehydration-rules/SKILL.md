...
- `version: 2` 允许 `projection_origin=persisted|synthesized` 与 `synthesis_mode=none|exact|fallback`；`fallback` 允许空 `source_document`，但 `plan_key` 必须以 `SYNTH-FALLBACK/` 开头。

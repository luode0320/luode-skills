...
结论：本模板用于把整体实施方案交接清楚；影响：业务读者可以了解计划带来的结果和推进方式；范围：覆盖方案、周期、任务、测试和风险的总体安排；非范围：不直接执行具体任务或自行补充未确认的实现细节；变化：把业务说明与执行、追踪信息分层呈现；完成标准：每个周期和任务都能找到明确的进入、完成与停止依据；术语说明：无技术术语需要解释；验证状态：计划阶段按本地真实验证结果确认。
...
正式计划必须脱离当前会话独立执行，不得依赖思考过程、悬浮窗或隐含工作目录：

- 新会话接手第一步：
- 主项目名称与项目根（本机绝对路径或仓库 URL）：
- 主项目仓库类型与代码基线（commit；无 commit 时写可验证文件指纹）：
- 计划源文件与版本：
...
无外部引用时必须在“外部项目代码引用”行写 `N/A + 原因 + 证据`；仅写项目名、相对路径或“参考旧项目”不合格。
...
- local 环境、执行顺序、精确命令、样本、日志、SQL 和接口报文：填写本次实施所需的可操作细节。
- 清理与回滚步骤：逐项回指“真实测试安排”和“风险、阻断、回滚与最大推进边界”。

## 追踪附录

- 稳定 ID、来源、证据、追踪矩阵和机器可读定位：回指“最小任务清单与追踪矩阵”和各周期文档。
- 附录维护规则：执行附录与追踪附录必须连续位于文档末尾，禁止在其后追加业务正文。

...
5. 如果当前运行在 Windows 下，看代码/改代码/git 操作（提交、拉取）用 Git Bash；编译、运行、测试、调试、依赖等一切执行类命令（`go build`、`go run`、`go test`、`dlv`、`go mod`）都走 WSL（`wsl.exe -e bash -lc "..."`）；涉及写文件操作时必须显式指定 UTF-8 编码。

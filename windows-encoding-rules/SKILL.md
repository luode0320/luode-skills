...
description: 当任务运行在 Windows（PowerShell 或 CMD）且涉及任意代码、文档、配置、脚本、测试资产或生成文本的读写、日志追加、README/Markdown 修改、脚本输出重定向、Git 提交信息或终端显示时触发。用于预防与修复中文乱码（mojibake）、统一跨平台 UTF-8 写入策略，并在落盘前做编码自检；尤其适用于“看到乱码”“中文变成问号/锟斤拷/Ãxx”“同文件中英正常但中文异常”“追加写入后部分行乱码”“担心文件被 GBK/ANSI/默认编码写入”等场景。
---
...
- 用户提到 `Git Bash`、`WSL`、`\\wsl.localhost`，或者明确希望在 Windows 下建立默认执行约束，避免 Windows PowerShell 未初始化 UTF-8 时导致换行/编码漂移。
...
5. 如果当前运行在 Windows 下，先检查当前 PowerShell 会话与用户级 PowerShell 配置是否已经固定为 UTF-8；未固定时，优先执行 `scripts/enable_powershell_utf8.ps1` 完成永久化初始化，再继续后续命令。
6. 若 PowerShell UTF-8 已固定，则 Windows 下默认 shell 可以使用 PowerShell；涉及写文件操作时仍必须显式指定 UTF-8 编码，并在落盘后核对换行是否被转换。
7. 先检查仓库是否缺少 `.gitattributes` / `.editorconfig`，以及 Git 是否开启了 `core.autocrlf=true` 或 `core.filemode=true`。
...
```
- 若要把当前用户 PowerShell 永久切到 UTF-8，优先执行：
```powershell
powershell -ExecutionPolicy Bypass -File .\windows-encoding-rules\scripts\enable_powershell_utf8.ps1
```
- 永久化脚本会同时补齐 Windows PowerShell 5.1 与 PowerShell 7 的用户级 profile，并写入 UTF-8 初始化片段；若脚本未执行成功，不得把“Windows 默认 PowerShell”视为已满足前提。
- 永久化脚本还应确保当前用户的 PowerShell 执行策略至少允许本地 profile 加载；若 profile 文件已落盘但新会话仍因 execution policy 被拦截，则视为脚本仍未完成闭环。
- 脚本执行完成后，应继续用新的 PowerShell 子进程验证 `[Console]::InputEncoding`、`[Console]::OutputEncoding`、`$OutputEncoding` 与 `chcp` 是否都已经切到 UTF-8；只有验证通过，才算满足默认 shell 前提。
...
- 对 Windows 下的仓库写入前，优先确认当前 PowerShell 会话是否已完成 UTF-8 初始化；若未完成且又无法先运行永久化脚本，则记录为环境阻断，不要直接把未初始化的 PowerShell 当默认 shell 使用。
...
1. 编码前提：先检查 PowerShell 是否已完成 UTF-8 永久化；未完成时先运行 `scripts/enable_powershell_utf8.ps1`，并在新的 PowerShell 子进程里做一次 UTF-8 验证。
2. 会话预设：设置 `InputEncoding`、`OutputEncoding` 与 `$OutputEncoding` 为 UTF-8。

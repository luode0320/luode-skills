...
- 实现风格是否与项目现有写法明显冲突。
- 实现结构是否存在显著可维护性问题。
- Go 项目是否存在 `doc/5-tests/` 外 `*_test.go` 落点违规（命令示例：`rg --files -g "*_test.go" | rg -v "^doc/5-tests/"`）。
...
- 高复用通用代码是否命中“7天冻结”仍直接改旧行为（命令示例：`git log -1 --format=%ct -- <file>`，并结合引用范围人工复核）。
...
- 输出中应包含通用代码 7 天冻结检查结论（命中路径 + 是否采用新增兼容方案）。
...
- 若需求涉及新增文件，需求文档必须逐条写明每个新文件的生成目录与目标文件名。
...
12. 若涉及新增文件，文档中必须补齐“新增文件清单（目录 + 文件名 + 用途）”；缺目录信息视为文档未收口。
13. 若涉及数据库新表或字段变更，文档中必须补齐对应 SQL（`CREATE TABLE` / `ALTER TABLE`）；缺 SQL 视为文档未收口。
14. 同步按 `artifact-storage-rules` 命名模板产出需求流程图（`.flow.svg`）与需求时序图（`.sequence.svg`）。
...
- 若涉及新增文件，必须在归档中包含“新增文件清单”，至少写明目录、文件名和用途。

...
- 若用户提供需求截图或需生成外部图片素材，必须统一保存到 `doc/需求/data/`，并在需求主文档中使用 Markdown 图片语法直接引用展示。
- 在需求文档落地时，除主文档外必须同步产出 SVG 流程图与 SVG 时序图，统一按 `artifact-storage-rules` 命名模板保存到 `doc/需求/`。
...
5. 先探索当前项目上下文；若用户只给 idea 或缺失信息可通过项目资源主动获得，先转交 `requirement-discovery-rules` 侦察，再用“一次一个关键问题”的方式补齐仍无法获得的目的、约束、成功标准和关键前提。
...
15. 若用户提供需求截图或需生成外部图片，统一保存到 `doc/需求/data/`，并在需求主文档中补齐 Markdown 图片引用（例如 `![登录页原型](data/2026-05-25_103000_支付路由改造.req-shot-login.png)`）。
...
20. 若用户在确认需求文档时已明确“开始执行”，默认激活连续执行状态，后续实现与测试阶段按 `autonomous-execution-rules` 直接推进，除阻断级节点外不再重复确认。
...
- 若用户提供需求截图或需要外部图片素材，必须归档到 `doc/需求/data/`，并在需求主文档中使用 Markdown 图片语法直接引用展示；仅保存文件不引用视为不合格。
...
- 进入最终回复前，必须联动 `artifact-delivery-gate-rules` 核对需求主文档、需求流程图、需求时序图以及 `doc/需求/data/` 资产引用是否已经真实落盘；未落盘不得判定需求文档已完成。

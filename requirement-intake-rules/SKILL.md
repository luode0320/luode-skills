...
- 若用户提供需求截图或需生成外部图片素材，必须统一保存到 `ment/data/`，并在需求主文档中使用 Markdown 图片语法直接引用展示。
...
15. 若用户提供需求截图或需生成外部图片，统一保存到 `ment/data/`，并在需求主文档中补齐 Markdown 图片引用（例如 `![登录页原型](data/2026-05-25_103000_支付路由改造.req-shot-login.png)`）。
...
- 若用户提供需求截图或需要外部图片素材，必须归档到 `ment/data/`，并在需求主文档中使用 Markdown 图片语法直接引用展示；仅保存文件不引用视为不合格。

...
- 当前仓库测试脚本、数据构造脚本、mock 辅助脚本默认都属于本地测试资产：只能读取 `local` 环境信息，禁止改动 `test` 配置文件，禁止把默认连接目标指向 `test` / `prod` / `staging` 环境数据库、缓存、消息队列、HTTP/RPC 上游或其他非 local 服务。
...
- 只负责测试程序的职责划分和辅助代码边界，不负责决定测试目录落点，那属于 `test-strategy-rules 的 test-asset-governance 条件路由`；散落资产治理属于 `test-strategy-rules 的 test-asset-governance 条件路由`，Go 编译路径属于 `go-test-compile-path-rules`。
...
> 本节是写接口（createTransaction、createOrder、cancelOrder、refund 等）测试脚本必须满足的最小控制台过程日志规范。写接口测试失败时若缺少关键步骤日志，定位时间会显著拉长；本节确保任何写接口测试脚本都能快速复现失败现场。
...
- 缺任一项过程日志的写接口脚本视为脚本不完整，必须补齐后才能进入正式测试。

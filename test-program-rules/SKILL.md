...
- 驳回：测试程序继续散落在业务目录、仓库根目录、中文说明目录或随意目录中；把中文引入 Go 可编译路径；通过源码目录 `*_test.go` 处理白盒诉求而不补 seam；测试脚本无关键过程控制台日志，仅输出最终结果；临时调试脚本长期滞留并伪装成正式测试资产；或第三方 API 响应结构不明时直接跳过探测脚本、盲写 map 解析。

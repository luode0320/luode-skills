...
```go
// 在 main.go 同目录创建 main_mock.go
//go:build mock

package main

import (
    mock_api "your-project/mock/internal/business/scalp/api"
)
...
当需要 mock 多个服务（交易所、推送、数据库）时，使用统一工厂装配：
...
1. 创建 `mock/<源码相对路径>/` 目录
2. 复制 mock 文件，改包名为 `mock_<源包名>`
3. 文件头追加 `//go:build mock`
4. 在装配层按构建标签选择实现
5. 删除源码目录的 mock 文件
6. 验证：`go build -tags mock ./...` 通过

### 场景四：测试排除运行时 Mock

运行时 Mock 默认不参与测试编译，因为 `go test` 不携带 `-tags mock`：

```bash
# 测试代码不包含运行时 Mock
go test ./test/...           # 正确：不包含 mock 标签
go test -tags mock ./test/...  # 错误：运行时 Mock 不应参与测试
```

### 场景五：真实项目迁移（F:\binance-wangge-go 案例）

该项目 internal/business/scalp/api/mock_gateway.go 同时被测试和运行时使用：

- MockGateway 结构体 + NewMockGateway() 被 	est/binance/gateway_test.go 等测试文件直接引用
- NewExchangeGateway() 被 main.go 运行时路由使用

迁移策略：

1. 在 mock/internal/business/scalp/api/gateway.go 创建运行时 Mock 文件（包名 mock_api，//go:build mock）
2. 保留源码目录的 MockGateway 和 NewMockGateway() 供测试使用（这是测试 Mock，不是运行时 Mock）
3. 将 NewExchangeGateway() 从 mock_gateway.go 拆出，移到 internal/assembly/ 或直接由 main.go 的条件编译工厂替代
4. main.go 按构建标签选择实现：//go:build mock 使用 mock_api.NewMockGateway()，//go:build !mock 使用 pi.NewFuturesGateway()
5. 删除源码目录的 mock_gateway.go 文件
6. 验证：go build -tags mock ./... 通过，go test ./test/... 通过，go build ./... 通过

关键技术点：运行时 Mock 与测试 Mock 可以共存。测试 Mock 保持原包名和路径，运行时 Mock 走根 mock/ 的独立包。
...
- 发现测试程序准备放到 `testing/`、`doc/5-tests/`、仓库根目录、业务目录、中文说明目录或其他非根 `test/` 位置。
...
2. 再读 `../artifact-storage-rules/references/path-map.yaml`、`../artifact-storage-rules/references/naming-templates.md` 与 `../artifact-storage-rules/references/update-policy.md`，确认根 `test/` ASCII 镜像路径、时间戳主说明 `README.md` 和证据目录的复用边界。
...
- 在定位根 `test/` 镜像、时间戳主说明 `README.md` 或判断是否继续沿用同一轮证据目录时，先读 `../artifact-storage-rules/references/path-map.yaml`、`../artifact-storage-rules/references/naming-templates.md` 与 `../artifact-storage-rules/references/update-policy.md`。

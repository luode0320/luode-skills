...
- 正式测试程序、模拟程序和验证脚本必须跟随 `test-strategy-rules 的 test-asset-governance 条件路由`，统一落在根 `test/` 的 ASCII 真实代码路径镜像中；若涉及 Go 可编译路径，还必须同步遵循本 skill《Go 测试编译路径（强制）》节。
- Go 白盒诉求也必须遵循同一落点规则；不得通过源码目录同包 `*_test.go` 绕开。
- `doc/5-tests/` 的时间戳说明目录只保留 README、证据和非可执行产物，不承载任何真实测试代码。

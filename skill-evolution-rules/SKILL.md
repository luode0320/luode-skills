...
- 当前任务涉及 Windows 下的仓库文件写入或格式化，而现有规则没有明确约束默认优先 Git Bash / WSL shell。
...
7. 如果 gap 是“Windows 下默认 shell 选择”这类稳定规则，优先回补到编码/仓库级 skill，而不是仅在单次任务里口头提醒。
...
- `execution-failure-learning-rules` 负责真实执行失败的分类、证据、恢复、复验和 candidate 生成；本 skill 只负责判断 owner Skill 是否缺少承接结构。
- 没有 owner 案例库时，不在运行中临时创建无归属文件；先记录候选的 owner、错误特征、根因、验证证据和建议落点，再按本 skill 的补旧 Skill / 新增相邻 Skill 决策矩阵处理。
- 候选未完成根因确认、同输入复验、脱敏或唯一归属判断前，不得写入 active，也不得把业务 Bug 伪装成 Skill 经验。

...
- 发送前自检（全程硬自检，逐条不可跳过）：**每一次**工具前导语 / 中间叙述 / 最终回复发送前，先自检草稿是否为简体中文；一旦发现成段非中文（英文 / 日文等）自然语言，必须改回简体中文后再发送，代码符号、命令、路径、原始字段名、报错原文等技术片段除外。此自检覆盖任务全程的过程叙述，不只在最终总结阶段生效。
- 思考 / 推理通道（reasoning）也应使用简体中文；此项为尽力而为，模型可能不完全遵守，且该通道未持久化、难以事后验证，但不得因此放弃前导语与叙述的中文自检。
...
- 需求、验收标准、实施总览、实施周期和最小任务文档默认采用极致完整性等级：结构不得删减，条件字段必须填写或写 `N/A + 原因 + 证据`。
...
- 每个实施最小任务必须逐个完成“实现 -> 真实测试 -> 审查 -> 验收”，写精确 local 命令、样本、断言、失败预期、清理、回滚、完成/停止条件和最大推进边界；禁止先做多个任务再统一验证。
- 文档落盘后必须运行 `artifact-delivery-gate-rules/scripts/validate_engineering_docs.py` 对应 profile；机器校验失败不得用最终回复口头说明替代。
- 需求、实施、计划、审查、验收、Bug、测试和交付文档的正文必须使用普通业务读者能理解的中文，先说明结论、影响、范围、变化和完成标准；技术细节、命令、稳定 ID、机器字段、追踪矩阵和证据统一放入执行附录或追踪附录。审查或验收不适用时写明原因和依据但不阻断，只有明确必需且无替代验证时才阻断。

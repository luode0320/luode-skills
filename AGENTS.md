...
- 需求、实施总览、实施周期和最小任务文档默认采用极致完整性等级：结构不得删减，完成条件、异常边界、停止条件和测试映射字段必须填写或写 `N/A + 原因 + 证据`；`AC-*` 继续作为实施计划内的可测试条件标识。
- 文档必须面向普通模型零决策执行：强模型冻结业务/技术/测试决策，执行模型不得自行补默认值、异常、权限、兼容、文件/符号落点、测试断言、回滚或停止边界。
- 必须维护 `SRC -> DEC -> REQ/RULE -> AC -> CYCLE -> TASK -> 文件/符号 -> TEST -> EVIDENCE` 双向追踪；孤立 ID、未决 P0/P1、失效链接或图文术语不一致均阻断。
- 需求 L2 及以上必须按语义提供流程图和时序图；实施总览必须有边界图、周期依赖图和端到端图，实施周期必须有任务依赖图及领域匹配图。图前写目的和关联 ID，Mermaid 必须可解析。
- 每个实施最小任务必须逐个完成“实现 -> 真实测试 -> 6-review 风格回归”，写精确 local 命令、样本、断言、失败预期、清理、回滚、完成/停止条件和最大推进边界；禁止先做多个任务再统一验证。
- 文档落盘后必须运行 `artifact-delivery-gate-rules/scripts/validate_engineering_docs.py` 对应 profile；机器校验失败不得用最终回复口头说明替代。
- 需求、实施、计划、Bug、测试、`6-review` 和交付文档的正文必须使用普通业务读者能理解的中文，先说明结论、影响、范围、变化和完成标准；技术细节、命令、稳定 ID、机器字段、追踪矩阵和证据统一放入执行附录或追踪附录。历史审查/验收资料只读，不参与活动收口。
...
- 默认口径：当前会话只能修改与本会话绑定的当前项目；其他项目（含同一父目录一层并列的兄弟项目）的代码和文件默认只读。
- 只读允许：读取其他项目的代码、配置、README、文档，以及 `git log`、`git show`、`git status` 等只读盘点命令，均免确认。
- 例外通道（会话级跨项目写入授权）：当任务确需写入 / 更新另一个项目（多项目功能对接、跨项目改造、联动修复等）时，必须走「跨项目写入授权」契约，取得用户当轮确认后生效：
  - 授权请求必须写明：目标项目名称 + 绝对路径、授权范围（目标文件 / 符号 / 模块，或整个项目）、操作类型（文件写入、构建测试、依赖安装、Git 写操作）、目标项目当前分支与 commit（版本冻结）、改动意图、验证方式、回滚方案。
  - 用户在当前轮显式确认后，生成一条会话级授权记录 `WRT-*`；本会话内对该目标项目 + 该授权范围 + 该操作类型持续有效。
  - 授权记录必须留痕：写入当前项目 `PROJECT_CURRENT.md`，或在会话内明确声明，至少含 `WRT-*` 编号、目标项目绝对路径、授权范围、操作类型、授权时间、用户确认来源。
  - 缺少目标项目绝对路径、授权范围或操作类型任一项时，不得生成授权，不得执行任何写入。
- 授权生效后的允许动作：在授权范围内，可对目标项目执行文件写入 / 创建 / 删除 / 重命名、命令执行 / 构建 / 测试 / 依赖安装，以及 Git 提交 / 推送；该会话级授权即视为目标项目写 Git 历史的当轮显式授权。
- 授权失效条件：会话结束、用户撤销、目标项目或范围超出授权、版本冻结值发生漂移时，授权立即失效，回到默认只读，须重新申请。
- 授权不覆盖的独立红线（分属本文件其他章节，仍按各章执行，不因跨项目授权而放开）：`local` 本地连接调试测试红线、凭据不回显、个人文件安全与删除保护、当前项目自身的 Git 历史写入仍遵循「当轮显式意图」要求。
- 未获授权时的唯一出口：只允许产出**改动计划**（目标项目绝对路径、目标文件/符号、改动意图、建议改法和验证方式，必要时给出 patch 文本），交用户在目标项目目录下新开会话执行；计划本身只能写入当前项目或直接回复用户，不得写入其他项目目录。
- 读取其他项目不等于获得其运行环境授权：不得用其配置连接数据库、缓存、消息队列或上下游服务，也不得启动其服务来补证据；`local` 连接红线仍以当前项目的 `local` 配置为准。
- 定位其他项目的发现顺序与只读 / 授权细则见 `implementation-planning-rules/references/sibling-project-discovery.md`；Bug 跨项目对照排查见 `bug-intake-rules`，跨项目 CodeGraph 查询见 `codegraph-analysis-rules`。
...
## 测试不得污染生产代码红线（最高优先级，强制）

- **完全禁止为了测试目的改动生产代码**。测试只能验证生产本来就有、生产路径本来就在用的能力，不允许先造一段专供测试的代码再去测它。
- 禁止在生产代码中新增：仅测试调用的函数与方法、仅供测试消费的静态数据（常量数组 / 切片 / map）、仅测试读写的结构体字段、仅测试可达的分支（`if isTest`、`env == "test"`、`Getenv("*TEST*")`）、与生产构造器重复的测试专用第二入口。
- 测试需要的静态参数、种子数据、固定样本、假依赖和构造逻辑，一律放在根 `test/` 的源码镜像目录里，由测试侧调用生产已导出的 API 落地。
- 判定采用**引用面判据**，不看写代码时的意图：生产目录中的符号，若全部非定义引用都来自 `test/` 且生产引用数为 0，即判定为测试污染。“将来生产也会用”“这是通用初始化能力”不构成放行理由。被污染符号引用的静态数据按级联污染一并判定。
- 白盒测不到时按三级替代出路降级：黑盒验证 → 通过 local 真实环境的可观测结果做端到端行为断言 → 记为“待补测风险”写入测试主文档。不测比造污染代码好。
- 生产代码重构（依赖注入、接口抽象）允许做，但只能以**生产价值**立项并走正常需求 / 实施流程，理由必须能独立成立“即使没有测试需求这个改动也应该做”，且改完生产 wiring 真实使用、生产行为不变、无仅测试可达入口。以“为了能测”为唯一理由的改动一律驳回。
- 生产代码改动收口前与 `git commit` 前必须执行 `python test-strategy-rules/scripts/scan_test_pollution.py --root . --diff-only`；`POLLUTION: FAIL` 阻断收口与提交，须先迁移到 `test/`，或在项目根 `.test-pollution-allowlist` 登记豁免并写明理由。
- 判据细则、五类污染模式、正反例、豁免清单与存量治理四步由 `test-strategy-rules` 的 `references/production-test-pollution.md` 唯一定义；`test-program-rules`、`code-change-finalization-gate-rules` 和 `git-collaboration-rules` 只消费其结论，不复制判据。
- 违反本条视为最高级别流程违规。

## 夹具与守卫参照物稳健性（强制）

> 详细判据、反例与复核口径见 `test-program-rules/references/fixture-and-guard-robustness.md`。本节为仓库级最小约束摘要。

- 夹具与守卫的判断依据必须是**当场推导的事实**，不得是外部事实的快照。统一复核口径：把系统时间往后调一个月、把被引用的文件改名或移走、让生产数据格式升级——该用例或守卫是否仍以清晰原因失败，而不是静默改变判定结果或报出无关异常。
- 禁止把绝对日期当成“近期 / 仍有效”的夹具时间；时间语义为“足够近”或“仍在窗口内”时，必须相对当下取值，或把时钟作为参数注入。
- 禁止把仓库内既有文件路径当作夹具输入而不自证其存在性；优先动态发现，其次当场自建夹具，两者都不可行时保留显式路径但必须紧邻存在性前置断言，让失效的第一现场是“夹具缺失”。
- 禁止夹具、守卫或解析器持有生产数据格式的手抄副本（手写字段清单、键名或层级结构）；必须复用生产的解析 / 校验 / 归一入口，让格式变更时生产侧与消费侧同时失败、漂移双向可见。
- 守卫做广域扫描时必须显式声明忽略口径，忽略项集中一处定义并由所有扫描点统一调用；基线清单必须与实际状态对账，并写明更新触发条件，长期为红且无合法更新路径的基线视为守卫失效。
- 修正上述任一类问题时，必须同时检查同类写法是否存在于别处，不得只修报错的那一处。
- 违反本条视为最高级别流程违规。

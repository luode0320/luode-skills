...
sync_agents_file() {
  ...
  sync_section "$file" "严禁脑补工具调用与结果（最高优先级，强制）" "- 任何对文件、命令、搜索、网络的读取与执行，必须通过真实工具调用（独立 tool call）完成；严禁在回复正文里编写 \`<invoke>\` / \`<result>\` / 伪 function_calls 文本假装调用工具，也禁止凭记忆“想象”文件内容当作已读取结果。
- 引用任何文件内容、行号、函数名、配置值前，必须来自本轮真实工具返回；未真实读取不得断言具体代码或数据。
- 若发现输出出现大段重复行、错乱或重复行号、源码文件莫名以 Markdown 代码块结束符收尾、import 与实际用法矛盾等异常，立即判定为生成异常：停止后重新发起真实工具调用，并用 \`md5sum\` / \`wc -c\` 等独立命令交叉校验再继续。
- 关键文件读取建议附带指纹校验（\`md5sum\` + \`wc -c\`），确保所读即磁盘真实内容。
- 违反本条视为最高级别流程违规。"

  sync_section "$file" "Skill 命中强制规则" "- 处理本仓库任务时，必须先命中并加载至少三个基础 skill。
- 最低要求：至少命中 \`skill-hit-check-rules\`、\`parallel-task-dispatch-rules\`、\`reasoning-summary-structure-rules\`。
- 若本轮涉及创建、补齐或更新仓库级 \`AGENTS.md\`，默认额外启用 \`project-agents-bootstrap\` 进行自举补齐；该规则同样适用于其他项目仓库。
- 对已经存在的 \`AGENTS.md\`，也必须继续做增量同步与受管章节 upsert，而不是只初始化一次。
- 必须在首条中间进度明确输出当前命中的 skill 列表。

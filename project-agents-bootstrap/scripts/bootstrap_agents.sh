...
- 本仓库命中 Godot 项目标记后，后续涉及场景、资源、脚本、运行验证或截图的任务，默认优先通过本地 `godot4` / `godot` 或项目自带启动脚本与 Godot 编辑器配合执行。
- 修改 Godot 场景、运行游戏、抓取截图或读取编辑器状态前，必须先确认本机存在可用的 Godot 可执行入口，并明确本轮实际使用的启动命令。
- 若当前环境暂无可用 Godot 可执行入口，可继续处理纯文档、纯规则或不依赖编辑器运行态的代码文件，但最终回复必须说明哪些 Godot 运行态检查未完成。
TEMPLATE
)

...
description: 当新增或修改请求参数、DTO、body 结构、参数校验或请求模型时触发。负责统一请求结构、参数表达和基础校验边界；必须以 api-endpoint-rules 为基准，只使用 POST 请求，所有参数通过 JSON body 传递；若本次改动影响 Swagger/OpenAPI 请求模型、字段说明或示例，应与 api-swagger-rules 同步生效；不要用它代替接口入口设计、响应结构或业务规则本身。
---

# 接口请求规则

只在判断"请求参数该放在哪里、怎么表达、怎么校验"时使用这个 skill。
如果当前问题是入口职责、响应结构、Swagger/OpenAPI 请求文档同步或错误处理，请转交相邻接口类 skill。
...
**重要：请求解析必须使用明确结构体（DTO）承接，禁止使用 `map[string]interface{}`、`any`、`json.RawMessage` 作为请求绑定入口。**

## Skill 作用与适用场景

- 统一使用 JSON body 作为唯一的请求参数载体（不使用 path、query 参数）。
- 强制请求解析使用明确结构体（DTO），字段通过 `json` tag 显式声明。
- 统一必填字段、可选字段和基础参数校验表达方式。
- 约束请求结构只表达输入契约，不吞掉业务规则本身。
- 要求请求 DTO、字段必填/可选、枚举和示例与 `api-swagger-rules` 保持同步。
...
3. 明确请求 DTO 结构，字段命名与 `json` tag 一一对应，禁止弱类型入参。
3. 判断字段属于写入载荷还是辅助上下文。
4. 明确哪些字段必须传、哪些字段可选。
...
6. 如果当前接口需要 Swagger/OpenAPI 文档或调试入口，请同步交给 `api-swagger-rules` 更新请求模型文档。
7. 请求结构稳定后，再交给响应、错误处理等后续 skill 继续收口。
8. 当存在“先接 map 再手动取值”的旧代码时，优先改为 DTO + `ShouldBindJSON` 的结构化解析。

## 权责边界与不负责事项

- 只负责请求结构和基础参数校验，不代替 `api-endpoint-rules` 设计入口职责。
- 不代替 `api-response-rules` 设计返回结构。
- 不代替 `api-swagger-rules` 维护 Swagger/OpenAPI 请求字段说明、示例和调试文档。
- 不代替业务层判断复杂业务规则是否成立。
- 不把鉴权、租户、trace 等请求头问题混入参数模型设计。
- 必须遵循 `api-endpoint-rules` 的约定，只使用 POST + JSON body。
- 不允许用 `map[string]interface{}`、`any`、`json.RawMessage` 作为请求入口解析容器。
...
- 通过：能明确字段落点在 JSON body、请求模型边界、必填 / 可选表达、基础参数校验层次、controller 中正确使用 ShouldBindJSON 进行参数绑定和错误处理，且请求体解析使用明确 DTO。
- 驳回：使用了 path 或 query 参数、DTO 与内部模型混淆、把业务规则硬塞进参数校验、controller 中没有使用 ShouldBindJSON、请求体仍用 map/any/RawMessage 解析，或让请求结构失去稳定语义。
...
func (ctl *OrderController) Create(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		response.Fail(c, "invalid request")
		return
	}
	// req 已是强类型结构体，后续直接传 service 层
}
```
...
ctype, _ := body["cType"].(string)
```

## 执行结果归档要求

- 如果本次定义了新的请求模型、调整了关键字段或改变了校验层次，将结论记录到 `analysis/` 或 `doc/6-review/`。

...
description: 当为当前需求、当前 Bug 或当前验证任务新增测试任务目录，或需要决定 `doc/tests/` 根目录下的当天时间戳根目录、中文说明目录和 ASCII 真实代码路径镜像布局时触发。负责统一测试任务根目录创建、当天时间戳校验、中文 README 说明目录和真实测试资产镜像布局；不要用它代替散落测试资产迁移、Go 编译路径冲突处理、测试命名规则或测试程序实现规则。
---
...
- 决定新的测试任务应该放到 `doc/tests/` 根目录下的哪个当天时间戳目录。
...
- 统一使用 `doc/tests/` 作为测试根目录，不得改用 `tests/`、`qa/`、`verify/` 等名称。
...
- 通过：当前测试任务已放在 `doc/tests/` 根目录下的当天时间戳根目录中，中文说明目录和 `README.md` 已就位，真实测试资产镜像目录保持 ASCII，且没有把新文件误写到未经允许的历史目录。

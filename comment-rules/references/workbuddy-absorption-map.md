...
## 内部更新：同构语句组内禁止插入多行块注释（2026-09-08）

**来源**：内部调整（无外部 skill）。用户在 ellipal_buysell_go 项目贴出 `internal/provider/buy/init.go` 的 `Init()` 正反对照：三行服务商注册本已是统一的行尾短注释并按列对齐，其中一行上方被插入 3 行块注释解释 moonpay 与 `onramper.excludedRamps` 的上线耦合，用户判定"绝对禁止，注释风格不一致、很乱"。

**通道**：内部更新通道。**裁决结果**：合并 2 条 / 拒绝 1 条。

| 条目 | 本地现状 | 裁决 | 落点 | 整理去重 | 环境依赖 | agent 通用性 |
|------|---------|------|------|---------|---------|-------------|
| 一组逐行平行的同构语句（注册列表、初始化列表、路由表、枚举常量、结构体字段）已形成统一行尾短注释风格时，禁止在其中某一行上方插入多行块注释 | `comment-placement.md` 第 9 条只说"块注释用于说明整体意图"，未约束块注释与同组行尾注释混用；反例库无同类条目 | 合并（紧随第 9 条，作为其边界收敛） | `comment-rules/references/comment-placement.md` 末条 | 不新起小节，挂在既有"块注释 vs 短注释"条目之后，共用同一判据语境 | N/A（纯文本规则，不依赖环境变量/配置/hook） | 通用（无 agent 专属路径与术语） |
| 背景、上线顺序、耦合约束、风险权衡等确需记录的信息改写函数头注释、模块文档或提交说明 | 无（原规则只说"该写在哪"，没说"被禁的内容改写去哪"，容易被理解成信息只能丢弃） | 合并 | 同上，与上一条同句收敛 | 与上一条合并为一条规则的"禁止 + 去处"，不拆成两条 | N/A | 通用 |
| 正反例落地 | 反例库承载写码前规避用的正反例 | 合并 | `code-style-consistency-rules/references/user-style-feedback-library.md` 新增 `STYLE-CASE-ALL-003` | 条目正文回指 `comment-placement.md` 为规则权威，只放正反例，不复写判据 | N/A | 通用 |
| 同步写进 `code-generation-style-rules` 通用风格契约 | 该 skill 已约定写码前加载反例库 active 条目 | **拒绝** | — | 拒绝理由同 2026-08-21：会形成三处交叉冗余；本条经反例库 `STYLE-CASE-ALL-003` 自动并入本轮风格契约 |  |  |

**同域冗余扫描（落盘后执行）**：范围 = `comment-rules`、`code-style-consistency-rules`、`code-generation-style-rules`、`project-style-rules`。全仓库 `grep -rln "同构语句\|块注释"` 仅命中 `comment-rules/SKILL.md`（职责行，未复写判据）、`comment-placement.md`（权威）、反例库（正反例）。① 重复段落：0 处；② 门控层叠：0 处（未新增门控，复用既有位置分区）；③ 散落产物：0 个（无新文件）；④ 引用链：反例库 → `comment-rules` 位置分区可达。结论 **PASS**。

**净增体积**：`comment-placement.md` +1 行、反例库 +31 行（条目 30 + 变更记录 1）、本登记文件 +1 小节。只增不减的理由：反例库为追加型正反例库，新条目与既有 5 条正交无重复，交叉冗余已通过「单一权威 + 引用」在落盘时规避。

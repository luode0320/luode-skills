...
source_ids: ["SRC-PSR-001", "SRC-PSR-002", "SRC-PSR-003", "SRC-PSR-RPC-003", "SRC-PSR-ADOPT-001", "SRC-PSR-IP-001", "SRC-PSR-GOVERNANCE-001", "SRC-PSR-CLAUDE-001", "SRC-PSR-DATABASE-002", "SRC-PSR-BACKEND-DATA-003"]
status: "accepted"
version: "v2.0"
current_slice: "CYCLE-14 删除后端根 data 目录规则并完成验证"
...
结论：后端项目根不再建立 `data/` 静态数据目录。影响：新项目初始化、目录查询和 strict 检查不会再为无明确职责的根目录保留入口；前端 `src/data/`、业务域数据与 `doc/data/` 保持原规则。范围：后端目录树、Catalog、本地 CLI 测试和研发文档。非范围：不迁移业务仓库、不删除任何用户现有目录、不连接真实数据服务、不执行 SQL 或自动迁移。变化：根 `data/` 成为禁止路径，`query --artifact data` 与 `init --enable backend.data` 均失败关闭。完成标准：render 不出现该节点，strict 拒绝该路径，检查前后样本哈希一致。术语说明：禁止路径是 strict 只读检查必须报告的项目相对路径。验证状态：本地行为测试、审查和验收将在本周期内重验。图片资产决策：N/A。原因：本需求没有视觉验收对象。证据：范围仅包含文本规则与命令行行为。

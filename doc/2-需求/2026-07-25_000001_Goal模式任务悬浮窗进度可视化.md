...
| ID | 内容 | 追踪 |
|---|---|---|
| `SRC-GTP-005` | `SRC-RTP-005/CHG-RTP-001` 新增“超时自动创建或复用 Goal”作为 Goal 生命周期触发来源。 | `REQ-RTP-008/009`、`AC-RTP-010..013` |
| `RULE-GTP-005` | 自动创建成功后仍复用既有固定安全三步；正式最小任务出现后继续按 `REQ-GTP-003` 替换。 | `CYCLE-RTP-06/TASK-RTP-17` |

自动触发不改变 Goal 的 create/restore/blocked/complete 投影语义，也不允许保存 Goal 摘要。失败路径由 RTP Owner 使用原 `ensure-timeout` 降级，不在 Goal 文档复制第二套投影状态机。

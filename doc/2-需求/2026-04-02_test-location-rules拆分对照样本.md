...
| 新 skill | 当前状态 | 说明 |
|---|---|---|
| `test-task-root-layout-rules` | 已创建 | 负责 `doc/5-tests/` 根目录、当天时间戳目录、中文 README 目录和 ASCII 镜像布局 |
| `test-scattered-asset-location-rules` | 已创建 | 负责识别并迁移散落在 `doc/5-tests/` 根目录之外的测试资产 |
...
| R-026 | 根目录只能是 `doc/5-tests/`，不能是 `tests/`、`qa/`、`verify/` 等 | `test-task-root-layout-rules` | `SKILL.md / 强制规则`、`references/task-root-layout.md / 1` | 原样迁移 | 已覆盖 | `test-task-root-layout-rules` | 无 |
...
| R-028 | 旧时间戳根目录按周或按月归档到 `doc/5-tests/archive/` | `test-task-root-layout-rules` | `references/task-root-layout.md / 5` | 原样迁移 | 已覆盖 | `test-task-root-layout-rules` | 无 |
...
| R-030 | 任何与测试相关的文件，只要不在 `doc/5-tests/` 下，就是违规 | `test-scattered-asset-location-rules` | `SKILL.md / 强制规则`、`references/scattered-asset-migration.md / 1` | 原样迁移 | 已覆盖 | `test-scattered-asset-location-rules` | 无 |
...
| R-033 | 任何 `*_test.go` 只要不在 `doc/5-tests/` 根目录下就应迁移并改 seam | `go-test-compile-path-rules`、`test-scattered-asset-location-rules` | `references/go-compile-path.md / 1,3`、`references/scattered-asset-migration.md / 4` | 组合覆盖 | 已覆盖 | `go-test-compile-path-rules` | Go 编译路径整改由 Go skill 主承接，散落识别由禁散落 skill 辅助承接 |
...
- 保留 `doc/5-tests/2026-04-02_231122/...` 下的历史证据作为审计材料，但这些文档不再作为现行入口说明。
...
- 历史证据仅保留在 `doc/2-需求/` 与 `doc/5-tests/2026-04-02_231122/` 这类审计材料中，不再作为运行中的依赖入口。

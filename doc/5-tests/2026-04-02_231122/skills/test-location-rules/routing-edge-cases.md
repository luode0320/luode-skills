...
验证新拆分 skill 集合在“非触发场景”“多 skill 联动顺序”“`doc/5-tests/` 根目录内部的 Go 特例路径”等更容易打架的边界上，是否仍能与旧 `test-location-rules` 对齐。
...
- 场景输入：
  - “我只是在现有 `doc/5-tests/2026-04-02_231122/某任务/README.md` 里补一段测试结论，不移动目录也不新建文件。”
- 旧 skill 预期结论：
...
- 场景输入：
  - “当前仓库没有 `doc/5-tests/`，但根目录已有 `create-order-test.js`，`docs/` 里有 `order_mock.json`，`internal/service/order_service_test.go` 也已经出现了。”
- 旧 skill 预期结论：
...
- 场景输入：
  - “测试文件放在 `doc/5-tests/2026-04-02_231122/登录验证/internal/中文/service/login_test.go`，已经在 `doc/5-tests/` 下了，这样可不可以？”
- 旧 skill 预期结论：
  - 仍然不可以
  - 因为中文进入了 Go 可编译测试路径
- 新 skill 集合预期结论：
  - 主命中 `go-test-compile-path-rules`
  - 即使已位于 `doc/5-tests/` 根目录，也仍要清理中文可编译路径
- 对照结果：
  - 一致
- 观察说明：
...
- 当前未发现“已经在 `doc/5-tests/` 根目录里就默认全都合规”的错误放宽。

## 与主 README 的关联说明

- 本文件是 `doc/5-tests/2026-04-02_231122/test-location-rules拆分对照验证/README.md` 的第二轮补充证据文档。

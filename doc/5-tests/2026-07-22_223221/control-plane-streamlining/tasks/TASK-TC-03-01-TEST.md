# TASK-TC-03-01 真实测试证据

## 测试目标

验证“修复压缩恢复与新会话预热冲突”已经按冻结写集实现，自动触发、用户习惯、安全、授权、停止、回滚和输出协议未被弱化。

## 输入与范围

- 实现范围：context-compression-rules/**、recent-context-bootstrap-rules/**
...
- 通过标准：压缩恢复不再无条件调用近期预热，共享恢复契约和编码许可存在。
...
**PASS**：压缩恢复不再无条件调用近期预热，共享恢复契约和编码许可存在。

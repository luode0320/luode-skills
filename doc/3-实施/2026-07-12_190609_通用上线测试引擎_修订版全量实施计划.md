...
结论：修订计划继续使用唯一执行引擎，并把消费者场景能力追加到既有周期之后。影响：旧接口门禁先作为对照，达标后由场景门禁独立放行。范围：真实网页请求、事件流、实时连接、外部判定、清理和兼容迁移。非范围：其他协议、性能和安全扫描。变化：废弃双来源和双执行入口，新增六个垂直周期。完成标准：连续三次本地全量运行满足覆盖、通过、清理和差异门槛。术语说明：硬切是达到切换条件后不再运行时回退旧门禁。验证状态：用户已授权实施，当前处于契约周期。
...
**2026-07-25 外部整体性测试增强修订**

上一版“新增来源对象或平行 Skill”的方案废弃。本修订继续沿用 `REQ-RT-20260712-001`，由 `project-interface-release-execution-rules` 作为唯一行为 Owner；基线 Skill 只维护接口、事件、消费者和场景事实资产。当前用户已明确实施授权，实施基线改为当前磁盘 `HEAD ca50d023580bb9f2d1966b9023d8a5ec54b9d2ca`，不得回退到计划记录的旧提交。

图形目的：冻结 C13-C18 的消费者场景执行链和门禁切换。关联 ID：`REQ-RT-009..014`、`CYCLE-RT-13..18`。

```mermaid
flowchart LR
    B[接口事件消费者事实] --> G[生成candidate]
    G --> V[确定性验证为verified]
    V --> R[真实HTTP SSE WS Socket.IO]
    R --> O[外部结果 探针 清理]
    O --> S[shadow三次对账]
    S -->|达标| H[scenario硬门禁]
    S -->|未达标| X[BLOCKED]
...
- Schema：现有 IR 保持 `2.0`；新增 `external-scenario/1.0`。
- 生命周期：`candidate -> verified -> stale/quarantined -> retired`；只有契约完整、正向通过、故障可识别、清理成功且来源未漂移时晋级。
- 动作白名单：`http.request`、`sse.expect`、`ws.connect/send/expect/close`、`socketio.connect/emit/expect/disconnect`、`state.probe`、`cleanup`。
- CLI：`external-doctor/generate/validate/verify/run/migrate`；`release-run --gate-mode legacy|shadow|scenario`。
- 退出码：`0=PASS`、`1=FAIL`、`2=契约或CLI无效`、`3=环境或安全BLOCKED`、`4=候选或覆盖PENDING`。
- 产物：`interface-results.json`、`scenario-results.json`、`consumer-coverage.json`、`protocol-capabilities.json`、`cleanup-report.json`、`dual-gate-diff.json` 和脱敏证据清单。

**周期与最小任务**

| 周期 | 最小任务 | 真实测试文件 | 完成条件 |
| --- | --- | --- | --- |
| C13 | `C13-01` 契约加载和 HTTP JSON；`C13-02` 生成、验证、生命周期 | `test_scenario_contract.py` | 合法/非法、漂移、错误晋级 PASS |
| C14 | `C14-01` form/multipart；`C14-02` 下载；`C14-03` SSE | `test_http_external_scenarios.py` | 媒体类型、摘要、断流与重连 PASS |
| C15 | `C15-01` RFC6455；`C15-02` Socket.IO；`C15-03` 跨协议 | `test_realtime_external_scenarios.py` | 握手、鉴权、顺序、重复、ack 和读回 PASS |
| C16 | `C16-01` 探针；`C16-02` 清理；`C16-03` oracle | `test_oracle_cleanup.py` | allowlist、污染阻断、最终一致性 PASS |
| C17 | `C17-01` 报告；`C17-02` shadow；`C17-03` 硬门禁 | `test_dual_gate_migration.py` | 连续三次覆盖/通过/清理/差异门槛成立 |
| C18 | `C18-01` 兼容；`C18-02` 工具环境；`C18-03` 交付 | `test_tool_env_and_trigger.py` | 旧兼容、依赖零污染、字典/validator/审查 PASS |

**真实测试与停止边界**

固定证据根：`doc/5-tests/2026-07-25_133843/project-interface-release-execution-rules/`。主命令为 `wsl.exe --cd /mnt/f/luode-skills python3 -X utf8 -m unittest discover -s "doc/5-tests/2026-07-25_133843/project-interface-release-execution-rules/tests" -p "test_*.py" -v`；同时重跑 `2026-07-12_180240` 和 `2026-07-12_191712` 两轮历史测试，不修改历史期望。

非 local、敏感信息落盘、探针越权、清理失败、runtime 缺失、P0/P1 覆盖不足、双轨差异未解释或 validator 失败时立即停止。每个任务只回滚自身文件和新测试资产；不得修改业务项目生产语义、数据库 schema、其他协议或用户现有工作树，不自动提交 Git。

**增量追踪**

| SRC | DEC | REQ | AC | CYCLE/TASK | TEST | EVIDENCE |
| --- | --- | --- | --- | --- | --- | --- |
| `SRC-USER-RT-002` | `DEC-RT-008..010` | `REQ-RT-009` | `AC-RT-010` | `C13/C13-01..02` | `TEST-RT-C13-*` | `EVD-RT-C13-*` |
| `SRC-USER-RT-002` | `DEC-RT-009` | `REQ-RT-010` | `AC-RT-011` | `C14/C14-01..03` | `TEST-RT-C14-*` | `EVD-RT-C14-*` |
| `SRC-USER-RT-002` | `DEC-RT-009` | `REQ-RT-011` | `AC-RT-012` | `C15/C15-01..03` | `TEST-RT-C15-*` | `EVD-RT-C15-*` |
| `SRC-USER-RT-002` | `DEC-RT-010` | `REQ-RT-012` | `AC-RT-013` | `C16/C16-01..03` | `TEST-RT-C16-*` | `EVD-RT-C16-*` |
| `SRC-USER-RT-002` | `DEC-RT-011` | `REQ-RT-013` | `AC-RT-014` | `C17/C17-01..03` | `TEST-RT-C17-*` | `EVD-RT-C17-*` |
| `SRC-USER-RT-002` | `DEC-RT-008/011` | `REQ-RT-014` | `AC-RT-015` | `C18/C18-01..03` | `TEST-RT-C18-*` | `EVD-RT-C18-*` |

...
**CYCLE-RTP-06 全量执行顺序补充**

**文档定位与维护状态**

| 来源对象 | 当前状态 | 回指 |
|---|---|---|
| `REQ-RTP-001/SLICE-RTP-005` | 已确认、执行中 | `SRC-RTP-005/CHG-RTP-001` |
| `AC-RTP-010..013` | 待最终验收 | RTP 验收标准 |
| `CYCLE-RTP-06` | 执行中 | `2026-07-25_203000_CodexDesktop任务悬浮窗断点恢复_实施周期06_Goal自动升级.md` |

**全量执行顺序**

`TASK-RTP-14 → TASK-RTP-15 → TASK-RTP-16 → TASK-RTP-17 → TASK-RTP-18 → TASK-RTP-19 → TASK-RTP-20 → TASK-RTP-21`。每个任务完成实现、真实测试、审查和验收后才能进入下一个任务。

图形目的：说明 Cycle 06 八个最小任务的串行顺序；关联 ID：`CYCLE-RTP-06`、`TASK-RTP-14..21`。

```mermaid
flowchart LR
    C5["CYCLE-RTP-05 已完成"] --> C6A["需求和周期冻结"]
    C6A --> C6B["probe 与 Goal 编排"]
    C6B --> C6C["规则、字典、回归"]
    C6C --> C6D["Desktop 最终验收"]
```
...
| 当前任务 | 下一步 | 阻断 |
|---|---|---|
| `TASK-RTP-14/15` | 完成文档 profile 后进入 `TASK-RTP-16` | ID 冲突、严格校验失败 |
| `TASK-RTP-16` | 61 项回归通过后进入 Goal 编排 | 无写入语义或兼容回归 |
| `TASK-RTP-17..21` | 按周期文档全量顺序执行 | Goal 无法清理、敏感落盘、UI 不一致 |

**依赖、进入、收口与阻断**

依赖为本地 Python、当前工作树、Goal/update_plan 宿主工具和 v4 registry。Goal 工具不可用时按正式契约降级普通投影，不构成普通悬浮窗阻断；无法证明用户可见 Goal 行为时最终自动 Goal 验收保持受限，不伪报通过。

**自审结论**

Cycle 06 已进入全量顺序，未覆盖 Cycle 05 历史结论；无未决 P0/P1，阻断和回滚边界完整。

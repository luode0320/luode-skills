...
version: "v1.7"
template_version: 1
current_slice: "TASK-14-02 删除后端根 data 目录最终验收"
...
review_acceptance_gates:
  ...
  - stage: acceptance
    ...
    basis: AC-PSR-UTILS-001 至 AC-PSR-UTILS-005、AC-PSR-RPC-001 至 AC-PSR-RPC-003、AC-PSR-ADOPT-001 至 AC-PSR-ADOPT-003、AC-PSR-IP-001 至 AC-PSR-IP-002、AC-PSR-GOVERNANCE-001 至 AC-PSR-GOVERNANCE-002、AC-PSR-CLAUDE-001 至 AC-PSR-CLAUDE-002、AC-PSR-DATABASE-001 至 AC-PSR-DATABASE-003、AC-PSR-BACKEND-DATA-001 至 AC-PSR-BACKEND-DATA-002。
    required_by_source: true
    required_now: true
    completed_validation: ["TEST-PSR-UTILS-001", "TEST-PSR-RPC-001", "TEST-PSR-RPC-002", "TEST-PSR-RPC-003", "TEST-PSR-ADOPT-001", "TEST-PSR-IP-001", "TEST-PSR-GOVERNANCE-001", "TEST-PSR-CLAUDE-001", "TEST-PSR-DATABASE-001", "TEST-PSR-DATABASE-002", "TEST-PSR-DATABASE-003", "TEST-PSR-BACKEND-DATA-001", "TEST-PSR-BACKEND-DATA-002"]
    substitute_validation: []
...
结论：本轮正式验收通过；影响：后端新项目不再获得无职责的根 `data/` 落点，旧项目也不会被自动迁移或删除；范围：根 `data/` 的目录事实、Catalog 禁止路径、CLI 失败关闭、测试、审查与研发文档；非范围：前端 `src/data/`、业务域数据、`doc/data/`、业务仓库迁移、数据库和外部服务、真实网络 RPC、前端工具目录和 Git 历史；变化：删除根 `data/` 及其 `business/`、`project/`、`seed/` 子树的合法目录定义，query、init 与 strict 均失败关闭；完成标准：适用验收项均通过且 strict/adoption 不写入样本；术语说明：strict 是拒绝违规目录的只读检查模式；验证状态：50 项本地行为测试、Python 编译、审查与根 data 负向 CLI 全部通过。图片资产决策：N/A。原因：本轮没有界面或视觉验收对象。证据：范围仅包含本地规则与命令行行为。

## 文档信息

| 项目 | 内容 |
|---|---|
| 验收对象 | `REQ-PSR-BACKEND-DATA-003` 的后端根 data 删除规则、Catalog、CLI 与本地测试资产。 |
...
| 后端根 data 删除 | 根 `data/`、`backend.data` init 和零匹配 query 的反向 fixture。 | render 不显示根 data；query、init 和 strict 稳定失败且不写入。 | 后端重新出现合法 data 入口，或误伤前端、业务域、文档 data。 |
...
- 验收目标：确认后端根 `data/` 已无合法入口，且三类项目原有目录规则仍对同一输入给出唯一、可解释且只读的结果。
- 通过标准：50 项本地行为测试全部成功；适用文档 profile 和目标 Skill 快速校验通过；没有 P0/P1 审查问题。
...
| 测试 | `doc/5-tests/2026-07-28_014412_代码位置目录规则V2/README.md` 与 50 项 unittest | 已通过 test profile 与本地行为测试 |
...
| 最小任务闭环 | 通过 | `TASK-05-01` 至 `TASK-14-02` 均有实现、测试、审查和验收记录 |
| 功能与回归验证 | 通过 | 50 项本地 unittest 覆盖双平台规则文件 query/render/init/strict/bootstrap、IP query、legacy、普通 YAML adoption、无写入、JSON 响应、跨域导入、数据存储目录、字段 SQL 与后端根 data 删除。 |
...
| 后端根 data 删除 | 通过 | Catalog 无合法 data artifact；render 不显示，query/init/strict 失败关闭且前后哈希一致。 |
...
| `CYCLE-14` | 后端根 data 删除已收口 | Catalog 禁止路径、query/render/init/strict 失败关闭与 50 项本地行为测试 |
...
- 若重新引入或调整后端根 `data/`、其旧子树或禁止路径边界，必须重新执行本轮 50 项测试、审查与最终验收。

## 完成条件、停止条件与交付物

| 类型 | 内容 |
|---|---|
| 完成条件 | 所有适用验收场景通过，50 项本地测试、文档 profile、Skill 快速校验和审查证据一致，且 `check` 全程只读。 |
| 停止条件 | 任一严格检查写入 fixture、根 data 重新取得合法入口、误伤非后端 data 路径，或必需门禁失败。 |
...
| `REQ-PSR-BACKEND-DATA-003`、`RULE-PSR-BACKEND-DATA-004` | `AC-PSR-BACKEND-DATA-001`、`AC-PSR-BACKEND-DATA-002` | 禁止路径、`test_catalog_schema.py`、`test_query_render.py`、`test_init_check.py`。 | 通过。 |
...
| `EVD-TASK-14-01-IMPL` | `TASK-14-01` | 根 data 从目录树、Skill 与 Catalog 合法条目中删除，并加入禁止路径。 |
| `EVD-TASK-14-02-TEST` | `TASK-14-02` | 50 项本地 unittest、根 data query/render/init/strict 失败关闭与无写入哈希。 |
| `EVD-TASK-14-02-REVIEW` | `TASK-14-02` | 本实现审查和当前改动审查未发现 P0/P1。 |
| `EVD-TASK-14-02-ACCEPT` | `TASK-14-02` | 本最终验收对两条后端根 data 验收条件逐条 PASS。 |

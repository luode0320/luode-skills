...
version: "v1.5"
template_version: 1
current_slice: "TASK-12-03 双平台规则文件最终验收"
...
结论：本轮正式验收通过；影响：开发者可以唯一查询、初始化、检查和审查根 `utils/`、其中 `utils/ip/`、源码根 `util/`、业务域 `rpc/` 与三类项目根的双平台协作规则，旧项目可在不搬迁的情况下逐步纳入 V2；范围：规则文本、Catalog、CLI、微业务隔离门禁、收敛清单、测试和研发文档；非范围：业务仓库迁移、数据库和外部服务、真实网络 RPC、前端工具目录和 Git 历史；变化：根 `AGENTS.md`、`CLAUDE.md`、`PROJECT_CURRENT.md`、`PROJECT_MEMORY.md`、`PROJECT_HISTORY.md` 改为必需文件，`PROJECT_STYLE.md` 仅显式启用时创建；`CLAUDE.md` 必须与 `AGENTS.md` 正文一致；IP 提取、规范化、公私网判断和国家/地区归属查询继续位于 `utils/ip/`；完成标准：适用验收项均通过且 strict/adoption 不写入样本；术语说明：strict 是拒绝违规目录的只读检查模式，adoption 是依据人工快照渐进采用的只读检查模式；验证状态：41 项本地行为测试、审查、五个文档 profile、两个目标 Skill 快速校验和字典刷新全部通过。图片资产决策：N/A。原因：本轮没有界面或视觉验收对象。证据：范围仅包含本地规则与命令行行为。
...
- 通过标准：41 项本地行为测试全部成功；适用文档 profile 和目标 Skill 快速校验通过；没有 P0/P1 审查问题。
...
| 浏览器和第三方验证 | 本轮没有页面、网络调用或外部服务。 | 不适用；原因：验收范围没有浏览器或外部服务接口；证据：41 项测试仅调用本地 CLI、Git Bash 与临时 fixture。 |
...
| 测试 | `doc/5-tests/2026-07-28_014412_代码位置目录规则V2/README.md` 与 41 项 unittest | 已通过 test profile 与本地行为测试 |
...
| 检查项 | 结果 | 依据 |
|---|---|---|
| 实施周期顺序 | 通过 | `CYCLE-05` 至 `CYCLE-12` 已按顺序收口 |
| 最小任务闭环 | 通过 | `TASK-05-01` 至 `TASK-12-03` 均有实现、测试、审查和验收记录 |
| 功能与回归验证 | 通过 | 41 项本地 unittest 覆盖双平台规则文件 query/render/init/strict/bootstrap、IP query、legacy、普通 YAML adoption、无写入、JSON 响应和跨域导入 |
...
- 若修改双平台规则文件、根 `utils/`、`utils/ip/`、源码根 `util/`、业务域 `rpc/`、收敛清单契约、CLI 参数或跨业务导入白名单，必须重新执行本轮 41 项测试、审查与最终验收。

## 完成条件、停止条件与交付物

| 类型 | 内容 |
|---|---|
| 完成条件 | 所有适用验收场景通过，41 项本地测试、文档 profile、Skill 快速校验和审查证据一致，且 `check` 全程只读。 |
...
- 来源与验收：`REQ-PSR-V2-001`、`REQ-PSR-UTILS-001`、`REQ-PSR-SOURCE-UTIL-002`、`REQ-PSR-CLI-003`、`REQ-PSR-IP-001`、`REQ-PSR-GOVERNANCE-001`、`REQ-PSR-CLAUDE-001`、`REQ-PSR-BUSINESS-RPC-004`、`REQ-PSR-ADOPT-001`、`AC-PSR-UTILS-001` 至 `AC-PSR-UTILS-005`、`AC-PSR-IP-001` 至 `AC-PSR-IP-002`、`AC-PSR-GOVERNANCE-001` 至 `AC-PSR-GOVERNANCE-002`、`AC-PSR-CLAUDE-001` 至 `AC-PSR-CLAUDE-002`、`AC-PSR-RPC-001` 至 `AC-PSR-RPC-003`、`AC-PSR-ADOPT-001` 至 `AC-PSR-ADOPT-003`。
- 实施与证据：`CYCLE-05` 至 `CYCLE-12`、`TASK-05-01` 至 `TASK-12-03`、`TEST-PSR-UTILS-001` 至 `TEST-PSR-UTILS-004`、`TEST-PSR-IP-001`、`TEST-PSR-GOVERNANCE-001`、`TEST-PSR-CLAUDE-001`、`TEST-PSR-RPC-001` 至 `TEST-PSR-RPC-003`、`TEST-PSR-ADOPT-001`。

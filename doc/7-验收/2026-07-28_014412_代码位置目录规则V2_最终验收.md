...
version: "v1.2"
template_version: 1
current_slice: "TASK-09-03 渐进采用最终验收"
...
结论：本轮正式验收通过；影响：后端开发者可以唯一查询、初始化、检查和审查根 `utils/`、源码根 `util/` 与业务域 `rpc/`，旧项目可在不搬迁的情况下逐步纳入 V2；范围：规则文本、Catalog、CLI、微业务隔离门禁、收敛清单、测试和研发文档；非范围：业务仓库迁移、数据库和外部服务、真实网络 RPC、前端工具目录和 Git 历史；变化：后端 SDK 改入 `utils/<package>/`，跨业务调用改为目标域 `rpc/` 的 JSON 字符串入口，旧目录只允许登记时已有内容维护，遗留根也不得以父子嵌套方式重复覆盖快照，收敛清单接受普通 YAML；完成标准：适用验收项均通过且 strict/adoption 不写入样本；术语说明：strict 是拒绝违规目录的只读检查模式，adoption 是依据人工快照渐进采用的只读检查模式；验证状态：30 项本地行为测试、审查、五个文档 profile 和两个目标 Skill 快速校验全部通过。图片资产决策：N/A。原因：本轮没有界面或视觉验收对象。证据：范围仅包含本地规则与命令行行为。

## 文档信息

| 项目 | 内容 |
|---|---|
| 验收对象 | `REQ-PSR-V2-001`、`REQ-PSR-UTILS-001`、`REQ-PSR-BUSINESS-RPC-004`、`REQ-PSR-ADOPT-001` 的目录规则、CLI 与本地测试资产。 |
...
关联 ID：`AC-PSR-UTILS-001` 至 `AC-PSR-UTILS-005`、`AC-PSR-RPC-001` 至 `AC-PSR-RPC-003`、`AC-PSR-ADOPT-001` 至 `AC-PSR-ADOPT-003`。
...
- 验收目标：确认三类项目目录规则与旧项目渐进采用对同一输入给出唯一、可解释且只读的结果。
- 通过标准：30 项本地行为测试全部成功；适用文档 profile 和目标 Skill 快速校验通过；没有 P0/P1 审查问题。
...
| 浏览器和第三方验证 | 本轮没有页面、网络调用或外部服务。 | 不适用；原因：验收范围没有浏览器或外部服务接口；证据：30 项测试仅调用本地 CLI 与临时 fixture。 |
...
| 测试 | `doc/5-tests/2026-07-28_014412_代码位置目录规则V2/README.md` 与 30 项 unittest | 已通过 test profile 与本地行为测试 |
...
| 检查项 | 结果 | 依据 |
|---|---|---|
| 实施周期顺序 | 通过 | `CYCLE-05` 至 `CYCLE-09` 已按顺序收口 |
| 最小任务闭环 | 通过 | `TASK-05-01` 至 `TASK-09-03` 均有实现、测试、审查和验收记录 |
| 功能与回归验证 | 通过 | 30 项本地 unittest 覆盖 query、render、init、strict、legacy、普通 YAML adoption、无写入、JSON 响应和跨域导入 |
...
- 若修改根 `utils/`、源码根 `util/`、业务域 `rpc/`、收敛清单契约、CLI 参数或跨业务导入白名单，必须重新执行本轮 30 项测试、审查与最终验收。

## 完成条件、停止条件与交付物

| 类型 | 内容 |
|---|---|
| 完成条件 | 所有适用验收场景通过，30 项本地测试、文档 profile、Skill 快速校验和审查证据一致，且 `check` 全程只读。 |
...
- 来源与验收：`REQ-PSR-V2-001`、`REQ-PSR-UTILS-001`、`REQ-PSR-SOURCE-UTIL-002`、`REQ-PSR-CLI-003`、`REQ-PSR-BUSINESS-RPC-004`、`REQ-PSR-ADOPT-001`、`AC-PSR-UTILS-001` 至 `AC-PSR-UTILS-005`、`AC-PSR-RPC-001` 至 `AC-PSR-RPC-003`、`AC-PSR-ADOPT-001` 至 `AC-PSR-ADOPT-003`。
- 实施与证据：`CYCLE-05` 至 `CYCLE-09`、`TASK-05-01` 至 `TASK-09-03`、`TEST-PSR-UTILS-001` 至 `TEST-PSR-UTILS-004`、`TEST-PSR-RPC-001` 至 `TEST-PSR-RPC-003`、`TEST-PSR-ADOPT-001`。

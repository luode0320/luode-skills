...
review_acceptance_gates:
  - stage: acceptance
    ...
    reason: 本文档需要确认前一轮文档规则交付是否满足验收标准。
    basis: 本文档是已修改的最终验收记录。
    required_by_source: true
    required_now: true
    completed_validation: [EVD-DOC-C05-ACCEPT]
    substitute_validation: []
    manual_follow_up: N/A
    pass_standard: 文档、校验器、测试和交付证据均可回指且没有未决阻断。
  - stage: browser_integration
    applicability: not_applicable
    reason: 前一轮交付不涉及页面或浏览器交互。
    basis: 验收对象是文档规则、Skill 规则和本地测试资产。
  ...
  - stage: third_party
    applicability: not_applicable
    reason: 前一轮交付不调用第三方接口或外部授权环境。
    basis: 验证使用本地文件、Python 和仓库内测试资产。
    required_by_source: false
    required_now: false
    completed_validation: []
    substitute_validation: []
...
本次验收确认前一轮文档规则交付已经完成；普通读者可以先看懂结论和影响，执行人员可以在后文找到测试、证据和未覆盖范围。

## 1. 验收结论

**最终结论：通过（C05-CLOSE）。**

图片资产决策：N/A + 原因 + 证据：本最终验收文档不新增视觉资产，只核对已有规则、测试和交付证据。
...
| `imagegen` 回退 `gpt-image-1.5` | N/A：原因是本轮无需降级，冻结模型为 `gpt-image-2` |

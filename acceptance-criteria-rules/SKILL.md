...
- 参考 `artifact-storage-rules` 的目录与命名约定，将验收标准落到统一的前置验收文档中，但与需求文档分开保存。
...
4. 找到当前需求对应的验收标准文档；如果还没有，就按 `artifact-storage-rules` 的命名模板初始化单独的验收标准文档。
...
4. 输出结构化验收标准、边界口径和验证关注点，并更新到当前需求对应的验收标准文档。
5. 如果该需求还没有验收标准文档，则按 `artifact-storage-rules` 约定的命名规则创建；如同一天存在多个近似标题，在不改变中文语义前提下补充更具体描述以避免重名。
...
- 不把验收标准直接混写进需求文档；验收标准必须单独维护在 `artifact-storage-rules` 约定的验收标准文档中。
...
- 通过：能够形成可验证、可测试、可复核的验收口径，至少覆盖主流程、异常流程、边界条件、范围外事项和通过标准，并回写到当前需求对应的验收标准文档。
- 驳回：仍停留在“体验更好”“支持一下”“处理正确”这类抽象描述，无法指导实现与验证。

## 执行结果归档要求

- 将验收标准记录到 `artifact-storage-rules` 约定的验收标准文档中，不与需求文档混写。
...
- 同一个需求的验收标准应持续更新同一份验收标准文档，而不是重复新建多份平行验收记录。

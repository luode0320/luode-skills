...
description: 当发现测试脚本、测试数据、mock、fixture、测试说明、日志或临时验证资产散落在 `doc/tests/` 根目录之外，或混放进业务目录、仓库根目录、文档目录和非测试目录时触发。负责识别、阻断并迁移散落测试资产，统一收拢到中央测试根目录下的正确任务目录；不要用它代替测试任务根目录创建、Go 编译路径禁放处理、测试命名规则或测试程序实现规则。
---
...
- 发现测试脚本、测试数据、mock、fixture、验证记录或测试说明不在 `doc/tests/` 根目录下。
...
- 任何测试相关资产，只要不在 `doc/tests/` 根目录下，默认按违规散落处理。
...
- 通过：当前任务直接相关的散落测试资产已从违规目录迁回 `doc/tests/` 根目录下的正确任务目录，生产目录中不再混放测试资产，且没有遗漏中文说明或镜像路径。

...
description: 当创建或修改测试时间戳根目录、中文说明目录、测试文件、测试脚本、测试数据目录、fixture 目录、mock 目录时触发。负责统一测试目录与文件命名规范，保证名称可读、可检索、与业务目标一致；必须以 `artifact-storage-rules` 与 `test-location-rules` 为基础，遵循中央约定的时间戳根目录、中文说明目录和真实代码路径镜像目录命名规则；尤其要避免中文进入会被 Go 工具链编译的路径。不要用它代替 test-location-rules、test-program-rules、test-doc-rules 或功能验证规则。
---
...
3. 默认先读 `references/naming-baseline.md`，确认命名维度、顺序和禁用写法。
4. 如果命名争议涉及落点、职责或需求含义不清，再读 `references/naming-boundaries.md`。
5. 如果需要判断当前名称是否合格，再读 `references/naming-examples.md` 对照正反例。
6. 输出推荐名称、命名问题清单、是否需要统一重命名，以及重命名时的最小改动建议。
7. 确认每个时间戳根目录都包含中文说明目录和其中的 `README.md`。
8. 名称未稳定前，不进入 `test-program-rules` 和 `test-doc-rules`。
...
- 通过：当前测试目录和文件名称能够稳定表达业务对象、测试层级和场景目标，且与项目现有命名模式一致，没有临时命名和歧义命名问题；所有测试资产都在中央约定的测试根目录下；时间戳根目录命名符合统一模板；中文说明目录清晰表达测试任务简介；Go 可编译路径保持 ASCII；每个时间戳根目录都包含说明 `README.md`。
- 驳回：当前名称含糊、重复、临时化或风格混乱；或测试资产不在中央约定的测试根目录下；或时间戳根目录命名不符合统一模板；或中文进入 Go 可编译路径，继续进入后续测试编写和验证会显著增加维护成本并直接破坏自动化测试链路。

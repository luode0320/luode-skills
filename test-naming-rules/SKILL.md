...
- 驳回：当前名称含糊、重复、临时化或风格混乱；或测试资产不在中央约定的测试根目录下；或时间戳根目录命名不符合统一模板或使用手写旧日期；或新增测试文件不在当前任务时间戳目录镜像路径；或复用旧目录但缺少“复用旧目录原因 + 旧任务ID”；或中文进入 Go 可编译路径，继续进入后续测试编写和验证会显著增加维护成本并直接破坏自动化测试链路。

...
只要任务语义属于生图、改图、参考图出新图、设计预览图、原始素材图、动作关键帧或 sprite 方向图，就必须自动联动 `imagegen` 产出原始图，不得等待用户额外明确说“使用 imagegen”。
...
- 对所有生图相关正式交付，默认也不得把程序绘制、Pillow、SVG、脚本拼接、自动布局结果、后处理合成图当作原始生图结果；这些方式只能作为草图、验证图或后处理辅助图。

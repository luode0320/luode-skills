...
- 约束 Go 结构体定义归位：请求/响应/第三方结果等数据结构优先放 `internal/entity/<domain>/`，避免散落在 `internal/service` 实现文件。
...
- 打算在 Go `internal/service` 的实现文件中直接新增请求/响应/第三方结果结构体。
...
7. 对 Go 结构体先判定类型：请求/响应/第三方结果等数据结构默认落 `internal/entity/<domain>/`；`internal/service` 文件只保留服务行为实现与必要的服务接收器结构体。
8. 若目标文件已达 500 行及以上，先按功能域、流程阶段和上下游边界制定拆分方案（拆文件优先，必要时拆子目录/子包）。
9. 最后确认该层允许依赖谁，不允许依赖谁。
...
- 不替代 `implementation-review-rules` 做编码完成后的落点复查；这里是编码阶段的前置结构判断。
...
- 通过：能明确给出代码落点、目录职责、允许依赖方向和不允许跨层访问的边界；Go 结构体归位清晰（`internal/entity` 与 `internal/service` 职责不混放）；对 500 行及以上且持续增长的文件已给出可执行拆分落位方案。
- 驳回：目录命名含糊、职责交叉、依赖方向混乱，把本应归属现有层的代码随意塞进新目录，在 `utils`、`common`、`global`、`middleware` 根目录直接放实现文件，或在 Go `internal/service` 根目录直接堆业务实现文件；或 Go 请求/响应/第三方结果结构体散落在 `internal/service` 实现文件；或 500 行及以上文件持续堆功能却没有拆分落位方案。

## 执行结果归档要求

- 如果本次新增了目录、调整了分层或决定了一套新的依赖方向，将结论记录到 `analysis/` 或 `doc/6-审查/`。

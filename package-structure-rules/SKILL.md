...
2. 后端根级唯一位置：`config/`、`database/`、`utils/`、`common/`、`global/`、`crontask/`、`async/`、`middleware/`；不建立根 `data/`。后端 `config/` 根直接存放 `load.<ext>`（配置加载与解析）与 `model.<ext>`（配置结构定义），`config/embedded/` 与 `config/yaml/` 只存放配置数据，且两者互斥——只能二选一，优先使用 `config/embedded/`。`test/` 与 `mock/` 目录不承载配置，测试和 Mock 配置统一使用 `config/` 下的 `test` 环境。`load.<ext>` 环境识别支持 `-env`、`APP_ENV`、`ENV`，优先级 `-env > APP_ENV > ENV > local`。

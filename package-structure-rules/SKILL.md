...
`check` 始终只读；后端 `strict` 必须同时传入 `--project-kind backend` 与 `--language`，以准确识别源码根 `util/`。strict 必须拒绝三类项目根缺失 `Dockerfile`，当 `AGENTS.md` 与 `CLAUDE.md` 同时存在时还必须拒绝正文不一致；旧项目的 `adoption` 不因缺失 Dockerfile 或治理文件而自动迁移。`init` 创建必需目录和必需根文件，并仅在 `--enable backend.root.project-style` 时创建条件 `PROJECT_STYLE.md`；它不移动、不删除、不重命名用户文件，也不代替对应 Owner 填充文件正文。

...
- `internal/`
  放项目私有业务代码，是大多数业务目录的主落点。这里可以包含项目自适应的实现细节，也可以包含只对当前项目成立的 `utils` 子包。
- `utils/`
...
- `global/`
  放已经初始化完成、可能被多个位置直接引用的全局实例或全局引用入口。例如 `global/config`、`global/database`。这里承载的是“全局已装配对象”，不是普通配置读取逻辑或业务服务。
- `middleware/`
  放鉴权、日志、追踪、限流等横切入口逻辑。`middleware` 作为明确的入口层目录单独存在，不放进 `internal/`。
- `crontask/`
  放定时任务入口、任务注册、调度装配和定时触发逻辑。`crontask` 属于最外层次的入口包，和 `common`、`global`、`middleware` 同级，不放进 `internal/`。
...
- `global`
  放已经装配完成的共享实例引用，例如配置实例、数据库实例、日志实例、缓存实例等。
- `crontask`
  负责定时任务命名、任务注册、调度入口和任务装配。这里处理的是定时触发入口，不承载核心业务实现，真实业务逻辑仍应下沉到 `service` 或其他明确业务目录。
...
- `crontask` 可以依赖 `service` 或稳定公共层完成任务编排，但不要把业务规则直接堆进定时任务入口层。

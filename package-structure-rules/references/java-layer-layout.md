...
项目根 `utils/` 只允许工具包子目录，所有实现必须位于 `utils/<package>/` 或更深的真实技术子目录，且不得依赖项目其他包。业务域内采用 `api/`、`service/`、`entity/`、`base/`、`constant/`、`init/`、`crontask/`、`util/`；业务域 `util/` 保留为域私有辅助能力。ORM 模型只放根 `database/model/`。

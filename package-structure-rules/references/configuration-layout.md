...
Go 的 embedded 文件名使用 `config_<env>_yaml.go`，格式名 `yaml` 必须后置。这样命名是因为 `config_<env>.go` 会撞上语言保留命名：Go 把 `config_test.go` 当成测试文件，`go build` 会把它排除、`go test` 会把它编译进测试包，而 `test` 恰好是标准环境名。把格式名放到环境名之后，环境文件就不再落进任何语言的保留命名空间。环境名本身不得以 `_yaml` 结尾，避免 `config_test_yaml_yaml.go` 这类无法唯一切分的文件名。示例：
...
外部 YAML 文件不加 `_yaml` 后缀：它不参与任何语言的编译，不存在保留命名冲突，`config_test_yaml.yaml` 只会造成格式名重复。

其他语言的 embedded 文件本周期只保留该语言既有源码扩展名检查，不套用 Go 的 `config_<env>_yaml.go` 文件名契约，也不新增环境名校验。例如 Java、Node.js 或 Python 继续使用项目已有的源码扩展名规则；Go 文件才必须使用 `config_<env>_yaml.go`。
...
- 独立 Go 后端：`config/load.go`（配置加载与解析入口）、`config/model.go`（配置结构定义）。
- 独立 Go 后端：`config/yaml/config_local.yaml`、`config/yaml/config_pre_prod.yml`、`config/embedded/config_prod_yaml.go`。
- 同仓 Go 后端：`backend/config/yaml/config_test.yaml`、`backend/config/embedded/config_local_yaml.go`。
- 同仓 Go 后端：`backend/config/load.go`、`backend/config/model.go`。
- 只有部分环境存在：仅有 `config/yaml/config_test.yaml`，不要求补建 `config_local.yaml` 或 `config_prod.yaml`。
- 环境未配对：存在 `config/yaml/config_prod.yaml` 但没有 `config/embedded/config_prod_yaml.go`，仍不因配对缺失而失败。

...
- 数据结构目录（如 `entity`、`dto`、`model`）与行为实现目录（如 `service`）应职责分离，避免在 `service` 实现文件散落结构体定义。
- 单文件达到 500 行及以上且仍在扩展时，应优先按职责拆分到多个文件；当业务流程明显增多时，再按业务域拆子目录/子包。
- 分层应尽量单向依赖，避免反向依赖和循环依赖。

## 公共与入口目录边界

- `utils`
  只放稳定、可复用、无明显业务状态的小工具，不是兜底目录。根目录默认作为命名空间，具体实现放在 `utils/<name>/`。
- `common`
  只在“项目相关、可跨多个模块复用、边界稳定”时使用，不能作为模糊的大杂烩目录。根目录默认作为命名空间，具体实现放在 `common/<name>/`。
- `global`
  只放已经初始化完成、可被多个位置直接引用的共享实例或共享引用入口。根目录默认作为命名空间，具体实现放在 `global/<name>/`。
- `middleware`
  只放鉴权、日志、追踪、限流等横切入口逻辑，不承载完整业务流程。根目录默认作为命名空间，具体实现放在 `middleware/<name>/`。
- `crontask`

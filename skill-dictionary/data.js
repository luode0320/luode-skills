window.SKILL_DICTIONARY = {
  "generated_at": "2026-06-27 23:45:00",
  "repo_root": "F:\\luode-skills",
  "plan_doc": "编码skill.md",
  "plan_doc_name": "编码skill.md",
  "summary": {
    "planned_total": 77,
    "implemented_total": 77,
    "planned_missing": 0,
    "seed_total": 23,
    "doc_total": 7,
    "references_total": 288,
    "agents_total": 87
  },
  "downloaded_seeds": {
  ...
  "domains": [
    {
      ...
      "items": [
        ...
        },
        {
          ...
          "domain_order": 1,
          "item_order": 2,
          "auto_trigger": "当需要定义、调整或解释项目中 `doc/需求/`、`doc/架构/`、`doc/实施/`、`doc/验收/`、`doc/bugs/`、`doc/tests/`、`doc/审查/`、`doc/`、`skill/` 以及根目录 `项目设计.md` 等研发产物根目录、主入口文件、命名模板、同任务复用策略或跨域文档引用关系时自动触发。负责提供全局唯一的目录与命名单一真相源，并为需求、实施、验收、Bug、测试、审查、记忆、项目设计和交付类 skill 提供统一引用基准；不要用它代替需求分析、Bug 定位、测试执行、生产代码存放决策或流程分流。",
    ...
    },
    {
      ...
      "items": [
        ...
        },
        {
          ...
          "domain_order": 3,
          "item_order": 6,
          "auto_trigger": "当需求条件闸门已收敛且前置验收标准已稳定，正式编码前仍需要先把当前优先闭环的文件落点、模块职责、实施周期、阶段步骤、验证步骤和阻断项拆清时触发。负责把已确认需求或已拆分出的当前优先子项转成可执行实施方案，并将结果单独保存到 `artifact-storage-rules` 约定的实施总览/实施周期文档中；不要用它代替需求拆分、验收标准编写、实际编码、测试执行或最终验收。",
          "core_responsibility": "把已确认需求转成可执行实施总览与实施周期。",
        ...
        },
        {
          ...
          "domain_order": 3,
          "item_order": 9,
          "auto_trigger": "当测试与审核均已完成、任务准备最终放行时触发。负责基于需求主文档、验收标准、实施总览/实施周期、测试结果和审核结果做后置最终验收，并将结论单独保存到 `artifact-storage-rules` 约定的最终验收文档中；不要用它代替前置验收标准、功能验证、回归验证或实现审查。",
          "core_responsibility": "基于验收标准逐条做最终验收。",
    ...
    },
    {
      ...
      "items": [
        ...
        },
        {
          ...
          "domain_order": 8,
          "item_order": 2,
          "auto_trigger": "当为当前需求、当前 Bug 或当前验证任务新增测试任务目录，或需要决定 `doc/tests/` 根目录下的当天时间戳根目录、中文说明目录和 ASCII 真实代码路径镜像布局时触发。负责统一测试任务根目录创建、当天时间戳校验、中文 README 说明目录和真实测试资产镜像布局；不要用它代替散落测试资产迁移、Go 编译路径冲突处理、测试命名规则或测试程序实现规则。",
        ...
        },
        {
          ...
          "domain_order": 8,
          "item_order": 3,
          "auto_trigger": "当发现测试脚本、测试数据、mock、fixture、测试说明、日志或临时验证资产散落在 `doc/tests/` 根目录之外，或混放进业务目录、仓库根目录、文档目录和非测试目录时触发。负责识别、阻断并迁移散落测试资产，统一收拢到中央测试根目录下的正确任务目录；不要用它代替测试任务根目录创建、Go 编译路径禁放处理、测试命名规则或测试程序实现规则。",
      ...
      "items": [
        {
          ...
          "domain_order": 10,
          "item_order": 1,
          "auto_trigger": "当用户主动提出“审核代码”“review 当前分支提交”“审查最近提交”时触发。负责读取项目 `main` 分支最近一条提交时间，并仅审查当前分支在该时间之后且尚未并入 `main` 的提交，逐条输出中文结构化结果（致命/严重/中等/建议），再将汇总报告保存到 `artifact-storage-rules` 约定的 `doc/审查/` 主文档位置（同主题覆盖或按中央模板更新）；禁止跨提交混审，禁止把非当前 commit 引入的问题混入结论；不因本轮已有代码改动或准备最终收口而自动触发，这类场景由 `project-change-review-rules` 承接。",
    ...
    },
    {
      ...
      "items": [
        ...
        },
        {
          ...
          "domain_order": 11,
          "item_order": 7,
          "auto_trigger": "当需求、Bug、测试或审查任务准备最终收口，且本轮已经产生或应当产生持久化研发文档时自动触发。负责在最终完成前核对文档是否已经真实落盘到 `artifact-storage-rules` 约定的位置，检查主入口文件、必需配套文件和同任务复用关系是否完整；若文档仍停留在最终回复、临时说明或内存结论中，必须阻断收口并先补齐落盘。适用于需求文档、Bug 根目录、测试任务 README 与审查报告，不代替需求分析、Bug 定位、测试执行或审查本身。",
          "core_responsibility": "当前已在仓库中，但尚未并入主规划域表。",
        ...
        {
          "id": "claude-desktop-zh-cn-latest",
          "name": "claude-desktop-zh-cn-latest",
          "title": "Claude Desktop Zh Cn Latest",
          "status": "seed",
          "status_label": "扩展种子",
          "domain_id": "seed",
          "domain_label": "扩展种子",
          ...
          "domain_order": 11,
          "item_order": 10,
          "auto_trigger": "Install, update, or uninstall the community Chinese UI patch for Claude Desktop on Windows, always using the latest GitHub release. Use when the user wants Claude Desktop in Chinese, needs a repeatable installer script, or wants a reversible community patch workflow.",
          "core_responsibility": "当前已在仓库中，但尚未并入主规划域表。",
          "skill_path": "claude-desktop-zh-cn-latest/SKILL.md",
          "directory_path": "claude-desktop-zh-cn-latest",
          "directory": "claude-desktop-zh-cn-latest",
          "sections": [
            "Overview",
            "Workflow",
            "Installer",
          ...
          "references": [],
          "agents": [
            "claude-desktop-zh-cn-latest/agents/openai.yaml"
          ],
          "has_license": false,
          "focus_points": [
            "先决定它是并入主规划、保持外部种子，还是拆成多个更窄的内部 skill。",
  ...
  "recommendations": [
    "77 个规划 skill 已全部独立落地，后续优化优先检查 description 命中率、相邻 skill 边界和 references 的信息密度。",

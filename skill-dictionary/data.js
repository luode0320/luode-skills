  ...
  "domains": [
    {
      ...
      "items": [
        ...
        },
        {
          ...
          "domain_order": 1,
          "item_order": 10,
          "auto_trigger": "【强制自动触发】当当前会话收到明确提问、需求、Bug、实施、审查、测试、提交、规则更新或其他可命名请求，且可稳定归纳出中文任务主题时触发。负责自动生成 8-24 字中文简要标题，并按平台能力矩阵调用真实线程重命名工具重命名当前会话：Codex 使用 `set_thread_title`，Claude Code 仅在存在真实改名工具时执行，Claude Desktop 默认视为无真实自动改名工具并显式跳过；不等待用户显式要求。问题过于闲聊、主题过散、标题已准确、工具不可用或用户明确禁止时不改名。不要用它代替需求分析、Bug 定位、实施规划、测试验证或提交动作。",

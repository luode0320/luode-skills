window.SKILL_DICTIONARY = {
  "generated_at": "2026-08-17 11:54:31",
    ...
    },
    {
      ...
      "items": [
        ...
        },
        {
          ...
          "domain_order": 6,
          "item_order": 2,
          "auto_trigger": "当新建项目 / 新会话首轮检测到新或空仓库（缺业务代码骨架），或用户提出「微业务 / 伪微服务 / 按业务分目录包 / 业务隔离 / 业务互不关联 / RPC 通信 / 新业务开新包 / 一个项目一个服务的伪微服务」等诉求，或项目已存在微业务标记时自动触发。负责守护单项目单服务中的业务垂直切分和跨业务导入隔离：调用方只能导入目标业务域 `rpc/` 的 JSON 字符串公开函数，禁止导入目标业务私有层；提供幂等脚手架、标记与确定性校验，并以 CodeGraph 导入节点作为审查证据。具体目录、JSON 响应语义和引用边界由 `package-structure-rules` 唯一拥有；不要用它代替需求分析、实施规划、分层落点或实际编码。",
    ...
    },
    {
      ...
      "items": [
        ...
        },
        {
          ...
          "domain_order": 10,
          "item_order": 36,
          "auto_trigger": "WSL2 内需要调用 Windows Python（Anaconda）、执行 PowerShell/CMD 命令、跨系统复制文件或转换路径时触发；提供 win-python / win-ps / win-cmd / win-copy / win-run-py / win-path 六个工具命令。承接已删除的 windows-wsl-execution-rules，但不包含其命令路由与编码规则（由 AGENTS.md 内联摘要承接）。",
          "core_responsibility": "当前已在仓库中，但尚未并入主规划域表。",

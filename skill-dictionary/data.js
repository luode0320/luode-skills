window.SKILL_DICTIONARY = {
  "generated_at": "2026-08-22 00:02:04",
  ...
  "domains": [
    {
      ...
      "items": [
        ...
        },
        {
          ...
          "domain_order": 1,
          "item_order": 5,
          "auto_trigger": "项目级 skill 自动沉淀与项目专属规则管理。任务收口时，若本轮涉及项目仓库写操作（代码/文档/配置修改）且产生可复用经验——如 5+ 次 tool call 的非平凡任务、踩坑后找到解法、用户纠正了方法、发现可复用多步骤 workflow——自动评估是否值得沉淀；用户明确要求\"分析项目/总结项目专属 skill/沉淀项目规则\"时同样触发。负责查重后以 project-项目名-主题-rules 命名写入用户级 skill 目录（经 junction 与 luode-skills 仓库同址），供后续会话自动命中；不代替 knowledge-flow 知识库笔记沉淀与 skill-absorption-rules 内部演进。",

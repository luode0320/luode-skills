window.SKILL_DICTIONARY = {
  "generated_at": "2026-04-02 17:51:59",
      ...
      "items": [
        {
          ...
          "domain_order": 9,
          "item_order": 1,
          "auto_trigger": "当准备整理提交、拆提交粒度、同步分支、处理协作变更、准备 PR 或合并前收口时触发。负责提交粒度、提交说明、分支同步、协作边界和 PR 整理；提交代码时必须分析未提交的代码，如果可以分成不同的业务修改，就用多次不同的 git 提交；git 提交消息必须遵循 feat/fix 等格式，提交说明必须使用真实换行（禁止在任意 shell 把 `\\n` 当文本写入）；git 提交前必须先在项目根目录 README.md 追加本次改动日志，格式为时间 + 提交标题，且 README.md 改动日志必须保持按时间正序排列；Go 项目提交前还必须扫描 `test/` 外 `*_test.go` 并阻断违规提交，同时阻断本次提交中直接落在 `internal/service/*.go` 根目录的业务实现文件；不要把它代替代码评审或发布上线规则。",

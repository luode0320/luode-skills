window.SKILL_DICTIONARY = {
  "generated_at": "2026-06-20 17:44:34",
  ...
  "domains": [
    {
      ...
      "items": [
        ...
        },
        {
          ...
          "domain_order": 1,
          "item_order": 2,
          "auto_trigger": "当需要定义、调整或解释项目中 `ment/`、`bug/`、`test/`、`doc/`、`skill/` 以及根目录 `项目设计.md` 等研发产物根目录、主入口文件、命名模板、同任务复用策略或跨域文档引用关系时自动触发。负责提供全局唯一的目录与命名单一真相源，并为需求、Bug、测试、记忆、项目设计和交付类 skill 提供统一引用基准；不要用它代替需求分析、Bug 定位、测试执行、生产代码存放决策或流程分流。",
        ...
        },
        {
          ...
          "domain_order": 1,
          "item_order": 6,
          "auto_trigger": "当仓库命中 `project.godot`、`.gd`、`.tscn`、`addons/`、`export_presets.cfg` 等 Godot 项目标记，且需要自动补齐项目级 `AGENTS.md`、Godot AI MCP 配置、图像生成配置模板或检查 Godot 开发环境是否可直接进入执行时强制自动触发。负责把 Godot 项目的环境准备、自举补齐、图像通道模板和只差人工配置的缺口一次性收口。",
        ...
        },
        {
          ...
          "domain_order": 1,
          "item_order": 9,
          "auto_trigger": "【强制总控】每轮用户新消息（含新会话第一条）都必须先做命中检查并在首条中间进度输出。凡涉及 Git 协作动作（含显式关键词与隐式语义，如“提交git/帮我提交/commit一下/推送代码/看下状态”），必须联动命中 git-collaboration-rules。凡处理本仓库任务，最低还必须联动命中 `parallel-task-dispatch-rules`。首条中间进度最小必填仅两行：命中检查/Git规则、命中技能。",
    ...
    },
    {
      ...
      "items": [
        ...
        },
        {
          ...
          "domain_order": 3,
          "item_order": 5,
          "auto_trigger": "当需求、边界和验收标准已基本稳定，且正式编码前仍需要先把当前优先闭环的文件落点、模块职责、开发周期、阶段步骤、验证步骤和阶段收口拆清时触发。负责把已确认需求或已拆分出的当前优先子项转成可执行实施计划，并将计划单独保存到 `artifact-storage-rules` 约定的实施计划文档中；不要用它代替需求拆分、验收标准编写、实际编码或测试执行。",
    ...
    },
    {
      ...
      "items": [
        ...
        },
        {
          ...
          "domain_order": 7,
          "item_order": 4,
          "auto_trigger": "【强制自动触发】当新增文件、移动文件、扩展模块、跨层调用、工具类落点或目录归属可能不合理时，在进入测试前触发。负责检查代码存放位置、模块归属、层级边界和依赖方向是否合理；Go 项目中还需检查 `*_test.go` 是否误落在 `test/` 之外，并检查 `internal/service` 是否散落请求/响应等结构体定义；对 500 行及以上且持续膨胀的文件需追加拆分审查；默认优先并行；不要用它代替编码前的包结构决策规则。",
    ...
    },
    {
      ...
      "items": [
        ...
        },
        {
          ...
          "domain_order": 10,
          "item_order": 19,
          "auto_trigger": "当当前会话已发生“压缩上下文 / 自动压缩上下文 / 上下文太多”后的压缩重组，或继续执行前刚得到压缩摘要时自动触发。负责在压缩后立即联动 recent-context-bootstrap-rules 重新加载最近项目上下文（含系统的所有 skills 与当前项目根目录下 `./skill`、`./.skills`），并强制重新读取当前项目根目录 `AGENTS.md`，避免压缩后丢失 skill 记忆或仓库级硬规则，再输出可直接续做的最小上下文包；压缩包必须显式携带“是否允许开始/继续实现代码”的许可状态，默认 `unknown`，在未重新确认前不得直接进入编码。不要把它代替 history-recall-rules 的深度历史回忆、project-timeline-rules 的长期时间线分析或当前主域执行。",
          "core_responsibility": "当前已在仓库中，但尚未并入主规划域表。",
        ...
        },
        {
          ...
          "domain_order": 10,
          "item_order": 30,
          "auto_trigger": "新会话第一轮默认自动触发（不依赖用户意图）；也可被“创建、补齐或更新 AGENTS.md / 补充仓库级规则”等显式请求触发。负责在项目根目录强制检测 AGENTS.md：不存在则必须创建最小可用模板，存在则执行增量补充且不覆盖用户已有规则，并确保包含注释类任务流程、UTF-8 中文编码约束，以及“上下文压缩后必须重新读取项目根目录 `AGENTS.md` 再继续主任务”的硬规则；若仓库命中 Godot 项目标记，还必须额外补齐 Godot 工具接管与图像生成配置模板，并明确 AGENTS.md 里不能存真实密钥。",
          "core_responsibility": "当前已在仓库中，但尚未并入主规划域表。",

window.SKILL_DICTIONARY = {
  "generated_at": "2026-03-30 11:07:56",
    ...
    },
    {
      ...
      "items": [
        ...
        },
        {
          ...
          "domain_order": 5,
          "item_order": 6,
          "auto_trigger": "当需要判断代码是否应该加注释、注释放在哪里、写到什么颗粒度、如何与实现同步维护时触发。负责注释必要性、位置选择、颗粒度控制、步骤注释、字段注释、函数方法参数返回注释、函数方法修改时间标注和过期注释治理；每个函数方法的修改都必须补充最近修改时间（北京时间：yyyy-MM-DD HH:mm:ss），并补充 `[参数]` / `[返回]` 简单注释；Swagger/OpenAPI 接口文档注解/注释不由本 skill 兜底，应转交 api-swagger-rules；不要把它代替中文语言表达规则。",
    ...
    },
    {
      ...
      "items": [
        ...
        },
        {
          ...
          "domain_order": 6,
          "item_order": 5,
          "auto_trigger": "当新增或修改 controller、router、路由声明、HTTP 方法、接口路径命名、接口入口职责或超时入口边界时触发。负责统一接口入口设计、路径命名和强制使用 POST 方法；必须以 package-structure-rules 为基准，不使用 handler 包名；若本次改动影响 Swagger/OpenAPI 的路径、tag、摘要或文档分组，应与 api-swagger-rules 同步生效；不要用它代替请求参数、响应结构或错误处理规则。",
      ...
      "items": [
        {
          ...
          "domain_order": 7,
          "item_order": 1,
          "auto_trigger": "当功能代码已经完成、准备进入测试前验证时触发。负责检查实现是否符合可读性优先、单一职责、命名语义化、注释完整、错误处理明确、日志可追溯、依赖使用审慎、魔法值治理、冗余逻辑清理和编码规范等实现质量要求，并在功能不变前提下检查最近改动代码是否还存在可直接收口的表达层冗余；Go 场景下还需在实现自审阶段扫描 `test/` 外 `*_test.go` 禁放问题，以及本轮改动是否把业务实现直接落在 `internal/service/*.go` 根目录文件；若本轮改动涉及后端 HTTP API，还必须检查 Swagger/OpenAPI 是否同步更新；不要用它代替语法检查、格式化执行、目录归位审查或功能验证规则。",
        ...
        },
        {
          ...
          "domain_order": 7,
          "item_order": 4,
          "auto_trigger": "当新增文件、移动文件、扩展模块、跨层调用、工具类落点或目录归属可能不合理时，在进入测试前触发。负责检查代码存放位置、模块归属、层级边界和依赖方向是否合理；Go 项目中还需检查 `*_test.go` 是否误落在 `test/` 之外；不要用它代替编码前的包结构决策规则。",
    ...
    },
    {
      ...
      "items": [
        ...
        },
        {
          ...
          "domain_order": 8,
          "item_order": 4,
          "auto_trigger": "当新增或修改测试程序、模拟程序、验证脚本、数据构造脚本、测试辅助代码时触发。负责统一测试程序职责拆分、辅助代码边界和长期保留策略；必须以 `artifact-storage-rules` 与 `test-location-rules` 为基准，把真实测试代码、脚本、mock、fixture 和执行产物统一落在中央约定的测试时间戳根目录下的 ASCII 真实代码路径镜像目录中，并避免中文进入会被 Go 工具链编译的路径；Go 场景下白盒/黑盒/集成测试都遵循同一落点规则，源码目录禁止 `*_test.go`，白盒诉求通过 seam 解决；不要用它代替 test-location-rules、test-naming-rules、test-doc-rules、bug-runtime-debug-rules 或功能验证规则。",

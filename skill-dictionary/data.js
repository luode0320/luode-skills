window.SKILL_DICTIONARY = {
  "generated_at": "2026-09-01 16:13:09",
    ...
    },
    {
      ...
      "items": [
        ...
        },
        {
          ...
          "domain_order": 5,
          "item_order": 7,
          "auto_trigger": "【强制自动触发】本轮发生代码新增/修改，或用户提到“补充注释/只补注释/注释完善/补下注释/加注释”时必须命中本 skill。负责代码注释全链路统一规则：中文语言表达（必须中文，除标准协议字段/第三方固定错误原文外）、注释放置与颗粒度（该不该写/写在哪/写多细）、改动位点补齐闸门（函数头 [参数]/[返回]/最近修改时间、方法体步骤编号、补丁原因注释、函数/补丁核对清单）。前端文件与后端文件同等纳入检查。缺任一项不得收口。仅负责注释规则，不代替 Swagger/OpenAPI 注解或业务规则。",
    ...
    },
    {
      ...
      "items": [
        ...
        },
        {
          ...
          "domain_order": 6,
          "item_order": 12,
          "auto_trigger": "创建具有鲜明风格、达到生产级质量的高品质前端界面。当用户要求构建 Web 组件、页面或前端应用，或进行前端 UI、组件、样式的调整/改进/界面 Bug 修复时优先使用这个 skill。它会生成有创意、打磨精细的代码，并避免出现千篇一律的 AI 模板化审美。触发词：前端设计、页面设计、UI 设计、组件设计、界面样式、前端审美、网页设计、设计风格。",
    ...
    },
    {
      ...
      "items": [
        ...
        },
        {
          ...
          "domain_order": 10,
          "item_order": 63,
          "auto_trigger": "\"正确使用 SQLite：并发与 WAL 模式、外键 pragma、类型亲和性、表结构变更限制、性能 pragma、VACUUM 维护、备份安全、索引、事务与批量写入优化。适用于新建/接入 SQLite 库、排查 SQLITE_BUSY/锁等待、批量写入慢、外键不生效、文件膨胀、备份问题场景。触发词：sqlite、sqlite3、数据库锁、SQLITE_BUSY、wal 模式、外键不生效、pragma、VACUUM、批量插入慢、内存数据库、:memory:、db 文件损坏、sqlite 备份、busy_timeout。\"",
          "core_responsibility": "当前已在仓库中，但尚未并入主规划域表。",

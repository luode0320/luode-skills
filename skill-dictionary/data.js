window.SKILL_DICTIONARY = {
  "generated_at": "2026-08-09 23:24:10",
    ...
    },
    {
      ...
      "items": [
        ...
        },
        {
          ...
          "domain_order": 2,
          "item_order": 5,
          "auto_trigger": "从对话、代码与项目文档中抽取并维护项目本地四件套：`PROJECT_CURRENT.md` 保存项目概览与多会话可交接的当前状态并覆盖更新，`PROJECT_MEMORY.md` 保存稳定规则、关键决策和长期事实，`PROJECT_HISTORY.md` 只追加关键历史事件。`PROJECT_MEMORY.md` 继续保留兼容的“人类阅读区 + 底部机器索引区”结构，但不再作为当前状态或历史流水的唯一承载文件；不得新增 `PROJECT_MEMORY_INDEX.yaml` 等平行记忆根文件。",
    ...
    },
    {
      ...
      "items": [
        ...
        },
        {
          ...
          "domain_order": 8,
          "item_order": 4,
          "auto_trigger": "当浏览器任务明确需要 Browser Use Cloud 的云端自主长链、托管并发、地域出口、托管代理、隐身浏览或服务商提供的合规验证码处理能力，或用户明确点名 Browser Use Cloud 时触发。作为 Browser Use Cloud 执行、安全、费用确认和 session 生命周期的唯一 Owner，负责检查 `BROWSER_USE_API_KEY`、Billing 余额、运行时 MCP 工具 schema 的硬费用上限、逐次 `run_session` / `send_task` 确认、默认无 profile/录制/保活，以及任务结束后的停止与费用回读。普通网页检索、真实 Chrome 登录态、应用内 Browser、DevTools 调试、本地 agent-browser、HAR/视觉 diff/trace 不使用本 Skill；不得用 Cloud 绕过权限、安全策略或真实浏览器连接失败。",

window.SKILL_DICTIONARY = {
  "generated_at": "2026-06-12 17:03:20",
  ...
  "domains": [
    {
      ...
      "items": [
        ...
        },
        {
          ...
          "domain_order": 1,
          "item_order": 5,
          "auto_trigger": "当用户要求分析项目、检查当前项目是否需要安装 MCP、判断浏览器或 Godot 编辑器应优先由哪个工具接管，或任务即将涉及前端页面验证、浏览器联动、Godot 编辑器操控且需要先根据项目结构决定是否安装 Chrome DevTools MCP 或 Godot AI MCP 时自动触发。负责识别前端项目与 Godot 项目标记，给出 MCP 安装结论、优先级和后续工具让路规则；若已具备对应 MCP，应优先使用它们控制浏览器或 Godot 编辑器，再在缺失或不可用时回退到其他本地工具。",
          "core_responsibility": "负责识别前端项目与 Godot 项目标记，给出 MCP 安装结论、优先级和后续工具让路规则。",
        ...
        },
        {
          ...
          "domain_order": 1,
          "item_order": 6,
          "auto_trigger": "当需要分析代码库结构、符号关系、调用链、被调用方、影响面、重构范围或跨文件根因时自动触发。任何仓库都默认先尝试由 CodeGraph 支撑；若 `.codegraph/` 未初始化，则先自动初始化再用；若初始化失败或当前环境不支持，则直接回退到 `rg`、`find`、`read` 等本地手段。不要把它代替项目设计、需求澄清、编码实现或测试验证主流程。",
        ...
        },
        {
          ...
          "domain_order": 1,
          "item_order": 8,
          "auto_trigger": "【强制总控】每轮用户新消息（含新会话第一条）都必须先做命中检查并在首条中间进度输出。凡涉及 Git 协作动作（含显式关键词与隐式语义，如“提交git/帮我提交/commit一下/推送代码/看下状态”），必须联动命中 git-collaboration-rules。首条中间进度最小必填仅两行：命中检查/Git规则、命中技能。",
      ...
      "items": [
        {
          ...
          "domain_order": 9,
          "item_order": 1,
          "auto_trigger": "【强制触发】凡出现 Git 协作动作即触发（显式关键词 + 隐式语义），包括“提交git/帮我提交/commit一下/推送代码/看下状态/看下改动”等；新会话第一条也必须触发。触发后必须与 skill-hit-check-rules 联动命中。默认执行“最小可执行闭环”：短清单 + 阻断脚本 + 统一证据模板。用户明确请求“提交git”时，目标是清空当前全部未提交改动（staged、unstaged、untracked），允许按业务拆分多次提交，不要求一次提交完成，但必须循环到 `git status --short` 为空（除阻断项外）。",
    ...
    },
    {
      ...
      "items": [
        ...
        },
        {
          ...
          "domain_order": 10,
          "item_order": 10,
          "auto_trigger": "新会话第一轮默认自动触发（不依赖用户意图）；也可被“创建 AGENTS.md/补充仓库级规则”等显式请求触发。负责在项目根目录强制检测 AGENTS.md：不存在则必须创建最小可用模板，存在则执行增量补充且不覆盖用户已有规则，并确保包含注释类任务流程（先声明命中的注释类 skill、读取对应 SKILL.md 后再改代码、最终回复给执行证据）与 UTF-8 中文编码约束；任何仓库默认都应先尝试由 CodeGraph 支撑，若 `.codegraph/` 尚未初始化，则先自动尝试初始化，不需要用户确认，失败后回退到不使用 CodeGraph。",
          "core_responsibility": "当前已在仓库中，但尚未并入主规划域表。",
  ...
  "recommendations": [
    "70 个规划 skill 已全部独立落地，后续优化优先检查 description 命中率、相邻 skill 边界和 references 的信息密度。",

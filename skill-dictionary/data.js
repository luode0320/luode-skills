window.SKILL_DICTIONARY = {
  "generated_at": "2026-03-30 10:09:38",
  "repo_root": "D:\\code\\luode-skills",
  "plan_doc": "编码skill.md",
  "plan_doc_name": "编码skill.md",
  "summary": {
    "planned_total": 56,
    "implemented_total": 56,
    "planned_missing": 0,
    "seed_total": 2,
    "doc_total": 2,
    "references_total": 192,
    "agents_total": 55
  },
  "downloaded_seeds": {
    ...
    },
    {
      ...
      "items": [
        ...
        },
        {
          ...
          "domain_order": 5,
          "item_order": 6,
          "auto_trigger": "当需要判断代码是否应该加注释、注释放在哪里、写到什么颗粒度、如何与实现同步维护时触发。负责注释必要性、位置选择、颗粒度控制、步骤注释、字段注释、函数方法参数返回注释、函数方法修改时间标注和过期注释治理；每个函数方法的修改都必须补充最近修改时间（北京时间：yyyy-MM-DD HH:mm:ss），并补充 `[参数]` / `[返回]` 简单注释；不要把它代替中文语言表达规则。",
      ...
      "items": [
        {
          ...
          "domain_order": 7,
          "item_order": 1,
          "auto_trigger": "当功能代码已经完成、准备进入测试前验证时触发。负责检查实现是否符合可读性优先、单一职责、命名语义化、注释完整、错误处理明确、日志可追溯、依赖使用审慎、魔法值治理、冗余逻辑清理和编码规范等实现质量要求，并在功能不变前提下检查最近改动代码是否还存在可直接收口的表达层冗余；Go 场景下还需在实现自审阶段扫描 `test/` 外 `*_test.go` 禁放问题，以及本轮改动是否把业务实现直接落在 `internal/service/*.go` 根目录文件；不要用它代替语法检查、格式化执行、目录归位审查或功能验证规则。",
  ...
  "recommendations": [
    "56 个规划 skill 已全部独立落地，后续优化优先检查 description 命中率、相邻 skill 边界和 references 的信息密度。",

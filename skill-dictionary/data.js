window.SKILL_DICTIONARY = {
  "generated_at": "2026-03-31 23:10:58",
    ...
    },
    {
      ...
      "items": [
        ...
        },
        {
          ...
          "domain_order": 5,
          "item_order": 2,
          "auto_trigger": "当新增或修改业务代码、工具代码、服务代码、脚本代码或 Bug 修复逻辑时触发。负责约束函数结构、逻辑顺序、复杂度和可维护性，并在功能不变前提下优先让最近改动代码表达更清楚、更直接；当单文件达到 500 行及以上且仍持续新增功能时，必须触发拆分评估；结构调整落地后必须联动 `code-comment-rules` 完成改动位点注释补齐；不要用它代替最小改动、风格一致性、注释规则或代码归位规则。",
    ...
    },
    {
      ...
      "items": [
        ...
        },
        {
          ...
          "domain_order": 7,
          "item_order": 4,
          "auto_trigger": "当新增文件、移动文件、扩展模块、跨层调用、工具类落点或目录归属可能不合理时，在进入测试前触发。负责检查代码存放位置、模块归属、层级边界和依赖方向是否合理；Go 项目中还需检查 `*_test.go` 是否误落在 `test/` 之外；对 500 行及以上且持续膨胀的文件需追加拆分审查；不要用它代替编码前的包结构决策规则。",

window.SKILL_DICTIONARY = {
  "generated_at": "2026-08-02 17:59:29",
    ...
    },
    {
      ...
      "items": [
        ...
        },
        {
          ...
          "domain_order": 8,
          "item_order": 6,
          "auto_trigger": "当 Bug 修复、原有功能迭代、公共模块修改、共享逻辑调整、接口兼容性变化后准备执行测试时触发。负责判定回归范围、选择回归用例、验证兼容性影响并输出回归结论；必须以 `artifact-storage-rules` 与 `test-strategy-rules 的 test-asset-governance 条件路由` 为基准，把回归结论写回测试任务 README，把可执行回归测试、mock、fixture 和 helper 放入根 `test/` 镜像目录，并把详细回归案例、执行证据和补充说明放到同时间戳 `doc/5-tests/` 的 `evidence/` 或 `artifacts/`；Go 测试还必须遵循 `test-program-rules` 的《Go 测试编译路径（强制）》；同时强制禁止为了测试目的污染生产代码。不要用它代替 functional-validation-rules、test-strategy-rules 或测试资源管理类规则。",
        ...
        },
        {
          ...
          "domain_order": 8,
          "item_order": 8,
          "auto_trigger": "当需要做上线前项目级全接口测试、消费者视角的 HTTP/SSE/原生 WebSocket/Socket.IO 外部整体性测试、替代人工接口回归验证或生成发布门禁结论时触发。负责在接口基线就绪后执行接口与 verified 消费者场景、确定性断言、受控 local 探针、清理、双轨对账和硬切门禁；是统一执行内核 `scripts/release_test_engine/` 的唯一行为 owner，所有测试资产强制落地到 `doc/5-tests/` 对应时间戳根目录，禁止用候选场景、fixture 结果或非 local 配置伪造通过。",

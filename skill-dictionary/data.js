window.SKILL_DICTIONARY = {
  "generated_at": "2026-03-31 22:45:18",
    ...
    },
    {
      ...
      "items": [
        ...
        },
        {
          ...
          "domain_order": 5,
          "item_order": 2,
          "auto_trigger": "当新增或修改业务代码、工具代码、服务代码、脚本代码或 Bug 修复逻辑时触发。负责约束函数结构、逻辑顺序、复杂度和可维护性，并在功能不变前提下优先让最近改动代码表达更清楚、更直接；结构调整落地后必须联动 `code-comment-rules` 完成改动位点注释补齐；不要用它代替最小改动、风格一致性、注释规则或代码归位规则。",
      ...
      "items": [
        {
          ...
          "domain_order": 7,
          "item_order": 1,
          "auto_trigger": "当功能代码已经完成、准备进入测试前验证时触发。负责检查实现是否符合可读性优先、单一职责、命名语义化、注释完整、错误处理明确、日志可追溯、依赖使用审慎、魔法值治理、冗余逻辑清理和编码规范等实现质量要求，并在功能不变前提下检查最近改动代码是否还存在可直接收口的表达层冗余；必须核验本轮改动是否完成 `code-comment-rules` 的改动位点注释补齐；Go 场景下还需在实现自审阶段扫描 `test/` 外 `*_test.go` 禁放问题，以及本轮改动是否把业务实现直接落在 `internal/service/*.go` 根目录文件；若本轮改动涉及后端 HTTP API，还必须检查 Swagger/OpenAPI 是否同步更新；不要用它代替语法检查、格式化执行、目录归位审查或功能验证规则。",

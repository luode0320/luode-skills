window.SKILL_DICTIONARY = {
  "generated_at": "2026-07-03 01:48:22",
  "repo_root": "F:\\luode-skills",
  "plan_doc": "编码skill.md",
  "plan_doc_name": "编码skill.md",
  "summary": {
    "planned_total": 81,
    "implemented_total": 81,
    "planned_missing": 0,
    "seed_total": 24,
    "doc_total": 7,
    "references_total": 318,
    "agents_total": 89
  },
  "downloaded_seeds": {
  ...
  "domains": [
    {
      ...
      "items": [
        ...
        },
        {
          ...
          "domain_order": 1,
          "item_order": 10,
          "auto_trigger": "【强制自动触发】当当前 Codex/Claude/agent 会话进入明确需求、Bug、实施、审查、测试、提交或其他可命名任务，且会话标题为空泛、过时、泛称或不匹配当前任务时触发。负责自动生成 8-24 字中文简要标题并调用当前环境真实线程重命名工具（Codex 使用 set_thread_title）重命名当前会话；不等待用户显式要求。任务主题尚未稳定、标题已准确、工具不可用或用户明确禁止时不改名。不要用它代替需求分析、Bug 定位、实施规划、测试验证或提交动作。",
          "core_responsibility": "负责生成 8-24 字中文简要标题，并调用当前环境真实线程重命名工具更新当前会话标题；标题已准确、工具不可用或用户明确禁止时记录跳过。",
    ...
    },
    {
      ...
      "items": [
        ...
        },
        {
          ...
          "domain_order": 2,
          "item_order": 4,
          "auto_trigger": "从对话和代码中自动提取、规范化、合并并增量更新项目事实，写入根目录 `PROJECT_MEMORY.md` 作为唯一长期记忆源。用于项目需要长期记住指标、参数、表字段、缓存键、变量、公式、方法映射或其他反复出现的信息，并且后续对话应优先读取和更新这份记忆的场景。当用户给出“根据 skill 补充更新 md / 根据规则更新 md / 按 skill 更新项目 md / 补充更新 md”等聚合指令时，本 skill 负责其中 `PROJECT_MEMORY.md` 的检测、缺失则创建、已存在则增量补齐（通常由 `project-agents-bootstrap` 统一编排联动）。",
        ...
        },
        {
          ...
          "domain_order": 2,
          "item_order": 5,
          "auto_trigger": "从对话和代码中自动提取、规范化、合并并增量更新项目代码风格示例，写入根目录 `PROJECT_STYLE.md` 作为唯一风格记忆源。用于项目需要长期记住方法、注释、类、结构体、变量、异步、日志、错误处理、接口、工具调用、循环等代码风格样例，并且后续写代码应优先参考和更新这份风格记忆的场景。当用户给出“根据 skill 补充更新 md / 根据规则更新 md / 按 skill 更新项目 md / 补充更新 md”等聚合指令时，本 skill 负责其中 `PROJECT_STYLE.md` 的检测、缺失则创建、已存在则增量补齐（通常由 `project-agents-bootstrap` 统一编排联动）。",
    ...
    },
    {
      ...
      "items": [
        ...
        },
        {
          ...
          "domain_order": 11,
          "item_order": 17,
          "auto_trigger": "当用户要求生成、补齐、刷新、维护项目 swag、更新 swag、导出 Apifox/OpenAPI/Swagger 接口文档，或需要让项目所有 HTTP 接口持续同步为 YAML 文档时触发。负责从真实路由、controller、请求 DTO、响应 DTO、统一响应包装和鉴权中间件读取接口契约，生成或更新项目根目录 swag/ 下的全量接口 OpenAPI/Swagger YAML；每个接口单独一个 YAML，同时维护一个包含所有接口的总 YAML。不要用它代替业务接口实现、接口需求设计、功能测试或线上联调。",
          "core_responsibility": "当前已在仓库中，但尚未并入主规划域表。",
  ...
  "recommendations": [
    "81 个规划 skill 已全部独立落地，后续优化优先检查 description 命中率、相邻 skill 边界和 references 的信息密度。",

  ...
  "domains": [
    {
      ...
      "items": [
        ...
        },
        {
          ...
          "domain_order": 1,
          "item_order": 2,
          "auto_trigger": "当需要定义、调整或解释项目中 `ment/`、`bug/`、`test/`、`doc/` 等研发产物根目录、命名模板、主入口文件、同任务复用策略或跨域文档引用关系时自动触发。负责提供全局唯一的目录与命名单一真相源，并为需求、Bug、测试、记忆和交付类 skill 提供统一引用基准；不要用它代替需求分析、Bug 定位、测试执行、生产代码存放决策或流程分流。",
          "core_responsibility": "作为跨域统一约定 skill，提供目录、命名和复用策略的单一真相源，供需求、Bug、测试、记忆和交付类 skill 统一引用。",
      ...
      "items": [
        {
          ...
          "domain_order": 2,
          "item_order": 1,
          "auto_trigger": "当当前会话刚开始、缺少历史上下文、用户直接提出与当前项目相关的需求、Bug、编码、测试或交付问题时自动触发。负责优先从 `artifact-storage-rules` 约定的近 3 天需求、测试、Bug、文档根目录和 git 提交中提取最近活动，并压缩成可继续工作的前置上下文；如果部分目录不存在，则只使用存在的目录和 git 信息；不要把它代替 history-recall-rules 的深度历史回忆、project-timeline-rules 的长期时间线，或当前主域本身的分析执行。",
          "core_responsibility": "负责优先从 `artifact-storage-rules` 约定目录和最近 Git 提交中压缩前置上下文，再把任务转交真正主域。",
    ...
    },
    {
      ...
      "items": [
        ...
        },
        {
          ...
          "domain_order": 5,
          "item_order": 2,
          "auto_trigger": "当新增或修改业务代码、工具代码、服务代码、脚本代码或 Bug 修复逻辑时触发。负责约束函数结构、逻辑顺序、复杂度和可维护性，并在功能不变前提下优先让最近改动代码表达更清楚、更直接；不要用它代替最小改动、风格一致性、注释规则或代码归位规则。",
      ...
      "items": [
        {
          ...
          "domain_order": 7,
          "item_order": 1,
          "auto_trigger": "当功能代码已经完成、准备进入测试前验证时触发。负责检查实现是否符合可读性优先、单一职责、命名语义化、注释完整、错误处理明确、日志可追溯、依赖使用审慎、魔法值治理、冗余逻辑清理和编码规范等实现质量要求，并在功能不变前提下检查最近改动代码是否还存在可直接收口的表达层冗余；不要用它代替语法检查、格式化执行、目录归位审查或功能验证规则。",
    ...
    },
    {
      ...
      "items": [
        ...
        },
        {
          ...
          "domain_order": 8,
          "item_order": 2,
          "auto_trigger": "当新增或修改测试目录、测试文件、测试脚本、验证程序、fixture、mock 数据、测试说明文档落点时触发。负责在 `artifact-storage-rules` 的统一约定下管理测试资源根目录、时间戳任务根目录、中文说明目录、真实代码路径镜像目录和禁止散落规则；尤其要避免把中文路径放进会被 Go 工具链编译的测试包目录。不要用它代替 test-naming-rules、test-program-rules、test-doc-rules、code-placement-review-rules 或功能验证规则。",

window.SKILL_DICTIONARY = {
  "generated_at": "2026-09-12 11:58:04",
    ...
    },
    {
      ...
      "items": [
        ...
        },
        {
          ...
          "domain_order": 5,
          "item_order": 2,
          "auto_trigger": "当新增或修改代码、调整功能、修复 Bug、补测试支撑代码或整理实现细节时触发。负责统一代码质量双主线：最小改动（约束改动范围、删除无关修改、编码前显式说明假设与成功标准、简单优先、默认反对过度封装与抽象接口）与可读性（函数结构、逻辑顺序、复杂度、副作用可见性、500 行拆分评估、DTO 结构化解析）；要求写库、发请求、改状态、发事件等副作用必须从命名和位置可见，避免 query 与 command 混成一个隐蔽职责；结构调整落地后必须联动 comment-rules 完成改动位点注释检查与补齐；不要用它代替风格一致性、注释规则、代码归位或测试规范等相邻 skill。",
      ...
      "items": [
        {
          ...
          "domain_order": 6,
          "item_order": 1,
          "auto_trigger": "用于判断前后端同仓、独立后端、独立前端项目中新增或修改目录、包、模块、数据库资产、静态数据、配置、公共结构、工具包、第三方 SDK、中间件、定时任务、缓存任务、异步任务与源码层的唯一位置、职责和依赖方向；目录查询、渲染、初始化和检查统一由本 Skill 的 Catalog 负责。",
    ...
    },
    {
      ...
      "items": [
        ...
        },
        {
          ...
          "domain_order": 10,
          "item_order": 69,
          "auto_trigger": "当需要验证/校正「生产实现」与「参考实现」行为是否一致时触发，典型场景为把某份回测、原型、脚本、旧系统或第三方算例移植成另一语言的线上实现（如 Python 回测 → Go 实盘、Excel 模型 → Java 服务、旧系统 → 新架构）。负责冻结参考语义、建立黄金回归、加参数漂移守卫、并以精确数字判定一致性；强制参考实现是唯一真值、禁止反向修改参考实现去迁就生产实现。不要用它代替 functional-validation-rules（新功能本身对不对）或 test-regression-rules（旧能力有没有被带坏）。",
          "core_responsibility": "当前已在仓库中，但尚未并入主规划域表。",

  ...
  "domains": [
    {
      ...
      "items": [
        ...
        },
        {
          ...
          "core_responsibility": "根 `test/` 是唯一活动测试代码根，`doc/5-tests/` 只保留 README 与非可执行证据；其余目录、命名和复用策略仍由该 skill 统一提供。",
    ...
    },
    {
      ...
      "items": [
        ...
        },
        {
          ...
          "domain_order": 8,
          "item_order": 5,
          "auto_trigger": "当需要验证新功能、修改后的功能、接口行为、页面交互、输入输出结果是否满足当前需求、当前变更和完成条件时触发。负责界定本次功能验证范围、验证步骤、通过驳回标准和结论留痕；必须以 `artifact-storage-rules` 与 `test-strategy-rules 的 test-asset-governance 条件路由` 为基准，把功能验证结论写回测试任务 README，把可执行测试代码、mock、stub、fake、fixture 和 helper 放入根 `test/` 的源码镜像目录，并把详细执行证据放到同时间戳 `doc/5-tests/` 的 `evidence/` 或 `artifacts/`；模拟程序与对应测试使用同一源码相对路径，`doc/5-tests/` 仅保存非可执行证据；Go 测试还必须遵循 `test-program-rules` 的《Go 测试编译路径（强制）》；同时强制禁止为了测试目的污染生产代码。不要用它代替 test-strategy-rules、test-strategy-rules 的 test-asset-governance 条件路由或 test-regression-rules。",
        ...
        },
        {
          ...
          "domain_order": 8,
          "item_order": 6,
          "auto_trigger": "当 Bug 修复、原有功能迭代、公共模块修改、共享逻辑调整、接口兼容性变化后准备执行测试时触发。负责判定回归范围、选择回归用例、验证兼容性影响并输出回归结论；必须以 `artifact-storage-rules` 与 `test-strategy-rules 的 test-asset-governance 条件路由` 为基准，把回归结论写回测试任务 README，把可执行回归测试、mock、stub、fake、fixture 和 helper 放入根 `test/` 的源码镜像目录，并把详细回归案例、执行证据和补充说明放到同时间戳 `doc/5-tests/` 的 `evidence/` 或 `artifacts/`；模拟程序与对应回归测试使用同一源码相对路径，`doc/5-tests/` 仅保存非可执行证据；Go 测试还必须遵循 `test-program-rules` 的《Go 测试编译路径（强制）》；同时强制禁止为了测试目的污染生产代码。不要用它代替 functional-validation-rules、test-strategy-rules 或测试资源管理类规则。",
    ...
    },
    {
      ...
      "items": [
        ...
        },
        {
          ...
          "domain_order": 10,
          "item_order": 7,
          "auto_trigger": "当需求、实施、Bug、真实测试或 6-review 风格回归任务准备最终收口，且本轮已经产生或应当产生持久化研发文档时自动触发。负责在最终完成前核对文档是否已经真实落盘到 `artifact-storage-rules` 约定的位置，检查主入口文件、必需配套文件和同任务复用关系是否完整；若文档仍停留在最终回复、临时说明或内存结论中，必须阻断收口并先补齐落盘。适用于需求主文档、实施文档、Bug 根目录、测试任务 README 与 `doc/6-review` 风格回归记录；历史 `doc/6-审查` 与 `doc/7-验收` 仅只读兼容，不代替需求分析、Bug 定位或真实测试本身。",
          "core_responsibility": "当前已在仓库中，但尚未并入主规划域表。",

...
DOMAIN_ORDER = [
    ...
    DomainConfig("style_regression", "代码风格回归域", "真实测试后的格式、写法、位置与既有风格回归"),
...
def build_focus_points(item: dict) -> list[str]:
    """构建单个 Skill 在字典中展示的维护关注点。

    [参数] item: 已归类的 Skill 字典条目。
    [返回] list[str]：该条目在字典中展示的关注点。
    最近修改时间：2026-08-01 02:10:52；编码审查域改为代码风格回归域。
    """
    # 1. 按条目状态和领域生成不越界的维护关注点。
    status = item["status"]
    ...
    domain_points = {
        ...
        "编码基线域": "重点看它是否能并行生效，并且不抢位点域或风格回归域职责。",
        "代码位点域": "重点看主位点与横切位点是否可并行叠加，而不是互相覆盖。",
        "代码风格回归域": "重点看它只在真实测试后检查写法、位置与格式，不越界替代功能测试。",

...
- **策略回测**：均线交叉、RSI 均值回归、布林带策略；支持策略迭代闭环（预览→确认→回测→归因→改法→再跑）
...
回测不是跑一次就结束的。**真正的价值在于迭代。**

### 完整闭环

```
基线 → 假设（只改一类）→ 预览确认 → 回测 → 拉执行历史 + 日志
  → 成败归因 → 1～3 条改法 → 再跑
```

### 每轮迭代的规范

1. **基线**：先跑一轮得到当前策略的完整指标（收益/夏普/回撤/胜率）
2. **假设**：选一个变量或一段逻辑改动，陈述预期（"如果我把 RSI 入场阈值从 30 改为 25，预期减少假信号"）
3. **预览确认**：向用户展示改动的参数和预期效果，确认后再发起回测
4. **回测**：执行 backtest.py
5. **归因分析**：解读结果——赚在哪、亏在哪、规则是否按预期触发、参数是否稳定
6. **改法**：建议 1～3 条可再跑改法，并说明预期效果
7. **再跑**：进入下一轮迭代
...
| 维度 | 解读内容 | 示例 |
|------|---------|------|
| **总览** | 收益/夏普/回撤/胜率/交易次数 | "年化收益 52%，夏普 1.21，最大回撤 -28%" |
| **偏差** | 数据偏差、逻辑偏差、参数敏感度 | "回撤集中在 2022 年 5 月，与 LUNA 崩盘同步" |
| **归因** | 收益来源分解、规则触发分析 | "Top 1 标的 BTC 贡献了 60% 收益，存在集中度风险" |
| **改法** | 1～3 条具体改进方向 | "建议将止损从 -15% 收紧到 -10%，预期减少最大回撤" |

### 关键纪律

- **一次只改一类**：同时改多个变量会导致无法归因——赚了不知道哪个改对了，亏了不知道哪个改错了
- **先确认再跑**：未获用户确认前，禁止直接发起回测
- **归因用数据说话**：不要凭感觉，要引用执行历史和日志作为证据
- **不得承诺未来收益**：回测结果只能说明"历史表现"，不构成投资建议

详细参考：`references/strategy-iteration-methodology.md`
...
### Q5: 回测结果可靠吗？可以照着做吗？
**A:** 回测是基于历史数据的模拟，默认不含手续费和滑点（实际交易会有成本）。回测结果只能说明 "如果在过去用这套策略，结果会如何"，不能预测未来。建议：
- 把回测作为**策略筛选方向**的工具，而不是交易指令
- 多轮迭代验证：跑一轮→归因→改参数→再跑，看规律是否稳定
- 用**归因分析**（`references/strategy-iteration-methodology.md`）深入理解赚/亏在哪
- 再用模拟盘或小额实盘验证

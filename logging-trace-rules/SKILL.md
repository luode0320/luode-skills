...
- 通过：能明确日志框架、配置来源、初始化顺序（LoadConfig 后初始化且仅一次）、日志类型、关键字段、记录点、trace 透传方式和脱敏边界；业务日志默认中文表达，仅少数协议字段/固定 key/第三方原文保留例外；本轮补日志请求时，所有改动位点的日志都已检查完毕。
- 驳回：使用控制台打印替代正式 logger、缺少配置化管理、配置加载前初始化 logger、重复初始化、使用空配置预初始化、日志类型混乱、字段随意、trace 断链、敏感信息裸打、业务日志长期英文化，或把临时 debug 日志当成长期正式日志，或补日志请求只改了一部分日志位点。

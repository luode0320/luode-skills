...
- 日志字段必须包含可反查的稳定标识（request-id / trace-id / 订单号等），标识命名稳定、可被 grep 定位；该标识用于读日志侧按标识反查一次请求全链路（读侧取证步骤见 `log-analysis-rules` 的 request-id-traceback）。
- 纯本地任务、无外部请求上下文的日志，可省略 request-id，但必须使用业务键（订单号、任务 ID 等）作为可反查标识，不允许整条日志无任何可反查字段。
...
8. 明确可反查标识字段（request-id / trace-id / 订单号）是否已透传并落入日志；缺失时补齐。
9. 确定日志只记录必要上下文，不做无意义刷屏。
10. 按日志排版规范收口代码样式：能一行则一行；必须换行则“模板一行 + 参数下一行（必要时参数续行）”。
11. 如果本轮改动文件包含日志调用，先扫描这些文件中的日志文本，发现英文业务日志、无业务前缀、裸打敏感字段或控制台打印时，必须在当前文件范围内同步修正。
12. 如果本轮是补日志请求，必须逐一检查本轮所有改动位点的日志，不允许只改一部分最显眼的日志。
13. 如果是补日志请求，只收敛当前未提交的改动文件，未变动文件即使存在日志问题也不在本轮范围内。
...
- 不允许日志缺少可反查稳定标识（request-id / trace-id / 订单号等）；读日志侧的取证与反查归 `log-analysis-rules`，本 skill 只负责写侧字段。
...
- 通过：能明确日志框架、配置来源、初始化顺序（LoadConfig 后初始化且仅一次）、日志类型、关键字段、记录点、trace 透传方式和脱敏边界；业务日志默认中文表达，仅少数协议字段/固定 key/第三方原文保留例外；业务日志统一以 `[业务简要说明]` 前缀开头；日志含可反查稳定标识（request-id / trace-id / 订单号）；本轮改动文件内与改动上下文相关的既有日志已复核；本轮补日志请求时，当前未提交改动文件的日志都已检查完毕，未变动文件未被纳入。
...
- 如果本次定义了日志字段规范、trace 透传方案或审计边界，将结论记录到 `analysis/` 或 `doc/6-review/`。

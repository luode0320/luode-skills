...
9. 产出本轮“代码风格契约”，把 active 反例、局部风格证据、接口参考实现、最小新增内容和不得新增内容一并写入契约，再开始写代码。

## 默认执行流程

1. 默认先读 [pre-coding-checklist.md](references/pre-coding-checklist.md)，确认编码前需要读取的风格来源，其中包含全局用户风格反例库。
2. 再读 [local-context-and-interface-style.md](references/local-context-and-interface-style.md)，识别局部统一风格和接口实现参考样例。
3. 再读 [style-priority.md](references/style-priority.md)，处理用户要求、当前文件、同目录、`PROJECT_STYLE.md` 与通用规则的冲突。
4. 按 [style-contract-template.md](references/style-contract-template.md) 组织本轮代码风格契约。
5. 写代码时把契约作为当前轮实现约束，不把契约当成可选建议；局部已有稳定模板时只做必要替换，不增加无需求代码。
6. 写完后读 [post-change-style-gate.md](references/post-change-style-gate.md)，逐项核对是否满足契约；若本轮代码命中全局反例库中任一 active 反例，或偏离局部模板/既有接口实现，驳回并按正例改写。
7. 若本轮形成新的稳定团队偏好，联动 `project-style-rules` 回写 `PROJECT_STYLE.md`；若只是一次性局部跟随，不回写长期风格。
...
- 局部风格证据：当前文件、同目录或同模块中可直接复用的稳定代码片段及其适用范围。
- 接口参考实现：接口名、参考文件/符号、采用的结构，以及与目标实现的必要差异。
- 最小新增内容：只允许为完成当前目标新增的字段、调用、分支、注释或错误处理。
- 禁用写法：本轮明确不能引入的抽象、模板化写法、过度防御、风格跳变或超出局部模板的额外代码；以及全局用户风格反例库中所有 active 反例对应的写法。
...
- 实现接口但找不到任何可参考实现，且当前文件/同目录也无法提供稳定降级风格。
- 多个接口实现样例存在结构冲突，无法按最近同模块和稳定多数原则收敛。
- 要满足风格契约必须扩大到无关文件或批量格式化。
- `PROJECT_STYLE.md` 中存在过时、冲突或不适用于当前语言的条目，且缺少更高优先级依据。

## 执行通过 / 驳回标准

- 通过：写代码前已读取相关风格来源并形成本轮代码风格契约；改动后与契约、当前文件和同目录样例一致；统一局部上下文只做必要模板替换；接口实现记录了既有实现参考或明确的降级依据；新的稳定偏好已联动 `project-style-rules` 回写或明确无需回写。
- 驳回：直接开始写代码而未形成契约；把 `PROJECT_STYLE.md` 当成唯一来源忽略当前文件；未查找既有接口实现；在统一局部上下文中新增 helper、变量、循环、日志、校验、抽象或其他无需求代码；把个人偏好、外部模板或一次性口头建议写成项目风格；借风格统一扩大无关改动。

## references 读取规则

- 默认先读 [pre-coding-checklist.md](references/pre-coding-checklist.md)。
- 只要任务涉及局部风格识别或接口实现，必须读 [local-context-and-interface-style.md](references/local-context-and-interface-style.md)。

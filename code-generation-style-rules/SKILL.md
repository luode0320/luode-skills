...
4. 识别本轮已经命中的编码类 skill，例如 `code-minimal-change-rules`、`code-readability-rules`、`code-style-consistency-rules`、`naming-rules`、注释类 skill、日志或错误处理类 skill。
5. 按 [style-priority.md](references/style-priority.md) 决定冲突优先级。
6. 产出本轮“代码风格契约”，再开始写代码。

## 默认执行流程

1. 默认先读 [pre-coding-checklist.md](references/pre-coding-checklist.md)，确认编码前需要读取的风格来源。
...
5. 写完后读 [post-change-style-gate.md](references/post-change-style-gate.md)，逐项核对是否满足契约。
...
- 禁用写法：本轮明确不能引入的抽象、模板化写法、过度防御或风格跳变。

## 与相邻 Skill 的边界

- `project-style-rules` 维护 `PROJECT_STYLE.md`，负责长期风格记忆的抽取、合并、废弃和回写；本 skill 负责在写代码前应用这些风格记忆。
- `code-style-consistency-rules` 检查本次改动是否与本轮风格契约和局部既有写法一致；本 skill 先生成契约。

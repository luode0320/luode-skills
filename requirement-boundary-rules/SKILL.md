...
- 防止把历史遗留缺陷混入当前需求一起做。
- 在边界未裁清前，阻断进入 `acceptance-criteria-rules` 与 `implementation-plan-rules`。
...
7. 同步更新文档中的 Mermaid 图和关键边界表，保证图文口径一致。
8. 如果问题不属于当前需求范围，不要在需求域内硬做下去。
9. 在边界结论未稳定前，不进入 `acceptance-criteria-rules` 或 `implementation-plan-rules`。
